...
Prims_list__uint32_t *Txout___proj__Mktxout__item__value(Txout_txout projectee)
{
  return projectee.value;
}

FStar_Bytes_bytes Txout___proj__Mktxout__item__scriptPubKey(Txout_txout projectee)
{
  return projectee.scriptPubKey;
}

uint32_t Txout_txout_validator(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos1 = Amount_amount_validator(input, pos);
  if (pos1 > LOWPARSE_LOW_BASE_VALIDATOR_MAX_LENGTH)
    return pos1;
  else
    return Txout_scriptPubKey_txout_scriptPubKey_validator(input, pos1);
}

uint32_t Txout_txout_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return Txout_scriptPubKey_txout_scriptPubKey_jumper(input, Amount_amount_jumper(input, pos));
}

uint32_t Txout_accessor_txout_value(LowParse_Slice_slice input, uint32_t pos)
{
  return pos;
}

uint32_t Txout_accessor_txout_scriptPubKey(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
  uint32_t res = Amount_amount_jumper(input, pos21);
  uint32_t pos3 = res;
  uint32_t pos30 = pos3;
  return pos30;
}
