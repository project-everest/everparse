...
T5_t5_parser32(FStar_Bytes_bytes input)
{
...
FStar_Bytes_bytes T5_t5_serializer32(Prims_list__FStar_Bytes_bytes *input)
{
  ...
  while (b)
  {
    ...
    else if (input_->tag == Prims_Cons)
    {
      ...
      FStar_Bytes_bytes sa = T3_t3_serializer32(a);
...
uint32_t T5_t5_size32(Prims_list__FStar_Bytes_bytes *input)
{
  bool b = true;
...
uint32_t T5_t5_validator(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t res;
      ...
      else
      {
        ...
        while (true)
        {
          ...
          else
          {
            uint32_t
            pos11 =
              T3_t3_validator(((LowParse_Slice_slice){ .base = input.base, .len = res + va }),
                pos1);
            bpos = pos11;
...
uint32_t T5_t5_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return pos + (uint32_t)1U + LowParse_Low_BoundedInt_read_bounded_integer_1(input, pos);
}

uint32_t T5_t5_nth(uint32_t i, LowParse_Slice_slice sl, uint32_t pos)
{
  return pos + (uint32_t)1U + i * (uint32_t)2U;
}
