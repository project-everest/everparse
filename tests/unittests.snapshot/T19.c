...
FStar_Pervasives_Native_option__K___T19_t19__uint32_t
T19_t19_parser32(Tag_tag k, FStar_Bytes_bytes input)
{
  ...
  {
    FStar_Pervasives_Native_option__K___T13_t13_uint32_t scrut = T13_t13_parser32(input);
      ...
      return
        (
          (FStar_Pervasives_Native_option__K___T19_t19__uint32_t){
            ...
            .v = { .fst = { .tag = T19_X_b, ._0 = v1 }, .snd = consumed }
          }
        );
    }
    else
...
FStar_Bytes_bytes T19_t19_serializer32(Tag_tag k, T19_t19_ input)
{
  if (__eq__Tag_tag(Tag_A, k))
  ...
  {
    T13_t13 x;
    if (input.tag == T19_X_b)
      x = input._0;
    else
      x = KRML_EABORT(T13_t13, "unreachable (pattern matches are exhaustive in F*)");
    return T13_t13_serializer32(x);
  }
}

uint32_t T19_t19_size32(Tag_tag k, T19_t19_ input)
{
  if (__eq__Tag_tag(Tag_A, k))
    return (uint32_t)0U;
  else
