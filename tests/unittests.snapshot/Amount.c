...
uint32_t Amount_amount_validator(LowParse_Slice_slice input, uint32_t pos)
{
  ...
  else
  {
    ...
    while (true)
    {
      ...
      {
        uint32_t pos11;
        if (pos + (uint32_t)8U - pos1 < (uint32_t)4U)
...
uint32_t Amount_amount_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return pos + (uint32_t)8U;
}

uint32_t Amount_amount_nth(uint32_t i, LowParse_Slice_slice input, uint32_t pos)
{
  return pos + i * (uint32_t)4U;
}
