...
uint32_t Block___proj__Mkblock__item__version(Block_block projectee)
{
  return projectee.version;
}

FStar_Bytes_bytes Block___proj__Mkblock__item__prev_block(Block_block projectee)
{
  return projectee.prev_block;
}

FStar_Bytes_bytes Block___proj__Mkblock__item__merkle_root(Block_block projectee)
{
  return projectee.merkle_root;
}

uint32_t Block___proj__Mkblock__item__timestamp(Block_block projectee)
{
  return projectee.timestamp;
}

uint32_t Block___proj__Mkblock__item__bits(Block_block projectee)
{
  return projectee.bits;
}

uint32_t Block___proj__Mkblock__item__nonce(Block_block projectee)
{
  return projectee.nonce;
}

Prims_list__Transaction_transaction *Block___proj__Mkblock__item__tx(Block_block projectee)
{
  return projectee.tx;
}

uint32_t Block_block_validator(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos10;
  if (input.len - pos < (uint32_t)4U)
    pos10 = LOWPARSE_LOW_BASE_VALIDATOR_ERROR_NOT_ENOUGH_DATA;
  else
    pos10 = pos + (uint32_t)4U;
  uint32_t pos12;
  if (pos10 > LOWPARSE_LOW_BASE_VALIDATOR_MAX_LENGTH)
    pos12 = pos10;
  else
    pos12 = Sha256_sha256_validator(input, pos10);
  uint32_t pos1;
  if (pos12 > LOWPARSE_LOW_BASE_VALIDATOR_MAX_LENGTH)
    pos1 = pos12;
  else
  {
    uint32_t pos11 = Sha256_sha256_validator(input, pos12);
  ...
  else
  {
    ...
    if (pos110 > LOWPARSE_LOW_BASE_VALIDATOR_MAX_LENGTH)
      pos11 = pos110;
    else if (input.len - pos110 < (uint32_t)4U)
      pos11 = LOWPARSE_LOW_BASE_VALIDATOR_ERROR_NOT_ENOUGH_DATA;
    else
      pos11 = pos110 + (uint32_t)4U;
    if (pos11 > LOWPARSE_LOW_BASE_VALIDATOR_MAX_LENGTH)
      return pos11;
    else
      return Block_tx_block_tx_validator(input, pos11);
  }
}

uint32_t Block_block_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return
    Block_tx_block_tx_jumper(input,
      Sha256_sha256_jumper(input,
...
uint32_t Block_accessor_block_version(LowParse_Slice_slice input, uint32_t pos)
{
  return pos;
}

uint32_t Block_accessor_block_prev_block(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
  return pos21 + (uint32_t)4U;
}

uint32_t Block_accessor_block_merkle_root(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
...
uint32_t Block_accessor_block_timestamp(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
  uint32_t res = Sha256_sha256_jumper(input, pos21 + (uint32_t)4U);
  uint32_t pos3 = res;
  uint32_t pos210 = pos3;
  uint32_t res0 = Sha256_sha256_jumper(input, pos210);
  uint32_t pos30 = res0;
  uint32_t pos31 = pos30;
  return pos31;
}

uint32_t Block_accessor_block_bits(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
  uint32_t
  res =
    Sha256_sha256_jumper(input,
      Sha256_sha256_jumper(input, pos21 + (uint32_t)4U))
    + (uint32_t)4U;
  uint32_t pos3 = res;
  uint32_t pos210 = pos3;
...
uint32_t Block_accessor_block_nonce(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
  uint32_t
  ...
  uint32_t pos210 = pos3;
  uint32_t pos211 = pos210;
  uint32_t pos30 = pos211 + (uint32_t)4U;
  return pos30;
}

uint32_t Block_accessor_block_tx(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
  uint32_t
  ...
  uint32_t pos3 = res;
  uint32_t pos210 = pos3;
  uint32_t pos30 = pos210 + (uint32_t)4U + (uint32_t)4U;
  return pos30;
}
