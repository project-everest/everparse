...
T3_t3_parser32(FStar_Bytes_bytes input)
{
    ...
    return
      (
        (FStar_Pervasives_Native_option__K___FStar_Bytes_bytes_uint32_t){
          ...
          .v = { .fst = s_, .snd = (uint32_t)2U }
        }
      );
  }
}

FStar_Bytes_bytes T3_t3_serializer32(FStar_Bytes_bytes input)
{
  return input;
}

uint32_t T3_t3_size32(FStar_Bytes_bytes x)
{
  return (uint32_t)2U;
}

uint32_t T3_t3_validator(LowParse_Slice_slice input, uint32_t pos)
{
...
uint32_t T3_t3_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return pos + (uint32_t)2U;
}
