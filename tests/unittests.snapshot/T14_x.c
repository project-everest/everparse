...
T14_x_t14_x_parser32(FStar_Bytes_bytes input)
{
  ...
  {
    FStar_Bytes_bytes s_ = FStar_Bytes_slice(input, (uint32_t)0U, (uint32_t)18U);
    return
      (
        (FStar_Pervasives_Native_option__K___FStar_Bytes_bytes_uint32_t){
          .tag = FStar_Pervasives_Native_Some,
          .v = { .fst = s_, .snd = (uint32_t)18U }
        }
      );
  }
}

FStar_Bytes_bytes T14_x_t14_x_serializer32(FStar_Bytes_bytes input)
{
  return input;
}

uint32_t T14_x_t14_x_size32(FStar_Bytes_bytes x)
{
  return (uint32_t)18U;
}
