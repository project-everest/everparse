...
bool T18_uu___is_X_a(T18_t18 projectee)
{
  if (projectee.tag == T18_X_a)
    return true;
  else
    return false;
}

bool T18_uu___is_X_b(T18_t18 projectee)
{
  if (projectee.tag == T18_X_b)
    return true;
  else
    return false;
}

T13_t13 T18___proj__X_b__item___0(T18_t18 projectee)
{
  if (projectee.tag == T18_X_b)
    return projectee._0;
  else
...
Tag_tag T18_tag_of_t18(T18_t18 x)
{
  if (x.tag == T18_X_a)
    return Tag_A;
  else if (x.tag == T18_X_b)
    return Tag_B;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
...
FStar_Pervasives_Native_option__K___T18_t18_uint32_t T18_t18_parser32(FStar_Bytes_bytes input)
{
  ...
  if (pi.tag == FStar_Pervasives_Native_None)
    return
      (
        (FStar_Pervasives_Native_option__K___T18_t18_uint32_t){
          .tag = FStar_Pervasives_Native_None
        }
      );
  else if (pi.tag == FStar_Pervasives_Native_Some)
  {
    ...
    {
      FStar_Pervasives_Native_option__K______uint32_t scrut = T18_x_a_t18_x_a_parser32(input_k);
      FStar_Pervasives_Native_option__K___T18_t18_uint32_t pcases2;
      if (scrut.tag == FStar_Pervasives_Native_Some)
      {
        uint32_t consumed = scrut.v;
        pcases2 =
          (
            (FStar_Pervasives_Native_option__K___T18_t18_uint32_t){
              .tag = FStar_Pervasives_Native_Some,
              .v = { .fst = { .tag = T18_X_a }, .snd = consumed }
            }
          );
      }
      else
    ...
    {
      FStar_Pervasives_Native_option__K___T13_t13_uint32_t
      scrut = T18_x_b_t18_x_b_parser32(input_k);
      FStar_Pervasives_Native_option__K___T18_t18_uint32_t pcases2;
      if (scrut.tag == FStar_Pervasives_Native_Some)
      {
        uint32_t consumed = scrut.v.snd;
        ...
        pcases2 =
          (
            (FStar_Pervasives_Native_option__K___T18_t18_uint32_t){
              .tag = FStar_Pervasives_Native_Some,
              .v = { .fst = { .tag = T18_X_b, ._0 = v1 }, .snd = consumed }
            }
          );
      }
      else
...
FStar_Bytes_bytes T18_t18_serializer32(T18_t18 x)
{
  Tag_tag tg;
  if (x.tag == T18_X_a)
    tg = Tag_A;
  ...
  if (__eq__Tag_tag(Tag_A, tg))
    s2 = T18_x_a_t18_x_a_serializer32();
  else
  {
    T13_t13 ite;
    if (x.tag == T18_X_b)
      ...
      ite = KRML_EABORT(T13_t13, "unreachable (pattern matches are exhaustive in F*)");
    s2 = T18_x_b_t18_x_b_serializer32(ite);
  }
  FStar_Bytes_bytes res = FStar_Bytes_append(s1, s2);
  return res;
}

uint32_t T18_t18_size32(T18_t18 x)
{
  Tag_tag tg;
  if (x.tag == T18_X_a)
    tg = Tag_A;
  ...
  else
  {
    ...
    if (x.tag == T18_X_b)
      ...
      ite = KRML_EABORT(T13_t13, "unreachable (pattern matches are exhaustive in F*)");
    s2 = T18_x_b_t18_x_b_size32(ite);
  }
  return s1 + s2;
}

uint32_t T18_t18_validator(LowParse_Slice_slice input, uint32_t pos)
{
  ...
  else
  {
    ...
    if (k_ == (uint8_t)0U)
      return T18_x_a_t18_x_a_validator(input, len_after_tag);
    else if (k_ == (uint8_t)1U)
      return T18_x_b_t18_x_b_validator(input, len_after_tag);
    else
      return LOWPARSE_LOW_BASE_VALIDATOR_ERROR_GENERIC;
  }
}

uint32_t T18_t18_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos_after_tag = pos + (uint32_t)1U;
  uint8_t k_ = LowParse_Low_Int_read_u8(input, pos);
  if (k_ == (uint8_t)0U)
    return T18_x_a_t18_x_a_jumper(input, pos_after_tag);
  else if (k_ == (uint8_t)1U)
    return T18_x_b_t18_x_b_jumper(input, pos_after_tag);
  else
    return (uint32_t)0U;
}

uint32_t T18_t18_accessor_a(LowParse_Slice_slice input, uint32_t pos)
{
  return pos + (uint32_t)1U;
}

uint32_t T18_t18_accessor_b(LowParse_Slice_slice input, uint32_t pos)
{
  return pos + (uint32_t)1U;
}
