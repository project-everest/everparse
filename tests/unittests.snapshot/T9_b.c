...
T9_b_t9_b_parser32(FStar_Bytes_bytes input)
{
  ...
  if (scrut1.tag == FStar_Pervasives_Native_Some)
  {
    ...
    {
      FStar_Pervasives_Native_option__K___Prims_list__FStar_Bytes_bytes_uint32_t
      scrut = T5_t5_parser32(FStar_Bytes_slice(input_, (uint32_t)0U, v1));
...
FStar_Bytes_bytes T9_b_t9_b_serializer32(Prims_list__FStar_Bytes_bytes *input)
{
  FStar_Bytes_bytes pl = T5_t5_serializer32(input);
...
uint32_t T9_b_t9_b_size32(Prims_list__FStar_Bytes_bytes *input)
{
  uint32_t len1 = T5_t5_size32(input);
  return (uint32_t)2U + len1;
}

uint32_t T9_b_t9_b_validator(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t res;
  ...
  {
    uint32_t va = LowParse_Low_BoundedInt_read_bounded_integer_2(input, pos);
    if (!(va < (uint32_t)0U || (uint32_t)65535U < va))
      if (input.len - res < va)
        return LOWPARSE_LOW_BASE_VALIDATOR_ERROR_NOT_ENOUGH_DATA;
      else
      {
        uint32_t
        pos_ = T5_t5_validator(((LowParse_Slice_slice){ .base = input.base, .len = res + va }), res);
...
uint32_t T9_b_t9_b_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return pos + (uint32_t)2U + LowParse_Low_BoundedInt_read_bounded_integer_2(input, pos);
}

uint32_t T9_b_t9_b_accessor(LowParse_Slice_slice input, uint32_t pos)
{
