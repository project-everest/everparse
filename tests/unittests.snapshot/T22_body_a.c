...
T22_body_a_t22_body_a_parser32(FStar_Bytes_bytes input)
{
  FStar_Pervasives_Native_option__K___uint32_t_uint32_t
  scrut0 = LowParse_SLow_BoundedInt_parse32_bounded_integer_3(input);
  ...
  if (scrut0.tag == FStar_Pervasives_Native_Some)
  {
    ...
    if (!(v1 < (uint32_t)4U || (uint32_t)259U < v1))
      scrut1 =
        (
          (FStar_Pervasives_Native_option__K___uint32_t_uint32_t){
...
FStar_Bytes_bytes T22_body_a_t22_body_a_serializer32(FStar_Bytes_bytes input)
{
  FStar_Bytes_bytes x = input;
  FStar_Bytes_bytes pl = x;
  uint32_t len1 = FStar_Bytes_len(pl);
  FStar_Bytes_bytes slen = LowParse_SLow_BoundedInt_serialize32_bounded_integer_3(len1);
  FStar_Bytes_bytes res = FStar_Bytes_append(slen, pl);
  return res;
}

uint32_t T22_body_a_t22_body_a_size32(FStar_Bytes_bytes input)
{
  uint32_t res = FStar_Bytes_len(input);
  uint32_t len1 = res;
  return (uint32_t)3U + len1;
}

uint32_t T22_body_a_t22_body_a_validator(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t res;
  ...
  {
    uint32_t va = LowParse_Low_BoundedInt_read_bounded_integer_3(input, pos);
    if (!(va < (uint32_t)4U || (uint32_t)259U < va))
      if (input.len - res < va)
...
uint32_t T22_body_a_t22_body_a_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return pos + (uint32_t)3U + LowParse_Low_BoundedInt_read_bounded_integer_3(input, pos);
}
