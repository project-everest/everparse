...
FStar_Pervasives_Native_option__K______uint32_t T28_t28_parser32(FStar_Bytes_bytes input)
{
  return
    (
...
FStar_Bytes_bytes T28_t28_serializer32()
{
  return FStar_Bytes_empty_bytes;
}

uint32_t T28_t28_size32()
{
  return (uint32_t)0U;
}

uint32_t T28_t28_validator(LowParse_Slice_slice input, uint32_t pos)
{
  if (input.len - pos < (uint32_t)0U)
    return LOWPARSE_LOW_BASE_VALIDATOR_ERROR_NOT_ENOUGH_DATA;
  else
    return pos + (uint32_t)0U;
}

uint32_t T28_t28_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return pos + (uint32_t)0U;
}
