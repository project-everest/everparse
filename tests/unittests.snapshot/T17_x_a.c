...
T17_x_a_t17_x_a_parser32(FStar_Bytes_bytes input)
{
...
FStar_Bytes_bytes T17_x_a_t17_x_a_serializer32()
{
...
uint32_t T17_x_a_t17_x_a_size32()
{
  uint32_t len1 = (uint32_t)0U;
  return (uint32_t)2U + len1;
}

uint32_t T17_x_a_t17_x_a_validator(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t res;
  ...
  else
  {
    ...
    if (!(va < (uint32_t)0U || (uint32_t)65535U < va))
      ...
      {
        uint32_t pos_;
        if (res + va - res < (uint32_t)0U)
          pos_ = LOWPARSE_LOW_BASE_VALIDATOR_ERROR_NOT_ENOUGH_DATA;
        else
          pos_ = res + (uint32_t)0U;
...
uint32_t T17_x_a_t17_x_a_jumper(LowParse_Slice_slice input, uint32_t pos)
{
