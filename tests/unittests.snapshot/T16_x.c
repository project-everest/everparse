...
FStar_Pervasives_Native_option__K___T16_x_t16_x_uint32_t
T16_x_t16_x_parser32(FStar_Bytes_bytes input)
{
  ...
  else if (pi.tag == FStar_Pervasives_Native_Some)
  {
    ...
    if (__eq__Tag_tag(Tag_A, k))
    {
      ...
      pcases2 =
        {
          .tag = FStar_Pervasives_Native_Some,
          .v = { .fst = { .tag = T16_x_X_a }, .snd = (uint32_t)0U }
        };
    ...
    {
      FStar_Pervasives_Native_option__K___T13_t13_uint32_t scrut = T13_t13_parser32(input_k);
      FStar_Pervasives_Native_option__K___T16_x_t16_x_uint32_t pcases2;
      if (scrut.tag == FStar_Pervasives_Native_Some)
      {
        uint32_t consumed = scrut.v.snd;
        ...
        pcases2 =
          (
            (FStar_Pervasives_Native_option__K___T16_x_t16_x_uint32_t){
              .tag = FStar_Pervasives_Native_Some,
              .v = { .fst = { .tag = T16_x_X_b, ._0 = v1 }, .snd = consumed }
            }
          );
      }
      else
...
FStar_Bytes_bytes T16_x_t16_x_serializer32(T16_x_t16_x x)
{
  Tag_tag tg;
  if (x.tag == T16_x_X_a)
    tg = Tag_A;
  ...
  else
  {
    ...
    s2 = T13_t13_serializer32(ite);
  }
  FStar_Bytes_bytes res = FStar_Bytes_append(s1, s2);
  return res;
}

uint32_t T16_x_t16_x_size32(T16_x_t16_x x)
{
  Tag_tag tg;
  if (x.tag == T16_x_X_a)
    tg = Tag_A;
...
uint32_t T16_x_t16_x_validator(LowParse_Slice_slice input, uint32_t pos)
{
  ...
  else
  {
    ...
    if (k_ == (uint8_t)0U)
      if (input.len - len_after_tag < (uint32_t)0U)
        return LOWPARSE_LOW_BASE_VALIDATOR_ERROR_NOT_ENOUGH_DATA;
      else
        return len_after_tag + (uint32_t)0U;
    else if (k_ == (uint8_t)1U)
      return T13_t13_validator(input, len_after_tag);
    else
...
uint32_t T16_x_t16_x_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  ...
  if (k_ == (uint8_t)0U)
    ...
    return T13_t13_jumper(input, pos_after_tag);
  else
    return (uint32_t)0U;
}
