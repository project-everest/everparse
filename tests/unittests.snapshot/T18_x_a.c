...
FStar_Pervasives_Native_option__K______uint32_t
T18_x_a_t18_x_a_parser32(FStar_Bytes_bytes input)
{
  ...
  if (scrut1.tag == FStar_Pervasives_Native_Some)
  {
    ...
    FStar_Pervasives_Native_option__K______uint32_t scrut;
    if (FStar_Bytes_len(input_) < v1)
      scrut =
        ((FStar_Pervasives_Native_option__K______uint32_t){ .tag = FStar_Pervasives_Native_None });
    else
      scrut =
        ((FStar_Pervasives_Native_option__K______uint32_t){ .tag = FStar_Pervasives_Native_None });
...
FStar_Bytes_bytes T18_x_a_t18_x_a_serializer32()
{
  FStar_Bytes_bytes pl = FStar_Bytes_empty_bytes;
...
uint32_t T18_x_a_t18_x_a_size32()
{
  uint32_t len1 = (uint32_t)0U;
  return (uint32_t)1U + len1;
}

uint32_t T18_x_a_t18_x_a_validator(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t res;
  ...
  else
  {
    ...
    if (!(va < (uint32_t)0U || (uint32_t)255U < va))
      ...
      {
        uint32_t pos_ = LOWPARSE_LOW_BASE_VALIDATOR_ERROR_GENERIC;
...
uint32_t T18_x_a_t18_x_a_jumper(LowParse_Slice_slice input, uint32_t pos)
{
