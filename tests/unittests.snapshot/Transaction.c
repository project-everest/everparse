...
uint32_t Transaction___proj__Mktransaction__item__version(Transaction_transaction projectee)
{
  return projectee.version;
}

Prims_list__Txin_txin
*Transaction___proj__Mktransaction__item__inputs(Transaction_transaction projectee)
{
  return projectee.inputs;
}

Prims_list__Txout_txout
*Transaction___proj__Mktransaction__item__outputs(Transaction_transaction projectee)
{
  return projectee.outputs;
}

uint32_t Transaction___proj__Mktransaction__item__lock_time(Transaction_transaction projectee)
{
  return projectee.lock_time;
}

uint32_t Transaction_transaction_validator(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos10;
  ...
  if (pos10 > LOWPARSE_LOW_BASE_VALIDATOR_MAX_LENGTH)
    ...
    pos1 = Transaction_inputs_transaction_inputs_validator(input, pos10);
  if (pos1 > LOWPARSE_LOW_BASE_VALIDATOR_MAX_LENGTH)
    return pos1;
  else
  {
    uint32_t pos11 = Transaction_outputs_transaction_outputs_validator(input, pos1);
...
uint32_t Transaction_transaction_jumper(LowParse_Slice_slice input, uint32_t pos)
{
  return
    Transaction_outputs_transaction_outputs_jumper(input,
      Transaction_inputs_transaction_inputs_jumper(input, pos + (uint32_t)4U))
    + (uint32_t)4U;
}

uint32_t Transaction_accessor_transaction_version(LowParse_Slice_slice input, uint32_t pos)
{
  return pos;
}

uint32_t Transaction_accessor_transaction_inputs(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
...
uint32_t Transaction_accessor_transaction_outputs(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
  uint32_t res = Transaction_inputs_transaction_inputs_jumper(input, pos21 + (uint32_t)4U);
  uint32_t pos3 = res;
...
uint32_t Transaction_accessor_transaction_lock_time(LowParse_Slice_slice input, uint32_t pos)
{
  uint32_t pos2 = pos;
  uint32_t pos21 = pos2;
  uint32_t res = Transaction_inputs_transaction_inputs_jumper(input, pos21 + (uint32_t)4U);
  uint32_t pos3 = res;
  uint32_t pos210 = pos3;
  uint32_t res0 = Transaction_outputs_transaction_outputs_jumper(input, pos210);
  uint32_t pos30 = res0;
