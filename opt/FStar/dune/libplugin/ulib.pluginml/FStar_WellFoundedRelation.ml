...
let uu___is_AccClassicalIntro (x : 'a)
  (projectee : ('a, 'r, Obj.t) acc_classical) : Prims.bool= true
let __proj__AccClassicalIntro__item__access_smaller (x : 'a)
  (projectee : ('a, 'r, Obj.t) acc_classical) :
  'a -> ('a, 'r, Obj.t) acc_classical=
...
type ('a, 'x1, 'x2) default_relation = ('a, 'a, 'x1, 'x2) Prims.precedes
let rec default_decreaser :
  'a . 'a -> ('a, ('a, unit, unit) default_relation, Obj.t) acc_classical =
...
let rec empty_decreaser :
  'a . 'a -> ('a, ('a, unit, unit) empty_relation, Obj.t) acc_classical =
...
let rec acc_decreaser :
  'a 'r .
    unit ->
      'a -> ('a, ('a, 'r, unit, unit) acc_relation, Obj.t) acc_classical
  =
  fun f x -> let smaller y = acc_decreaser () y in AccClassicalIntro smaller
let acc_to_wfr (f : unit) : 'a wfr_t=
  {
    relation = ();
...
let rec subrelation_decreaser :
  'a 'r . 'a wfr_t -> 'a -> ('a, 'r, Obj.t) acc_classical =
  fun wfr x ->
    let smaller y = subrelation_decreaser wfr y in AccClassicalIntro smaller
let subrelation_to_wfr (wfr : 'a wfr_t) : 'a wfr_t=
  {
    relation = ();
    decreaser = (fun uu___ -> (Obj.magic (subrelation_decreaser wfr)) uu___);
    proof = ()
  }
let rec inverse_image_decreaser :
  'a 'b 'r . ('a -> 'b) -> 'b wfr_t -> 'a -> ('a, 'r, Obj.t) acc_classical =
  fun f wfr x ->
    let smaller y = inverse_image_decreaser f wfr y in
    AccClassicalIntro smaller
let inverse_image_to_wfr (f : 'a -> 'b) (wfr : 'b wfr_t) : 'a wfr_t=
  {
    relation = ();
    decreaser =
...
let rec lex_dep_decreaser :
  'a 'b .
    'a wfr_t ->
      ('a -> 'b wfr_t) ->
        ('a, 'b) Fstarcompiler.Prims.dtuple2 ->
          (('a, 'b) Fstarcompiler.Prims.dtuple2, Obj.t, Obj.t) acc_classical
  =
...
let lex_dep_wfr (wfr_a : 'a wfr_t) (a_to_wfr_b : 'a -> 'b wfr_t) :
  ('a, 'b) Fstarcompiler.Prims.dtuple2 wfr_t=
  {
    relation = ();
...
let option_wfr (wfr : 'a wfr_t) : 'a FStar_Pervasives_Native.option wfr_t=
  let f opt =
    ...
    | FStar_Pervasives_Native.Some x ->
        Fstarcompiler.Prims.Mkdtuple2 (true, (Obj.magic x))
    | FStar_Pervasives_Native.None ->
        Fstarcompiler.Prims.Mkdtuple2
          (false, (Obj.magic (FStar_Universe.raise_val ()))) in
