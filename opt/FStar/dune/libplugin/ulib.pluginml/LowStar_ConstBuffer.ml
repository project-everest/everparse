...
type 'a qbuf = (unit, Obj.t) Fstarcompiler.Prims.dtuple2
...
let sub (c : 'a const_buffer) (i : FStar_UInt32.t) (len : unit) :
  ...
  let uu___ = c in
  match uu___ with
  | Fstarcompiler.Prims.Mkdtuple2 (q, x) ->
      let x1 = Obj.magic x in
      let y = LowStar_Monotonic_Buffer.msub x1 i () in
      Fstarcompiler.Prims.Mkdtuple2 ((), (Obj.magic y))
