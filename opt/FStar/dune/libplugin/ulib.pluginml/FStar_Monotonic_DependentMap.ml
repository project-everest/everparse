...
let upd : 'a 'b . ('a, 'b) map -> 'a -> 'b -> ('a, 'b) map =
  fun r -> fun x -> fun v -> (Fstarcompiler.Prims.Mkdtuple2 (x, v)) :: r
...
let alloc : 'a 'b 'inv . unit -> unit -> (Obj.t, 'a, 'b, 'inv) t =
  fun r -> fun uu___ -> FStar_HyperStack_ST.ralloc () []
let extend : 'a 'b 'inv . unit -> (Obj.t, 'a, 'b, 'inv) t -> 'a -> 'b -> unit
  =
  fun r ->
    fun t1 ->
      fun x ->
...
let lookup :
  'a 'b 'inv .
    unit ->
      (Obj.t, 'a, 'b, 'inv) t -> 'a -> 'b FStar_Pervasives_Native.option
  =
  fun r ->
    fun t1 ->
      fun x ->
