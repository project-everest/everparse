...
let lookup_bvar (e : FStarC_Reflection_Types.env) (x : Prims.int) :
  FStarC_Reflection_Types.term FStar_Pervasives_Native.option= Prims.magic ()
let lookup_fvar_uinst (e : FStarC_Reflection_Types.env)
  (x : FStarC_Reflection_Types.fv)
  (us : FStarC_Reflection_Types.universe Prims.list) :
  FStarC_Reflection_Types.term FStar_Pervasives_Native.option= Prims.magic ()
let lookup_fvar (e : FStarC_Reflection_Types.env)
  (x : FStarC_Reflection_Types.fv) :
...
let subst_ctx_uvar_and_subst
  (uu___ : FStarC_Reflection_Types.ctx_uvar_and_subst) (uu___1 : subst) :
  FStarC_Reflection_Types.ctx_uvar_and_subst= Prims.magic ()
...
let rec subst_term (t : FStarC_Reflection_Types.term) (ss : subst) :
  FStarC_Reflection_Types.term=
        ...
        (FStarC_Reflection_V2_Data.Tv_Let
           (recf, (subst_terms attrs ss), b1,
             (if recf
              ...
              else subst_term def ss), (subst_term body (shift_subst ss))))
        ...
        (FStarC_Reflection_V2_Data.Tv_AscribedC
           ((subst_term e ss), (subst_comp c ss),
             (match tac with
              ...
              | FStar_Pervasives_Native.Some tac1 ->
                  FStar_Pervasives_Native.Some (subst_term tac1 ss)), b))
and subst_binder (b : FStarC_Reflection_Types.binder) (ss : subst) :
  ...
  FStarC_Reflection_V2_Builtins.pack_binder
    {
      ...
      FStarC_Reflection_V2_Data.qual = (bndr.FStarC_Reflection_V2_Data.qual);
      FStarC_Reflection_V2_Data.attrs =
...
and subst_comp (c : FStarC_Reflection_Types.comp) (ss : subst) :
  FStarC_Reflection_Types.comp=
  ...
  | FStarC_Reflection_V2_Data.C_Lemma (pre, post, pats) ->
      FStarC_Reflection_V2_Builtins.pack_comp
        (FStarC_Reflection_V2_Data.C_Lemma
           ((subst_term pre ss), (subst_term post ss), (subst_term pats ss)))
...
and subst_terms (ts : FStarC_Reflection_Types.term Prims.list) (ss : subst) :
  FStarC_Reflection_Types.term Prims.list=
  match ts with
  | [] -> []
  | t::ts1 -> (subst_term t ss) :: (subst_terms ts1 ss)
and subst_args (ts : FStarC_Reflection_V2_Data.argv Prims.list) (ss : subst)
...
and subst_patterns
  (ps : (FStarC_Reflection_V2_Data.pattern * Prims.bool) Prims.list)
  (ss : subst) : (FStarC_Reflection_V2_Data.pattern * Prims.bool) Prims.list=
  match ps with
  | [] -> ps
  | (p, b)::ps1 ->
      let n = binder_offset_pattern p in
...
and subst_pattern (p : FStarC_Reflection_V2_Data.pattern) (ss : subst) :
...
and subst_branch (br : FStarC_Reflection_V2_Data.branch) (ss : subst) :
...
and subst_branches (brs : FStarC_Reflection_V2_Data.branch Prims.list)
  (ss : subst) : FStarC_Reflection_V2_Data.branch Prims.list=
  match brs with
  | [] -> []
  | br::brs1 -> (subst_branch br ss) :: (subst_branches brs1 ss)
and subst_match_returns (m : FStarC_Syntax_Syntax.match_returns_ascription)
  (ss : subst) : FStarC_Syntax_Syntax.match_returns_ascription=
  let uu___ = m in
  match uu___ with
  | (b, (ret, as_, eq)) ->
      let b1 = subst_binder b ss in
...
and ln'_match_returns (m : FStarC_Syntax_Syntax.match_returns_ascription)
  ...
  | (b, (ret, as_, eq)) ->
      ...
      let ret1 =
        match ret with
        | Fstarcompiler.FStar_Pervasives.Inl t -> ln' t (i + Prims.int_one)
        | Fstarcompiler.FStar_Pervasives.Inr c ->
...
let close_comp_typ' (c : comp_typ) (x : FStarC_Reflection_V2_Data.var)
  (i : Prims.nat) :
...
let open_comp_typ' (c : comp_typ) (x : FStarC_Reflection_V2_Data.var)
  (i : Prims.nat) :
  (FStarC_TypeChecker_Core.tot_or_ghost * FStarC_Reflection_Types.term)=
  ((FStar_Pervasives_Native.fst c),
...
let rec __close_term_vs (i : Prims.nat)
  (vs : FStarC_Reflection_V2_Data.var Prims.list)
  (t : FStarC_Reflection_Types.term) : FStarC_Reflection_Types.term=
  match vs with
  | [] -> t
  | v::vs1 ->
      subst_term (__close_term_vs (i + Prims.int_one) vs1 t) [ND (v, i)]
let close_term_vs (vs : FStarC_Reflection_V2_Data.var Prims.list)
  (t : FStarC_Reflection_Types.term) : FStarC_Reflection_Types.term=
  __close_term_vs Prims.int_zero vs t
let close_term_bs (bs : binding Prims.list)
  (t : FStarC_Reflection_Types.term) : FStarC_Reflection_Types.term=
  close_term_vs (FStar_List_Tot_Base.map FStar_Pervasives_Native.fst bs) t
...
let subtyping_token_renaming (g : FStarC_Reflection_Types.env)
  (bs0 : bindings) (bs1 : bindings) (x : FStarC_Reflection_V2_Data.var)
  (y : FStarC_Reflection_V2_Data.var) (t : FStarC_Reflection_Types.term)
  (t0 : FStarC_Reflection_Types.term) (t1 : FStarC_Reflection_Types.term)
  (d : (Obj.t, Obj.t, Obj.t) FStarC_Tactics_Types_Reflection.subtyping_token)
  : (Obj.t, Obj.t, Obj.t) FStarC_Tactics_Types_Reflection.subtyping_token=
  Prims.magic ()
let subtyping_token_weakening (g : FStarC_Reflection_Types.env)
  (bs0 : bindings) (bs1 : bindings) (x : FStarC_Reflection_V2_Data.var)
  (t : FStarC_Reflection_Types.term) (t0 : FStarC_Reflection_Types.term)
  (t1 : FStarC_Reflection_Types.term)
  (d : (Obj.t, Obj.t, Obj.t) FStarC_Tactics_Types_Reflection.subtyping_token)
  : (Obj.t, Obj.t, Obj.t) FStarC_Tactics_Types_Reflection.subtyping_token=
  Prims.magic ()
...
let equiv_arrow (g : FStarC_Reflection_Types.env)
  (e1 : FStarC_Reflection_Types.term) (e2 : FStarC_Reflection_Types.term)
  (ty : FStarC_Reflection_Types.typ) (q : FStarC_Reflection_V2_Data.aqualv)
  (x : FStarC_Reflection_V2_Data.var) (eq : (Obj.t, Obj.t, Obj.t) equiv) :
  (Obj.t, Obj.t, Obj.t) equiv=
...
let equiv_abs_close (g : FStarC_Reflection_Types.env)
  (e1 : FStarC_Reflection_Types.term) (e2 : FStarC_Reflection_Types.term)
  (ty : FStarC_Reflection_Types.typ) (q : FStarC_Reflection_V2_Data.aqualv)
  (x : FStarC_Reflection_V2_Data.var) (eq : (Obj.t, Obj.t, Obj.t) equiv) :
  (Obj.t, Obj.t, Obj.t) equiv=
  ...
  Rel_abs
    (g, ty, ty, q, (subst_term e1 [ND (x, Prims.int_zero)]),
      (subst_term e2 [ND (x, Prims.int_zero)]), x, (Rel_refl (g, ty, R_Eq)),
      eq1)
...
type dsl_tac_t =
  (fstar_top_env * FStarC_Reflection_Types.typ
    FStar_Pervasives_Native.option) ->
    ((Obj.t, Obj.t) dsl_tac_result_t, unit) FStar_Tactics_Effect.tac_repr
let if_complete_match (g : FStarC_Reflection_Types.env)
...
let mkif (g : fstar_env) (scrutinee : FStarC_Reflection_Types.term)
  ...
  T_Match
    (g, u_zero, bool_ty, scrutinee, FStarC_TypeChecker_Core.E_Total,
      ...
      (MC_Tok
         (g, scrutinee, bool_ty,
           (FStar_List_Tot_Base.map FStar_Pervasives_Native.fst [brt; bre]),
           [[]; []], ())), (brty ()))
...
let typing_to_token (g : FStarC_Reflection_Types.env)
  (e : FStarC_Reflection_Types.term) (c : comp_typ)
  (uu___ : (Obj.t, Obj.t, Obj.t) typing) :
  (Obj.t, Obj.t, Obj.t) FStarC_Tactics_Types_Reflection.typing_token=
  Prims.magic ()
