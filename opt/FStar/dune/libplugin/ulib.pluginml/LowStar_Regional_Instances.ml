...
let buffer_copyable :
  'a .
    'a ->
      nonzero ->
        (('a * nonzero), 'a LowStar_Buffer.buffer, Obj.t)
          LowStar_RVector.copyable
  = fun ia -> fun len -> LowStar_RVector.Cpy (buffer_copy ())
let vector_dummy :
  'a 'uuuuu . unit -> ('a, 'uuuuu, Obj.t) LowStar_RVector.rvector =
  fun uu___ ->
    LowStar_Vector.Vec
...
let vector_r_alloc :
  'a 'rst .
    ('rst, 'a) LowStar_Regional.regional ->
      unit -> ('a, 'rst, Obj.t) LowStar_RVector.rvector
  =
  fun rg ->
    fun r ->
  ...
  'uuuuu 'uuuuu1 .
    unit ->
      ('uuuuu1, 'uuuuu) LowStar_Regional.regional ->
        ('uuuuu, 'uuuuu1, Obj.t) LowStar_RVector.rvector -> unit
