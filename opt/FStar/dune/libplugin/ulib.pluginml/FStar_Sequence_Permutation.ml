...
type ('a, 's) index_fun = Obj.t nat_at_most -> Obj.t nat_at_most
...
let rec permutation_from_equal_counts :
  'a .
    'a FStar_Sequence_Base.seq ->
      'a FStar_Sequence_Base.seq -> ('a, Obj.t, Obj.t) seqperm
  =
  fun s0 s1 ->
    ...
    then let f i = i in f
      ...
      (let uu___1 = find (FStar_Sequence_Util.head s0) s1 in
       ...
       | (pfx, sfx) ->
           ...
           let f' =
             permutation_from_equal_counts (FStar_Sequence_Util.tail s0) s1' in
...
let shift_perm' (s0 : 'a FStar_Sequence_Base.seq)
  (s1 : 'a FStar_Sequence_Base.seq) (uu___ : unit)
  (p : ('a, Obj.t, Obj.t) seqperm) : ('a, Obj.t, Obj.t) seqperm=
