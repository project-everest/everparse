...
let deflookup (env : FStarC_TypeChecker_Env.env)
  (pos : FStarC_Interactive_JsonHelper.txdoc_pos) :
  ...
  match uu___ with
  | FStar_Pervasives_Native.Some
      { slr_name = uu___1; slr_def_range = FStar_Pervasives_Native.Some r;
        slr_typ = uu___2; slr_doc = uu___3; slr_def = uu___4;_}
      ->
...
let hoverlookup (env : FStarC_TypeChecker_Env.env)
  (pos : FStarC_Interactive_JsonHelper.txdoc_pos) :
  FStarC_Interactive_JsonHelper.assoct FStar_Pervasives_Native.option=
  let uu___ =
    let uu___1 =
  ...
  | uu___1 -> FStarC_Interactive_JsonHelper.nullResponse
let complookup (st : FStarC_Interactive_Ide_Types.repl_state)
  (pos : FStarC_Interactive_JsonHelper.txdoc_pos) :
  FStarC_Interactive_JsonHelper.assoct FStar_Pervasives_Native.option=
      ...
      (match uu___1 with
       | FStar_Pervasives_Native.Some (uu___2, text) ->
           let rec find_col l =
             ...
             | [] -> Prims.int_zero
             | h::t ->
                 if (h = 32) && ((FStarC_List.length t) < current_col)
                 then (FStarC_List.length t) + Prims.int_one
                 else find_col t in
           let str =
           ...
           let begin_col =
             let uu___3 = let uu___4 = explode str in FStarC_List.rev uu___4 in
             find_col uu___3 in
           let term =
             FStarC_Util.substring str begin_col (current_col - begin_col) in
