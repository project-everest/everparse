...
let fresh_token (uu___ : (Prims.string * sort)) (id : Prims.int) : decl=
  match uu___ with
  | (tok_name, sort1) ->
...
let mk_subtype_of_unit (v : term) : term=
  mkApp ("Prims.subtype_of", [v; mk_unit_type]) v.rng
...
let mk_ApplyTT (t : term) (t' : term) (r : FStarC_Range_Type.t) : term=
  mkApp ("ApplyTT", [t; t']) r
let kick_partial_app (t : term) : term=
  let uu___ =
...
let mk_String_const (s : Prims.string) (r : FStarC_Range_Type.t) : term=
  ...
  mkApp uu___ r
let mk_Precedes (x1 : term) (x2 : term) (x3 : term) (x4 : term)
  (r : FStarC_Range_Type.t) : term=
  let uu___ = mkApp ("Prims.precedes", [x1; x2; x3; x4]) r in mk_Valid uu___
...
let mk_haseq (t : term) : term=
  let uu___ = mkApp ("Prims.hasEq", [t]) t.rng in mk_Valid uu___
