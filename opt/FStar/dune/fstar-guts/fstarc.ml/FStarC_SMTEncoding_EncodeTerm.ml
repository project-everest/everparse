...
let mkForall_fuel (mname : Prims.string) (r : FStarC_Range_Type.t) :
  (FStarC_SMTEncoding_Term.pat Prims.list Prims.list *
    FStarC_SMTEncoding_Term.fvs * FStarC_SMTEncoding_Term.term) ->
    FStarC_SMTEncoding_Term.term=
  mkForall_fuel' mname r Prims.int_one
let head_normal (env : FStarC_SMTEncoding_Env.env_t)
  ...
  | FStarC_Syntax_Syntax.Tm_fvar fv ->
      let uu___ =
        FStarC_TypeChecker_Env.lookup_definition
          ...
          env.FStarC_SMTEncoding_Env.tcenv fv.FStarC_Syntax_Syntax.fv_name in
  ...
  | uu___ -> false
let head_redex (env : FStarC_SMTEncoding_Env.env_t)
  (t : FStarC_Syntax_Syntax.term) : Prims.bool=
  let uu___ =
...
let whnf (env : FStarC_SMTEncoding_Env.env_t) (t : FStarC_Syntax_Syntax.term)
  ...
  then t
    ...
    norm_with_steps
      ...
      FStarC_TypeChecker_Env.EraseUniverses] env.FStarC_SMTEncoding_Env.tcenv
      t
let norm (env : FStarC_SMTEncoding_Env.env_t) (t : FStarC_Syntax_Syntax.term)
  : FStarC_Syntax_Syntax.term=
  norm_with_steps
    [FStarC_TypeChecker_Env.Beta;
    FStarC_TypeChecker_Env.Exclude FStarC_TypeChecker_Env.Zeta;
    FStarC_TypeChecker_Env.Eager_unfolding;
    FStarC_TypeChecker_Env.EraseUniverses] env.FStarC_SMTEncoding_Env.tcenv t
let maybe_whnf (env : FStarC_SMTEncoding_Env.env_t)
...
let trivial_post (t : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
...
let mk_Apply (e : FStarC_SMTEncoding_Term.term)
...
let mk_Apply_args (e : FStarC_SMTEncoding_Term.term)
  ...
  FStarC_SMTEncoding_Term.term=
  FStarC_List.fold_left FStarC_SMTEncoding_Util.mk_ApplyTT e args
let raise_arity_mismatch (head : Prims.string) (arity : Prims.int)
  (n_args : Prims.int) (rng : FStarC_Range_Type.t) : 'a=
  let uu___ =
    let uu___1 = FStarC_Class_Show.show FStarC_Class_Show.showable_int arity in
    let uu___2 = FStarC_Class_Show.show FStarC_Class_Show.showable_int n_args in
    FStarC_Format.fmt3
      "Head symbol %s expects at least %s arguments; got only %s" head uu___1
      uu___2 in
...
let isTotFun_axioms (pos : FStarC_Range_Type.t)
  (head : FStarC_SMTEncoding_Term.term) (vars : FStarC_SMTEncoding_Term.fvs)
  (guards : FStarC_SMTEncoding_Term.term Prims.list) (is_pure : Prims.bool) :
  FStarC_SMTEncoding_Term.term=
  let maybe_mkForall pat vars1 body =
    match vars1 with
    | [] -> body
    | uu___ -> FStarC_SMTEncoding_Term.mkForall pos (pat, vars1, body) in
  let rec is_tot_fun_axioms ctx ctx_guard head1 vars1 guards1 =
    ...
    | (uu___::[], uu___1) ->
        ...
        then
          let uu___2 =
            let uu___3 =
              let uu___4 = FStarC_SMTEncoding_Term.mk_IsTotFun head1 in
              (ctx_guard, uu___4) in
    ...
    | (x::vars2, g_x::guards2) ->
        let is_tot_fun_head =
          let uu___ =
            let uu___1 =
              let uu___2 = FStarC_SMTEncoding_Term.mk_IsTotFun head1 in
              (ctx_guard, uu___2) in
...
let maybe_curry_app (rng : FStarC_Range_Type.t)
  ...
  (arity : Prims.int) (args : FStarC_SMTEncoding_Term.term Prims.list) :
  FStarC_SMTEncoding_Term.term=
  ...
  | FStar_Pervasives.Inl head1 ->
      ...
      then FStarC_SMTEncoding_Util.mkApp' (head1, args)
        ...
        if n_args > arity
        then
          (let uu___1 = FStarC_Util.first_N arity args in
           match uu___1 with
           | (args1, rest) ->
               let head2 = FStarC_SMTEncoding_Util.mkApp' (head1, args1) in
...
let maybe_curry_fvb (rng : FStarC_Range_Type.t)
  (fvb : FStarC_SMTEncoding_Env.fvar_binding)
  (args : FStarC_SMTEncoding_Term.term Prims.list) :
  FStarC_SMTEncoding_Term.term=
  ...
  then
    ...
    maybe_curry_app rng
      (FStar_Pervasives.Inl
         (FStarC_SMTEncoding_Term.Var (fvb.FStarC_SMTEncoding_Env.smt_id)))
      fvb.FStarC_SMTEncoding_Env.smt_arity args
let is_app (uu___ : FStarC_SMTEncoding_Term.op) : Prims.bool=
  match uu___ with
                  ...
                  (FStarC_Class_Setlike.union ()
                     (Obj.magic
                        (FStarC_FlatSet.setlike_flat_set
                           FStarC_Syntax_Syntax.ord_bv)) (Obj.magic out)
                     (Obj.magic uu___1))) uu___2 uu___1) uu___ tl in
      let uu___ =
        FStarC_Option.find
          (fun uu___1 ->
             match uu___1 with
      ...
      (match uu___ with
       ...
       | FStar_Pervasives_Native.Some
           ...
           let pos =
             FStarC_List.fold_left
               (fun out t ->
                  FStarC_Range_Ops.union_ranges out
                    t.FStarC_Syntax_Syntax.pos) hd.FStarC_Syntax_Syntax.pos
               tl in
           let uu___4 =
             let uu___5 =
               FStarC_Class_Show.show FStarC_Syntax_Print.showable_bv x in
             FStarC_Format.fmt1
               "SMT pattern misses at least one bound variable: %s" uu___5 in
           FStarC_Errors.log_issue FStarC_Class_HasRange.hasRange_range pos
             ...
             (Obj.magic uu___4))
type label =
  (FStarC_SMTEncoding_Term.fv * Prims.string * FStarC_Range_Type.t)
...
let __proj__Mkpattern__item__pat_vars (projectee : pattern) :
  (FStarC_Syntax_Syntax.bv * FStarC_SMTEncoding_Term.fv) Prims.list=
  match projectee with
  | { pat_vars; pat_term; guard; projections;_} -> pat_vars
let __proj__Mkpattern__item__pat_term (projectee : pattern) :
  unit -> (FStarC_SMTEncoding_Term.term * FStarC_SMTEncoding_Term.decls_t)=
  match projectee with
  | { pat_vars; pat_term; guard; projections;_} -> pat_term
let __proj__Mkpattern__item__guard (projectee : pattern) :
  FStarC_SMTEncoding_Term.term -> FStarC_SMTEncoding_Term.term=
  match projectee with | { pat_vars; pat_term; guard; projections;_} -> guard
let __proj__Mkpattern__item__projections (projectee : pattern) :
  FStarC_SMTEncoding_Term.term ->
    (FStarC_Syntax_Syntax.bv * FStarC_SMTEncoding_Term.term) Prims.list=
  match projectee with
  | { pat_vars; pat_term; guard; projections;_} -> projections
let as_function_typ (env : FStarC_SMTEncoding_Env.env_t)
  (t0 : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax) :
  FStarC_Syntax_Syntax.term=
  let rec aux norm1 t =
    ...
    | uu___ ->
        ...
        then let uu___1 = whnf env t1 in aux false uu___1
          ...
          (let uu___2 =
             let uu___3 =
               FStarC_Range_Ops.string_of_range t0.FStarC_Syntax_Syntax.pos in
             let uu___4 =
  ...
  | FStarC_Syntax_Syntax.Tm_refine
      ...
      (match uu___1 with
       | (args, res) ->
           (match args with
            | [] ->
                let uu___2 = FStarC_Syntax_Syntax.mk_Total k1 in ([], uu___2)
            | uu___2 -> (args, res)))
...
let is_BitVector_primitive
  ...
  | (FStarC_Syntax_Syntax.Tm_fvar fv, (sz_arg, uu___)::uu___1::[]) ->
      (((FStarC_Syntax_Syntax.fv_eq_lid fv FStarC_Parser_Const.nat_to_bv_lid)
        ...
        && (isInteger sz_arg.FStarC_Syntax_Syntax.n)
  | uu___ -> false
let rec encode_const (c : FStarC_Const.sconst)
...
and encode_binders
  ...
  (FStarC_SMTEncoding_Term.fv Prims.list * FStarC_SMTEncoding_Term.term
    ...
    FStarC_SMTEncoding_Term.decls_t * FStarC_Syntax_Syntax.bv Prims.list)=
  (let uu___1 = FStarC_Debug.medium () in
   if uu___1
   then
     let uu___2 =
       ...
       (fun uu___2 b ->
          ...
          | (vars, guards, env1, decls, names) ->
              let uu___3 =
                ...
                let uu___4 = FStarC_SMTEncoding_Env.gen_term_var env1 x in
                match uu___4 with
                | (xxsym, xx, env') ->
                    let uu___5 =
                      let uu___6 = norm env1 x.FStarC_Syntax_Syntax.sort in
              ...
              (match uu___3 with
               | (v, g, env2, decls', n) ->
                   ((v :: vars), (g :: guards), env2,
                     (FStarC_List.op_At decls decls'), (n :: names))))
       ([], [], env, [], []) bs in
   match uu___1 with
   | (vars, guards, env1, decls, names) ->
       ((FStarC_List.rev vars), (FStarC_List.rev guards), env1, decls,
         (FStarC_List.rev names)))
and encode_term_pred
  (fuel_opt : FStarC_SMTEncoding_Term.term FStar_Pervasives_Native.option)
  (t : FStarC_Syntax_Syntax.typ) (env : FStarC_SMTEncoding_Env.env_t)
  (e : FStarC_SMTEncoding_Term.term) :
  (FStarC_SMTEncoding_Term.term * FStarC_SMTEncoding_Term.decls_t)=
  let uu___ = encode_term t env in
  match uu___ with
  | (t1, decls) ->
      let uu___1 = FStarC_SMTEncoding_Term.mk_HasTypeWithFuel fuel_opt e t1 in
      (uu___1, decls)
and encode_arith_term (env : FStarC_SMTEncoding_Env.env_t)
  (head : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax)
  (args_e : FStarC_Syntax_Syntax.args) :
  (FStarC_SMTEncoding_Term.term * FStarC_SMTEncoding_Term.decls_t)=
  ...
  | (arg_tms, decls) ->
      ...
      let mk_default uu___1 =
        ...
        | (fname, fuel_args, arity) ->
            ...
            maybe_curry_app head.FStarC_Syntax_Syntax.pos fname arity args in
      let mk_l box op mk_args ts =
        let uu___1 = FStarC_Options.smtencoding_l_arith_native () in
        if uu___1
        then let uu___2 = let uu___3 = mk_args ts in op uu___3 in box uu___2
        else mk_default () in
      ...
      let mul box unbox nm = mk_nl box unbox nm FStarC_SMTEncoding_Util.mkMul in
      let div box unbox nm = mk_nl box unbox nm FStarC_SMTEncoding_Util.mkDiv in
      ...
      let ops =
        ...
        (FStarC_Parser_Const.real_op_LT,
          (mk_l FStarC_SMTEncoding_Term.boxBool FStarC_SMTEncoding_Util.mkLT
             (binary FStarC_SMTEncoding_Term.unboxReal)));
        (FStarC_Parser_Const.real_op_LTE,
          (mk_l FStarC_SMTEncoding_Term.boxBool FStarC_SMTEncoding_Util.mkLTE
             (binary FStarC_SMTEncoding_Term.unboxReal)));
        (FStarC_Parser_Const.real_op_GT,
          (mk_l FStarC_SMTEncoding_Term.boxBool FStarC_SMTEncoding_Util.mkGT
             (binary FStarC_SMTEncoding_Term.unboxReal)));
        (FStarC_Parser_Const.real_op_GTE,
          (mk_l FStarC_SMTEncoding_Term.boxBool FStarC_SMTEncoding_Util.mkGTE
      ...
      let uu___1 =
        let uu___2 =
          FStarC_List.tryFind
            (fun uu___3 ->
               match uu___3 with
               | (l, uu___4) -> FStarC_Syntax_Syntax.fv_eq_lid head_fv l) ops in
        FStar_Pervasives_Native.__proj__Some__item__v uu___2 in
      (match uu___1 with
       | (uu___2, op) -> let uu___3 = op arg_tms in (uu___3, decls))
and encode_BitVector_term (env : FStarC_SMTEncoding_Env.env_t)
  (head : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax)
  (args_e :
  ...
  (FStarC_SMTEncoding_Term.term * FStarC_SMTEncoding_Term.decls_elt
    Prims.list)=
  let uu___ = FStarC_List.hd args_e in
  match uu___ with
  | (tm_sz, uu___1) ->
      let uu___2 = uu___ in
      let sz = getInteger tm_sz.FStarC_Syntax_Syntax.n in
      let sz_key =
        let uu___3 = FStarC_Class_Show.show FStarC_Class_Show.showable_int sz in
        FStarC_Format.fmt1 "BitVector_%s" uu___3 in
      let sz_decls =
                    ...
                    let uu___6 =
                      let uu___7 =
                        FStarC_SMTEncoding_Env.varops.FStarC_SMTEncoding_Env.fresh
                          env.FStarC_SMTEncoding_Env.current_module_name "x" in
             ...
             | (decls, typing_inversion) ->
                 let uu___5 =
                   FStarC_SMTEncoding_Term.mk_decls "" sz_key
                     (FStarC_List.op_At t_decls [typing_inversion]) [] in
            ...
            | (arg_tms1, decls) ->
                ...
                let bv_sub =
                  mk_bv FStarC_SMTEncoding_Util.mkBvSub binary
                    (FStarC_SMTEncoding_Term.boxBitVec sz) in
                let bv_shl =
                  mk_bv (FStarC_SMTEncoding_Util.mkBvShl sz) binary_arith
                    (FStarC_SMTEncoding_Term.boxBitVec sz) in
                let bv_shr =
                ...
                let bv_udiv =
                  mk_bv (FStarC_SMTEncoding_Util.mkBvUdiv sz) binary_arith
                    (FStarC_SMTEncoding_Term.boxBitVec sz) in
                let bv_mod =
                  mk_bv (FStarC_SMTEncoding_Util.mkBvMod sz) binary_arith
                    (FStarC_SMTEncoding_Term.boxBitVec sz) in
                let bv_mul =
                  mk_bv (FStarC_SMTEncoding_Util.mkBvMul sz) binary_arith
                    (FStarC_SMTEncoding_Term.boxBitVec sz) in
                let bv_shl' =
                ...
                let bv_udiv_unsafe =
                  mk_bv (FStarC_SMTEncoding_Util.mkBvUdivUnsafe sz) binary
                    (FStarC_SMTEncoding_Term.boxBitVec sz) in
                let bv_mod_unsafe =
                  mk_bv (FStarC_SMTEncoding_Util.mkBvModUnsafe sz) binary
                    (FStarC_SMTEncoding_Term.boxBitVec sz) in
                let bv_mul' =
                ...
                let bv_not =
                  mk_bv FStarC_SMTEncoding_Util.mkBvNot unary
                    (FStarC_SMTEncoding_Term.boxBitVec sz) in
                let to_int =
                  mk_bv FStarC_SMTEncoding_Util.mkBvToNat unary
                    FStarC_SMTEncoding_Term.boxInt in
                let bv_to =
                  mk_bv (FStarC_SMTEncoding_Util.mkNatToBv sz) unary_arith
                    (FStarC_SMTEncoding_Term.boxBitVec sz) in
                let ops =
                  ...
                  (FStarC_Parser_Const.bv_not_lid, bv_not);
                ...
                (match uu___5 with
                 | (uu___6, op) ->
                     ...
                     (uu___7, (FStarC_List.op_At sz_decls decls)))))
and encode_deeply_embedded_quantifier (t : FStarC_Syntax_Syntax.term)
  (env : FStarC_SMTEncoding_Env.env_t) :
  (FStarC_SMTEncoding_Term.term * FStarC_SMTEncoding_Term.decls_t)=
  let env1 =
    {
           ...
           (match uu___2 with
            | (phi, decls') ->
                let interp =
                  ...
                  | uu___3 ->
                      let uu___4 =
                        let uu___5 =
                          let uu___6 =
                            let uu___7 = FStarC_SMTEncoding_Term.mk_Valid tm in
                            (uu___7, phi) in
                ...
                let ax =
                  let uu___3 =
                    let uu___4 =
                      let uu___5 = FStarC_Util.digest_of_string tkey_hash in
                ...
                let uu___3 =
                  let uu___4 =
                    let uu___5 =
                      FStarC_SMTEncoding_Term.mk_decls "" tkey_hash [ax]
                        (FStarC_List.op_At decls decls') in
                    FStarC_List.op_At decls' uu___5 in
                  FStarC_List.op_At decls uu___4 in
                (tm, uu___3)))
and encode_term (t : FStarC_Syntax_Syntax.typ)
  (env : FStarC_SMTEncoding_Env.env_t) :
  (FStarC_SMTEncoding_Term.term * FStarC_SMTEncoding_Term.decls_t)=
  FStarC_Defensive.def_check_scoped FStarC_TypeChecker_Env.hasBinders_env
    ...
    t.FStarC_Syntax_Syntax.pos "encode_term" env.FStarC_SMTEncoding_Env.tcenv
    t;
  (let t1 = FStarC_Syntax_Subst.compress t in
   let t0 = t1 in
   ...
   (match t1.FStarC_Syntax_Syntax.n with
    | FStarC_Syntax_Syntax.Tm_delayed uu___2 ->
        let uu___3 =
          let uu___4 =
            FStarC_Range_Ops.string_of_range t1.FStarC_Syntax_Syntax.pos in
          let uu___5 =
            FStarC_Class_Tagged.tag_of FStarC_Syntax_Syntax.tagged_term t1 in
          let uu___6 =
            FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t1 in
    ...
    | FStarC_Syntax_Syntax.Tm_quoted (qt, uu___2) ->
        ...
        ((let uu___4 = FStarC_Effect.op_Bang dbg_SMTEncoding in
          if uu___4
          then
            let uu___5 =
              FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t0 in
                              ...
                              (match uu___8 with
                               | (guards, guard_decls) ->
                                   let is_pure =
                                     let uu___9 =
                                       FStarC_TypeChecker_Normalize.maybe_ghost_to_pure
                                         env.FStarC_SMTEncoding_Env.tcenv res in
                                     FStarC_Syntax_Util.is_pure_comp uu___9 in
                                   let t_interp =
                                     let uu___9 =
                                       let uu___10 =
                                   ...
                                   let t_interp1 =
                                     let tot_fun_axioms =
                                       isTotFun_axioms
                                         t1.FStarC_Syntax_Syntax.pos f vars
                                         guards_l is_pure in
                                   ...
                                   let cvars =
                                     ...
                                     FStarC_List.filter
                                       (fun x ->
                                          let uu___10 =
                                            FStarC_SMTEncoding_Term.fv_name x in
                                          let uu___11 =
                                   ...
                                   let tsym =
                                     let uu___9 =
                                       FStarC_Util.digest_of_string tkey_hash in
                                     Prims.strcat prefix uu___9 in
                                   let cvar_sorts =
                                     FStarC_List.map
                                       FStarC_SMTEncoding_Term.fv_sort cvars in
                                   let caption =
                                     let uu___9 =
                                       ...
                                       let uu___10 =
                                         let uu___11 =
                                           FStarC_TypeChecker_Normalize.term_to_string
                                             ...
                                             t0 in
                                         FStarC_Util.replace_char uu___11 10
                                           32 in
                                   ...
                                   let t_has_kind =
                                     FStarC_SMTEncoding_Term.mk_HasType t2
                                       ...
                                       let uu___10 =
                                         FStarC_SMTEncoding_Term.mkForall
                                           ...
                                           ([[t_has_kind]], cvars,
                                             t_has_kind) in
                                       (uu___10,
                                         (FStar_Pervasives_Native.Some a_name),
                                         a_name) in
                                     FStarC_SMTEncoding_Util.mkAssume uu___9 in
                                   let f_has_t =
                                     FStarC_SMTEncoding_Term.mk_HasType f t2 in
                                   let f_has_t_z =
                                     FStarC_SMTEncoding_Term.mk_HasTypeZ f t2 in
                                   let pre_typing =
                                     let a_name =
                                   ...
                                   let t_interp2 =
                                     let a_name =
                                       Prims.strcat "interpretation_" tsym in
                                     let uu___9 =
                                       let uu___10 =
                                         let uu___11 =
                                           let uu___12 =
                                         ...
                                         let uu___12 =
                                           FStarC_SMTEncoding_Term.mk_decls
                                             ...
                                             (FStarC_List.op_At decls
                                                (FStarC_List.op_At decls'
                                                   guard_decls)) in
                                         FStarC_List.op_At guard_decls
                                           uu___12 in
                                       FStarC_List.op_At decls' uu___11 in
                ...
                let uu___5 =
                  let fvs =
                    ...
                    FStarC_Class_Setlike.elems ()
                      (Obj.magic
                         (FStarC_FlatSet.setlike_flat_set
                            FStarC_Syntax_Syntax.ord_bv)) (Obj.magic uu___6) in
                  let getfreeV t2 =
                      ...
                      (fun uu___7 bv ->
                         ...
                         | (env1, decls, vars, tms, guards) ->
                             ...
                             (match uu___8 with
                              | (sort, uu___9) ->
                                  let uu___10 =
                                    FStarC_SMTEncoding_Env.gen_term_var env1
                                      bv in
                                  (match uu___10 with
                                   | (sym, smt_tm, env2) ->
                  ...
                  (fvs, uu___6) in
                match uu___5 with
                | (fstar_fvs, (env1, fv_decls, fv_vars, fv_tms, fv_guards))
                    ->
                    let fv_decls1 = FStarC_List.rev fv_decls in
                    ...
                    let tdecl =
                      FStarC_SMTEncoding_Term.DeclFun
                        (tsym, arg_sorts, FStarC_SMTEncoding_Term.Term_sort,
                          FStar_Pervasives_Native.None) in
                      ...
                      let axiom =
                        let uu___6 =
                          ...
                          let uu___8 =
                            let uu___9 =
                              let uu___10 =
                                FStarC_SMTEncoding_Util.mk_and_l fv_guards1 in
                              let uu___11 =
                                FStarC_SMTEncoding_Term.mk_HasType tapp
                                  FStarC_SMTEncoding_Term.mk_Term_type in
                              (uu___10, uu___11) in
                    ...
                    let uu___6 =
                      let uu___7 =
                        FStarC_SMTEncoding_Term.mk_decls tsym tkey_hash
                          [tdecl; t_kinding] [] in
                      FStarC_List.op_At fv_decls1 uu___7 in
    ...
    | FStarC_Syntax_Syntax.Tm_refine uu___2 ->
        let uu___3 =
          ...
          let uu___4 =
            normalize_refinement steps env.FStarC_SMTEncoding_Env.tcenv t0 in
          match uu___4 with
          | {
                             ...
                             | (fsym, fterm) ->
                                 let tm_has_type_with_fuel =
                                   FStarC_SMTEncoding_Term.mk_HasTypeWithFuel
                                     (FStar_Pervasives_Native.Some fterm) xtm
                                     base_t in
                                 let encoding =
                                   FStarC_SMTEncoding_Util.mkAnd
                                 ...
                                 let xfv =
                                   FStarC_SMTEncoding_Term.mk_fv
                                     (x1, FStarC_SMTEncoding_Term.Term_sort) in
                                 let ffv =
                                   FStarC_SMTEncoding_Term.mk_fv
                                     (fsym,
                                       FStarC_SMTEncoding_Term.Fuel_sort) in
                                 ...
                                 ((let uu___9 =
                                  ...
                                  (let tsym =
                                   ...
                                   let cvar_sorts =
                                     FStarC_List.map
                                       FStarC_SMTEncoding_Term.fv_sort cvars1 in
                                   let tdecl =
                                     FStarC_SMTEncoding_Term.DeclFun
                                       (tsym, cvar_sorts,
                                         FStarC_SMTEncoding_Term.Term_sort,
                                   ...
                                   let t2 =
                                     ...
                                     FStarC_SMTEncoding_Util.mkApp uu___9 in
                                   let x_has_base_t =
                                     FStarC_SMTEncoding_Term.mk_HasType xtm
                                       base_t in
                                   let x_has_t =
                                     FStarC_SMTEncoding_Term.mk_HasTypeWithFuel
                                   ...
                                   let t_has_kind =
                                     FStarC_SMTEncoding_Term.mk_HasType t2
                                       FStarC_SMTEncoding_Term.mk_Term_type in
                                   let t_haseq_base =
                                     FStarC_SMTEncoding_Term.mk_haseq base_t in
                                   let t_haseq_ref =
                   ...
                   let uu___6 =
                     let uu___7 =
                       ...
                       FStarC_Class_Show.show FStarC_Class_Show.showable_int
                         uu___8 in
                     FStarC_Format.fmt1 "uvar_typing_%s" uu___7 in
                    ...
                    then encode_BitVector_term env head1 args_e1
                      ...
                      (match uu___5 with
                       ...
                       | (FStarC_Syntax_Syntax.Tm_constant
                          (FStarC_Const.Const_range_of), (arg, uu___6)::[])
                           ->
                           encode_const
                            ...
                            | uu___7 ->
                                ...
                                ((let uu___9 =
                                  ...
                                  then
                                    let uu___10 =
                                      FStarC_Class_Show.show
                                        FStarC_Syntax_Print.showable_term e0 in
                            ...
                            | (args, decls) ->
                                ...
                                let encode_full_app fv =
                                  ...
                                  | (fname, fuel_args, arity) ->
                                      let tm =
                                        maybe_curry_app
                                          t0.FStarC_Syntax_Syntax.pos fname
                                          arity
                                          (FStarC_List.op_At fuel_args args) in
                                      (tm, decls) in
                                let head2 =
                                  ...
                                  | FStarC_Syntax_Syntax.Tm_ascribed
                                      { FStarC_Syntax_Syntax.tm = uu___8;
                                        FStarC_Syntax_Syntax.asc =
                                          (FStar_Pervasives.Inl t2, uu___9,
                                           uu___10);
               ...
               let uu___4 =
                 let fvs =
                   ...
                   FStarC_Class_Setlike.elems ()
                     (Obj.magic
                        (FStarC_FlatSet.setlike_flat_set
                           FStarC_Syntax_Syntax.ord_bv)) (Obj.magic uu___5) in
                 let tms =
                   FStarC_List.map
                     (FStarC_SMTEncoding_Env.lookup_term_var env) fvs in
                 let uu___5 =
                   FStarC_List.map
                     (fun uu___6 -> FStarC_SMTEncoding_Term.Term_sort) fvs in
                 (uu___5, tms) in
               match uu___4 with
               | (arg_sorts, arg_terms) ->
                   let f =
                          ...
                          | (body3, decls') ->
                              ...
                              let uu___8 =
                                ...
                                | FStar_Pervasives_Native.Some c ->
                                    let tfun = FStarC_Syntax_Util.arrow bs1 c in
                                         ...
                                         let uu___10 =
                                           let uu___11 =
                                             let uu___12 =
                                               FStarC_SMTEncoding_Term.free_variables
                                                 t2 in
                                             FStarC_List.op_At uu___12 cvars in
                                           FStarC_Util.remove_dups
                                    ...
                                    | (cvars1, key_body1) ->
                                        ...
                                        ((let uu___11 =
                                         ...
                                         (let cvar_sorts =
                                          ...
                                          let fsym =
                                            let uu___11 =
                                              FStarC_Util.digest_of_string
                                                tkey_hash in
                                            Prims.strcat "Tm_abs_" uu___11 in
                                          let fdecl =
                                            FStarC_SMTEncoding_Term.DeclFun
                                              (fsym, cvar_sorts,
                                                FStarC_SMTEncoding_Term.Term_sort,
                                          ...
                                          let uu___11 =
                                            let uu___12 =
                                              ...
                                              FStarC_List.op_At decls'
                                                uu___13 in
                                            FStarC_List.op_At decls uu___12 in
                                          (f, uu___11)))))))))
    ...
    | FStarC_Syntax_Syntax.Tm_let
          ...
          FStarC_List.map
            (fun lb ->
               ...
               | { FStarC_Syntax_Syntax.lbname = lbname;
                   ...
                   let x = FStar_Pervasives.__proj__Inl__item__v lbname in
                   let uu___11 =
        ...
        FStarC_Effect.raise (FStarC_SMTEncoding_Env.Inner_let_rec names)
    | FStarC_Syntax_Syntax.Tm_match
        ...
        encode_match e pats FStarC_SMTEncoding_Term.mk_Term_unit env
          encode_term))
and encode_let (x : FStarC_Syntax_Syntax.bv) (t1 : FStarC_Syntax_Syntax.typ)
  ...
  let uu___ =
    let uu___1 =
      FStarC_Syntax_Util.ascribe e1
        ((FStar_Pervasives.Inl t1), FStar_Pervasives_Native.None, false) in
...
and encode_match (e : FStarC_Syntax_Syntax.term)
  (pats : FStarC_Syntax_Syntax.branch Prims.list)
  (default_case : FStarC_SMTEncoding_Term.term)
  (env : FStarC_SMTEncoding_Env.env_t)
  (encode_br :
    FStarC_Syntax_Syntax.term ->
      FStarC_SMTEncoding_Env.env_t ->
                    ...
                    | (p, w, br) ->
                        ...
                        (match uu___5 with
                         | (env0, pattern1) ->
                             ...
                             let projections = pattern1.projections scr' in
                             let env2 =
                               FStarC_List.fold_left
                                 (fun env3 uu___6 ->
                                    match uu___6 with
                                    | (x, t) ->
                                        FStarC_SMTEncoding_Env.push_term_var
                                          env3 x t) env1 projections in
                             let uu___6 =
                               match w with
                               | FStar_Pervasives_Native.None -> (guard, [])
                                   ...
                                   | (br1, decls3) ->
                                       ...
                                       (uu___8,
                                         (FStarC_List.op_At decls1
                                            (FStarC_List.op_At decls2 decls3))))))) in
             FStarC_List.fold_right encode_branch pats (default_case, decls) in
           (match uu___2 with
            | (match_tm, decls1) ->
                let uu___3 =
                  let uu___4 =
...
and encode_pat (env : FStarC_SMTEncoding_Env.env_t)
  ...
  (FStarC_SMTEncoding_Env.env_t * pattern)=
  (let uu___1 = FStarC_Debug.medium () in
   if uu___1
   then
     let uu___2 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_pat pat in
   ...
   | (vars, pat_term) ->
       ...
       (match uu___2 with
        | (env1, vars1) ->
            let rec mk_guard pat1 scrutinee =
              ...
              | FStarC_Syntax_Syntax.Pat_cons (f, uu___3, args) ->
                  let is_f =
                    ...
                    match uu___4 with
                    | (uu___5, uu___6::[]) -> FStarC_SMTEncoding_Util.mkTrue
                    | uu___5 ->
                  ...
                  FStarC_SMTEncoding_Util.mk_and_l (is_f :: sub_term_guards) in
            ...
            (env1, pattern1)))
and encode_args (l : FStarC_Syntax_Syntax.args)
  (env : FStarC_SMTEncoding_Env.env_t) :
  (FStarC_SMTEncoding_Term.term Prims.list * FStarC_SMTEncoding_Term.decls_t)=
  let uu___ =
    FStarC_List.fold_left
      (fun uu___1 uu___2 ->
         ...
         | ((tms, decls), (t, uu___3)) ->
             ...
             (match uu___4 with
              | (t1, decls') ->
                  ((t1 :: tms), (FStarC_List.op_At decls decls')))) ([], [])
      l in
  match uu___ with | (l1, decls) -> ((FStarC_List.rev l1), decls)
and encode_smt_patterns
  (pats_l : FStarC_Syntax_Syntax.arg Prims.list Prims.list)
  (env : FStarC_SMTEncoding_Env.env_t) :
            ...
            uu___1::(x, uu___2)::(t1, uu___3)::[]) when
             ...
             (match uu___4 with
              | (x1, decls) ->
                  ...
                  (match uu___5 with
                   | (t2, decls') ->
                       let uu___6 = FStarC_SMTEncoding_Term.mk_HasType x1 t2 in
                       (uu___6, (FStarC_List.op_At decls decls'))))
  ...
  FStarC_List.fold_right
    ...
    ([], [])
and encode_formula (phi : FStarC_Syntax_Syntax.typ)
  (env : FStarC_SMTEncoding_Env.env_t) :
  (FStarC_SMTEncoding_Term.term * FStarC_SMTEncoding_Term.decls_t)=
  let debug phi1 =
    let uu___ = FStarC_Effect.op_Bang dbg_SMTEncoding in
    if uu___
    then
      let uu___1 =
  ...
  let enc f r l =
    ...
    | (decls, args) ->
        let uu___1 =
          ...
          {
            FStarC_SMTEncoding_Term.tm = (uu___2.FStarC_SMTEncoding_Term.tm);
            FStarC_SMTEncoding_Term.freevars =
        ...
        (match uu___3 with
         | (l1, decls1) ->
             (match l1.FStarC_SMTEncoding_Term.tm with
              | FStarC_SMTEncoding_Term.App
                  (FStarC_SMTEncoding_Term.TrueOp, uu___4) -> (l1, decls1)
              | uu___4 ->
         ...
         | (g, decls1) ->
             ...
             (match uu___5 with
              | (t, decls2) ->
                  ...
                  (match uu___6 with
                   | (e, decls3) ->
                       let res = FStarC_SMTEncoding_Term.mkITE (g, t, e) r in
                       (res,
                         (FStarC_List.op_At decls1
            ...
            uu___::(x, uu___1)::(t, uu___2)::[]) when
             ...
             (match uu___3 with
              | (x1, decls) ->
                  ...
                  (match uu___4 with
                   | (t1, decls') ->
                       let uu___5 = FStarC_SMTEncoding_Term.mk_HasType x1 t1 in
                       (uu___5, (FStarC_List.op_At decls decls'))))
         | (FStarC_Syntax_Syntax.Tm_fvar fv, uu___::(phi2, uu___1)::[]) when
             FStarC_Syntax_Syntax.fv_eq_lid fv
               FStarC_Parser_Const.by_tactic_lid
                    ...
                    let uu___4 =
                      let uu___5 =
                        let uu___6 =
                          let uu___7 =
                            let uu___8 = FStarC_Errors_Msg.mkmsg s in
                            (uu___8, (phi2.FStarC_Syntax_Syntax.pos), false) in
                          FStarC_Syntax_Syntax.Meta_labeled uu___7 in
                        {
                          FStarC_Syntax_Syntax.tm2 = phi2;
                          FStarC_Syntax_Syntax.meta = uu___6
               ...
               | (tt, decls) ->
                   let tt1 =
                     let uu___3 =
                       ...
                       FStarC_Range_Ops.rng_included uu___4 uu___5 in
                     if uu___3
                     then tt
                     else
                       {
  ...
  (let phi1 = FStarC_Syntax_Util.unascribe phi in
   ...
   | FStar_Pervasives_Native.Some (FStarC_Syntax_Formula.BaseConn (op, arms))
       ->
       let uu___2 =
         FStarC_List.tryFind
           (fun uu___3 ->
              match uu___3 with | (l, uu___4) -> FStarC_Ident.lid_equals op l)
           connectives in
       (match uu___2 with
        | FStar_Pervasives_Native.None -> fallback phi1
       ...
       (FStarC_List.iter (check_pattern_vars env vars) pats;
        (let uu___3 = encode_q_body env vars pats body in
         ...
         | (vars1, pats1, guard, body1, decls) ->
             let tm =
               let uu___4 =
                 let uu___5 = FStarC_SMTEncoding_Util.mkImp (guard, body1) in
                 (pats1, vars1, uu___5) in
               FStarC_SMTEncoding_Term.mkForall phi1.FStarC_Syntax_Syntax.pos
                 uu___4 in
             (tm, decls)))
       ...
       (FStarC_List.iter (check_pattern_vars env vars) pats;
        (let uu___3 = encode_q_body env vars pats body in
         ...
         | (vars1, pats1, guard, body1, decls) ->
             let uu___4 =
               let uu___5 =
                 let uu___6 = FStarC_SMTEncoding_Util.mkAnd (guard, body1) in
                 (pats1, vars1, uu___6) in
               FStarC_SMTEncoding_Term.mkExists phi1.FStarC_Syntax_Syntax.pos
                 uu___5 in
             (uu___4, decls))))
let encode_function_type_as_formula (t : FStarC_Syntax_Syntax.typ)
  (env : FStarC_SMTEncoding_Env.env_t) :
  (FStarC_SMTEncoding_Term.term * FStarC_SMTEncoding_Term.decls_t)=
  let universe_of_binders binders =
    FStarC_List.map (fun uu___ -> FStarC_Syntax_Syntax.U_zero) binders in
