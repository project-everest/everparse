...
let op_let_Question (f : 't mm) (g : 't -> 's mm) : 's mm=
  fun b ->
    let uu___ = f b in
    match uu___ with | (t1, b1) -> let uu___1 = g t1 in uu___1 b1
let ret (x : 't) : 't mm= fun b -> (x, b)
let should_memo : Prims.bool mm= fun b -> (b, b)
let no_memo : unit mm= fun uu___ -> ((), false)
let maybe_memoize (h : 'a FStarC_Syntax_Syntax.syntax)
  (f : 'a FStarC_Syntax_Syntax.syntax -> FStarC_Hash.hash_code mm) :
  FStarC_Hash.hash_code mm=
  fun should_memo1 ->
    if should_memo1
    then
      let uu___ = FStarC_Effect.op_Bang h.FStarC_Syntax_Syntax.hash_code in
      match uu___ with
...
let of_int (i : Prims.int) : FStarC_Hash.hash_code mm=
  let uu___ = FStarC_Hash.of_int i in ret uu___
let of_string (s : Prims.string) : FStarC_Hash.hash_code mm=
  let uu___ = FStarC_Hash.of_string s in ret uu___
let mix (f : FStarC_Hash.hash_code mm) (g : FStarC_Hash.hash_code mm) :
  FStarC_Hash.hash_code mm=
  fun b ->
    let uu___ = f b in
    match uu___ with
...
let mix_list (l : FStarC_Hash.hash_code mm Prims.list) :
  FStarC_Hash.hash_code mm= FStarC_List.fold_right mix l nil_hc
let mix_list_lit :
  FStarC_Hash.hash_code mm Prims.list -> FStarC_Hash.hash_code mm= mix_list
let hash_list (h : 'a -> FStarC_Hash.hash_code mm) (ts : 'a Prims.list) :
  FStarC_Hash.hash_code mm=
  let uu___ = FStarC_List.map h ts in mix_list uu___
let hash_option (h : 'a -> FStarC_Hash.hash_code mm)
  (o : 'a FStar_Pervasives_Native.option) : FStarC_Hash.hash_code mm=
  match o with
  | FStar_Pervasives_Native.None ->
      let uu___ = FStarC_Hash.of_int (Prims.of_int (1237)) in ret uu___
  | FStar_Pervasives_Native.Some o1 ->
      let uu___ =
        let uu___1 = FStarC_Hash.of_int (Prims.of_int (1249)) in ret uu___1 in
      let uu___1 = h o1 in mix uu___ uu___1
let hash_doc (d : FStar_Pprint.document) : FStarC_Hash.hash_code mm=
  let uu___ =
    FStarC_Pprint.pretty_string (FStarC_Util.float_of_string "1.0")
      (Prims.of_int (80)) d in
  of_string uu___
let hash_doc_list (ds : FStar_Pprint.document Prims.list) :
  FStarC_Hash.hash_code mm= hash_list hash_doc ds
let hash_pair (h : 'a -> FStarC_Hash.hash_code mm)
  (i : 'b -> FStarC_Hash.hash_code mm) (x : ('a * 'b)) :
  FStarC_Hash.hash_code mm=
  let uu___ = h (FStar_Pervasives_Native.fst x) in
  let uu___1 = i (FStar_Pervasives_Native.snd x) in mix uu___ uu___1
let rec hash_term (t : FStarC_Syntax_Syntax.term) : FStarC_Hash.hash_code mm=
  maybe_memoize t hash_term'
and hash_comp (c : FStarC_Syntax_Syntax.comp' FStarC_Syntax_Syntax.syntax) :
  FStarC_Hash.hash_code mm= maybe_memoize c hash_comp'
and hash_term' (t : FStarC_Syntax_Syntax.term) : FStarC_Hash.hash_code mm=
  let uu___ =
  ...
  | FStarC_Syntax_Syntax.Tm_bvar bv ->
      ...
      let uu___2 = of_int bv.FStarC_Syntax_Syntax.index in mix uu___1 uu___2
      ...
      let uu___2 =
        ...
        let uu___4 =
          let uu___5 =
            ...
            let uu___7 = hash_list hash_branch branches in mix uu___6 uu___7 in
          let uu___6 = hash_option hash_rc rcopt in mix uu___5 uu___6 in
        mix uu___3 uu___4 in
      mix uu___1 uu___2
...
and hash_comp' (c : FStarC_Syntax_Syntax.comp) : FStarC_Hash.hash_code mm=
  ...
  | FStarC_Syntax_Syntax.Total t ->
      let uu___ =
        ...
        let uu___2 = let uu___3 = hash_term t in [uu___3] in uu___1 :: uu___2 in
      mix_list_lit uu___
  ...
  | FStarC_Syntax_Syntax.Comp ct ->
      let uu___ =
        ...
        uu___1 :: uu___2 in
      mix_list_lit uu___
and hash_lb (lb : FStarC_Syntax_Syntax.letbinding) :
  FStarC_Hash.hash_code mm=
  let uu___ =
...
and hash_branch
  ...
  | (p, topt, t) ->
      let uu___1 =
        ...
        uu___2 :: uu___3 in
      mix_list_lit uu___1
and hash_pat (p : FStarC_Syntax_Syntax.pat' FStarC_Syntax_Syntax.withinfo_t)
  : FStarC_Hash.hash_code mm=
  ...
  | FStarC_Syntax_Syntax.Pat_dot_term t ->
      let uu___ =
        ...
        uu___1 :: uu___2 in
      mix_list_lit uu___
and hash_bv (b : FStarC_Syntax_Syntax.bv) : FStarC_Hash.hash_code mm=
  hash_term b.FStarC_Syntax_Syntax.sort
and hash_fv (fv : FStarC_Syntax_Syntax.fv) : FStarC_Hash.hash_code mm=
  let uu___ = FStarC_Ident.string_of_lid fv.FStarC_Syntax_Syntax.fv_name in
  of_string uu___
and hash_binder (b : FStarC_Syntax_Syntax.binder) : FStarC_Hash.hash_code mm=
  let uu___ =
    let uu___1 = hash_bv b.FStarC_Syntax_Syntax.binder_bv in
    let uu___2 =
      let uu___3 = hash_option hash_bqual b.FStarC_Syntax_Syntax.binder_qual in
      let uu___4 =
        let uu___5 = hash_list hash_term b.FStarC_Syntax_Syntax.binder_attrs in
        [uu___5] in
      uu___3 :: uu___4 in
    uu___1 :: uu___2 in
  mix_list_lit uu___
and hash_universe (uu___ : FStarC_Syntax_Syntax.universe) :
  FStarC_Hash.hash_code mm=
  match uu___ with
...
and hash_arg_qualifier (aq : FStarC_Syntax_Syntax.arg_qualifier) :
  FStarC_Hash.hash_code mm=
  ...
  mix uu___ uu___1
and hash_bqual (uu___ : FStarC_Syntax_Syntax.binder_qualifier) :
  FStarC_Hash.hash_code mm=
  match uu___ with
...
and hash_uvar
  ...
  | (u, uu___1) ->
      let uu___2 =
        FStarC_Syntax_Unionfind.uvar_id u.FStarC_Syntax_Syntax.ctx_uvar_head in
      of_int uu___2
and hash_universe_uvar
  ...
  : FStarC_Hash.hash_code mm=
  let uu___ = FStarC_Syntax_Unionfind.univ_uvar_id u in of_int uu___
and hash_ascription
  (uu___ :
    ((FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax,
      FStarC_Syntax_Syntax.comp' FStarC_Syntax_Syntax.syntax)
...
and hash_bool (b : Prims.bool) : FStarC_Hash.hash_code mm=
  if b then of_int (Prims.of_int (307)) else of_int (Prims.of_int (311))
and hash_constant (uu___ : FStarC_Syntax_Syntax.sconst) :
  FStarC_Hash.hash_code mm=
  match uu___ with
  ...
  | FStarC_Const.Const_range r ->
      ...
      let uu___2 =
        let uu___3 = FStarC_Range_Ops.string_of_range r in of_string uu___3 in
      mix uu___1 uu___2
...
and hash_sw (uu___ : (FStarC_Const.signedness * FStarC_Const.width)) :
  FStarC_Hash.hash_code mm=
  match uu___ with
  | (s, w) ->
      let uu___1 =
      ...
      mix uu___1 uu___2
and hash_ident (i : FStarC_Syntax_Syntax.univ_name) :
  FStarC_Hash.hash_code mm=
  let uu___ = FStarC_Ident.string_of_id i in of_string uu___
and hash_lid (l : FStarC_Ident.lident) : FStarC_Hash.hash_code mm=
  let uu___ = FStarC_Ident.string_of_lid l in of_string uu___
and hash_lbname
  (l :
...
and hash_rc (rc : FStarC_Syntax_Syntax.residual_comp) :
  FStarC_Hash.hash_code mm=
  let uu___ =
    let uu___1 = hash_lid rc.FStarC_Syntax_Syntax.residual_effect in
    let uu___2 =
      let uu___3 = hash_option hash_term rc.FStarC_Syntax_Syntax.residual_typ in
      let uu___4 =
        let uu___5 =
    ...
    uu___1 :: uu___2 in
  mix_list_lit uu___
and hash_flag (uu___ : FStarC_Syntax_Syntax.cflag) :
  FStarC_Hash.hash_code mm=
  match uu___ with
...
and hash_meta (m : FStarC_Syntax_Syntax.metadata) : FStarC_Hash.hash_code mm=
  match m with
  ...
  | FStarC_Syntax_Syntax.Meta_named l ->
      let uu___ =
        ...
        let uu___2 = let uu___3 = hash_lid l in [uu___3] in uu___1 :: uu___2 in
      mix_list_lit uu___
  ...
  | FStarC_Syntax_Syntax.Meta_monadic_lift (m0, m1, t) ->
      let uu___ =
        ...
        uu___1 :: uu___2 in
      mix_list_lit uu___
and hash_meta_source_info (m : FStarC_Syntax_Syntax.meta_source_info) :
  FStarC_Hash.hash_code mm=
  match m with
...
and hash_lazyinfo (li : FStarC_Syntax_Syntax.lazyinfo) :
  FStarC_Hash.hash_code mm= of_int Prims.int_zero
and hash_quoteinfo (qi : FStarC_Syntax_Syntax.quoteinfo) :
  FStarC_Hash.hash_code mm=
  let uu___ =
    hash_bool
...
let rec equal_list :
  ...
  fun f l1 l2 ->
    ...
    | uu___ -> false
let equal_opt (f : 'uuuuu -> 'uuuuu1 -> Prims.bool)
  (o1 : 'uuuuu FStar_Pervasives_Native.option)
  (o2 : 'uuuuu1 FStar_Pervasives_Native.option) : Prims.bool=
  match (o1, o2) with
  | (FStar_Pervasives_Native.None, FStar_Pervasives_Native.None) -> true
  | (FStar_Pervasives_Native.Some a, FStar_Pervasives_Native.Some b) -> f a b
  | uu___ -> false
let equal_pair (f : 'uuuuu -> 'uuuuu1 -> Prims.bool)
  (g : 'uuuuu2 -> 'uuuuu3 -> Prims.bool) (uu___ : ('uuuuu * 'uuuuu2))
  (uu___1 : ('uuuuu1 * 'uuuuu3)) : Prims.bool=
  match (uu___, uu___1) with | ((x1, y1), (x2, y2)) -> (f x1 x2) && (g y1 y2)
let equal_poly (x : 'uuuuu) (y : 'uuuuu) : Prims.bool= x = y
let ext_hash_term (t : FStarC_Syntax_Syntax.term) : FStarC_Hash.hash_code=
  let uu___ = let uu___1 = hash_term t in uu___1 true in
  FStar_Pervasives_Native.fst uu___
let ext_hash_term_no_memo (t : FStarC_Syntax_Syntax.term) :
  FStarC_Hash.hash_code=
  let uu___ = let uu___1 = hash_term t in uu___1 false in
  FStar_Pervasives_Native.fst uu___
let rec equal_term (t1 : FStarC_Syntax_Syntax.term)
  (t2 : FStarC_Syntax_Syntax.term) : Prims.bool=
  let uu___ = FStarC_Util.physical_equality t1 t2 in
  if uu___
  then true
  else
    (let uu___2 =
       FStarC_Util.physical_equality t1.FStarC_Syntax_Syntax.n
    ...
    (match ((c1.FStarC_Syntax_Syntax.n), (c2.FStarC_Syntax_Syntax.n)) with
     ...
     | (FStarC_Syntax_Syntax.Comp ct1, FStarC_Syntax_Syntax.Comp ct2) ->
         ((((FStarC_Ident.lid_equals ct1.FStarC_Syntax_Syntax.effect_name
              ...
              &&
              (equal_list equal_universe ct1.FStarC_Syntax_Syntax.comp_univs
...
and equal_binder (b1 : FStarC_Syntax_Syntax.binder)
  ...
  let uu___ = FStarC_Util.physical_equality b1 b2 in
  if uu___
  then true
  else
...
and equal_ascription
  ...
  (x2 :
    ((FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax,
      ...
      Prims.bool))
  : Prims.bool=
  let uu___ = FStarC_Util.physical_equality x1 x2 in
  if uu___
  then true
  else
    (let uu___2 = x1 in
...
and equal_letbinding (l1 : FStarC_Syntax_Syntax.letbinding)
  ...
  let uu___ = FStarC_Util.physical_equality l1 l2 in
  if uu___
  then true
  else
    (((((equal_lbname l1.FStarC_Syntax_Syntax.lbname
          ...
          (equal_list FStarC_Ident.ident_equals
             l1.FStarC_Syntax_Syntax.lbunivs l2.FStarC_Syntax_Syntax.lbunivs))
         &&
...
and equal_bv (b1 : FStarC_Syntax_Syntax.bv) (b2 : FStarC_Syntax_Syntax.bv) :
  Prims.bool=
  let uu___ = FStarC_Util.physical_equality b1 b2 in
  if uu___
  then true
    ...
    (FStarC_Ident.ident_equals b1.FStarC_Syntax_Syntax.ppname
      ...
      &&
      (equal_term b1.FStarC_Syntax_Syntax.sort b2.FStarC_Syntax_Syntax.sort)
and equal_fv (f1 : FStarC_Syntax_Syntax.fv) (f2 : FStarC_Syntax_Syntax.fv) :
  Prims.bool=
  let uu___ = FStarC_Util.physical_equality f1 f2 in
  if uu___
  then true
  else
    FStarC_Ident.lid_equals f1.FStarC_Syntax_Syntax.fv_name
      f2.FStarC_Syntax_Syntax.fv_name
and equal_universe (u1 : FStarC_Syntax_Syntax.universe)
  (u2 : FStarC_Syntax_Syntax.universe) : Prims.bool=
  let uu___ = FStarC_Util.physical_equality u1 u2 in
  if uu___
  then true
  else
    (let uu___2 =
       let uu___3 = FStarC_Syntax_Subst.compress_univ u1 in
       let uu___4 = FStarC_Syntax_Subst.compress_univ u2 in (uu___3, uu___4) in
     match uu___2 with
  ...
  then true
    ...
    (match (c1, c2) with
     ...
     | (FStarC_Const.Const_range r1, FStarC_Const.Const_range r2) ->
         let uu___2 = FStarC_Range_Ops.compare r1 r2 in
...
and equal_arg
  ...
  (arg2 :
    (FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax *
      FStarC_Syntax_Syntax.arg_qualifier FStar_Pervasives_Native.option))
  : Prims.bool=
  let uu___ = FStarC_Util.physical_equality arg1 arg2 in
  if uu___
  then true
  else
    (let uu___2 = arg1 in
...
and equal_bqual
  ...
  : Prims.bool= equal_opt equal_binder_qualifier b1 b2
and equal_binder_qualifier (b1 : FStarC_Syntax_Syntax.binder_qualifier)
  (b2 : FStarC_Syntax_Syntax.binder_qualifier) : Prims.bool=
  match (b1, b2) with
  | (FStarC_Syntax_Syntax.Implicit b11, FStarC_Syntax_Syntax.Implicit b21) ->
      b11 = b21
...
and equal_branch
  ...
  | ((p1, w1, t1), (p2, w2, t2)) ->
      ((equal_pat p1 p2) && (equal_opt equal_term w1 w2)) &&
        (equal_term t1 t2)
and equal_pat
  (p1 : FStarC_Syntax_Syntax.pat' FStarC_Syntax_Syntax.withinfo_t)
  (p2 : FStarC_Syntax_Syntax.pat' FStarC_Syntax_Syntax.withinfo_t) :
  Prims.bool=
  let uu___ = FStarC_Util.physical_equality p1 p2 in
  if uu___
  then true
  else
    (match ((p1.FStarC_Syntax_Syntax.v), (p2.FStarC_Syntax_Syntax.v)) with
...
and equal_meta (m1 : FStarC_Syntax_Syntax.metadata)
  ...
  | (FStarC_Syntax_Syntax.Meta_named l1, FStarC_Syntax_Syntax.Meta_named l2)
      -> FStarC_Ident.lid_equals l1 l2
  ...
  | (FStarC_Syntax_Syntax.Meta_monadic_lift (m11, n1, t1),
     FStarC_Syntax_Syntax.Meta_monadic_lift (m21, n2, t2)) ->
      ((FStarC_Ident.lid_equals m11 m21) && (FStarC_Ident.lid_equals n1 n2))
        && (equal_term t1 t2)
  | uu___ -> false
and equal_lazyinfo (l1 : FStarC_Syntax_Syntax.lazyinfo)
  (l2 : FStarC_Syntax_Syntax.lazyinfo) : Prims.bool=
  FStarC_Util.physical_equality l1.FStarC_Syntax_Syntax.blob
...
and equal_rc (r1 : FStarC_Syntax_Syntax.residual_comp)
  (r2 : FStarC_Syntax_Syntax.residual_comp) : Prims.bool=
...
and equal_flag (f1 : FStarC_Syntax_Syntax.cflag)
  (f2 : FStarC_Syntax_Syntax.cflag) : Prims.bool=
  match (f1, f2) with
...
and equal_decreases_order (d1 : FStarC_Syntax_Syntax.decreases_order)
  ...
  | (FStarC_Syntax_Syntax.Decreases_lex ts1,
     FStarC_Syntax_Syntax.Decreases_lex ts2) -> equal_list equal_term ts1 ts2
  | (FStarC_Syntax_Syntax.Decreases_wf (t1, t1'),
     FStarC_Syntax_Syntax.Decreases_wf (t2, t2')) ->
      (equal_term t1 t2) && (equal_term t1' t2')
  | uu___ -> false
and equal_arg_qualifier (a1 : FStarC_Syntax_Syntax.arg_qualifier)
  (a2 : FStarC_Syntax_Syntax.arg_qualifier) : Prims.bool=
...
and equal_lbname
  ...
  | (FStar_Pervasives.Inr f1, FStar_Pervasives.Inr f2) ->
      FStarC_Ident.lid_equals f1.FStarC_Syntax_Syntax.fv_name
        f2.FStarC_Syntax_Syntax.fv_name
  | uu___ -> false
and equal_subst_elt (s1 : FStarC_Syntax_Syntax.subst_elt)
  (s2 : FStarC_Syntax_Syntax.subst_elt) : Prims.bool=
  match (s1, s2) with
  | (FStarC_Syntax_Syntax.DB (i1, bv1), FStarC_Syntax_Syntax.DB (i2, bv2)) ->
      (i1 = i2) && (equal_bv bv1 bv2)
  | (FStarC_Syntax_Syntax.NM (bv1, i1), FStarC_Syntax_Syntax.NM (bv2, i2)) ->
      (i1 = i2) && (equal_bv bv1 bv2)
  | (FStarC_Syntax_Syntax.NT (bv1, t1), FStarC_Syntax_Syntax.NT (bv2, t2)) ->
  ...
  | (FStarC_Syntax_Syntax.DT (i1, t1), FStarC_Syntax_Syntax.DT (i2, t2)) ->
      (i1 = i2) && (equal_term t1 t2)
  | uu___ -> false
let hashable_term : FStarC_Syntax_Syntax.term FStarC_Class_Hashable.hashable=
    ...
    FStarC_Class_Hashable.hash =
      (fun uu___ ->
         ...
         | FStarC_Syntax_Syntax.Binding_var bv ->
             FStarC_Class_Hashable.hash hashable_term
...
let hashable_bv : FStarC_Syntax_Syntax.bv FStarC_Class_Hashable.hashable=
  {
    FStarC_Class_Hashable.hash =
      (fun b ->
         FStarC_Class_Hashable.hash hashable_term b.FStarC_Syntax_Syntax.sort)
  }
...
let rec hash_sigelt (se : FStarC_Syntax_Syntax.sigelt) :
  FStarC_Hash.hash_code= hash_sigelt' se.FStarC_Syntax_Syntax.sigel
and hash_sigelt' (se : FStarC_Syntax_Syntax.sigelt') : FStarC_Hash.hash_code=
      ...
      let uu___ =
        let uu___1 =
          let uu___2 =
            ...
            let uu___4 = FStarC_Class_Hashable.hash hashable_term t in
  ...
  | FStarC_Syntax_Syntax.Sig_datacon
      { FStarC_Syntax_Syntax.lid1 = lid; FStarC_Syntax_Syntax.us1 = us;
        ...
        FStarC_Syntax_Syntax.proj_disc_lids = uu___;_}
      ->
      let uu___1 =
        let uu___2 =
          let uu___3 =
            let uu___4 =
              let uu___5 =
                let uu___6 =
                  ...
                  let uu___8 = FStarC_Class_Hashable.hash hashable_lident lid in
                  FStarC_Hash.mix uu___7 uu___8 in
                let uu___7 =
                  FStarC_Class_Hashable.hash
                    (FStarC_Class_Hashable.hashable_list hashable_ident) us in
                FStarC_Hash.mix uu___6 uu___7 in
              let uu___6 = FStarC_Class_Hashable.hash hashable_term t in
              FStarC_Hash.mix uu___5 uu___6 in
            let uu___5 = FStarC_Class_Hashable.hash hashable_lident ty_lid in
            FStarC_Hash.mix uu___4 uu___5 in
          let uu___4 =
            FStarC_Class_Hashable.hash FStarC_Class_Hashable.hashable_int
              num_ty_params in
      ...
      let uu___ =
        ...
        let uu___2 =
          FStarC_Class_Hashable.hash
            (FStarC_Class_Hashable.hashable_list hashable_ident) us in
        FStarC_Hash.mix uu___1 uu___2 in
      let uu___1 = FStarC_Class_Hashable.hash hashable_term t in
      FStarC_Hash.mix uu___ uu___1
  | FStarC_Syntax_Syntax.Sig_let
      { FStarC_Syntax_Syntax.lbs1 = lbs; FStarC_Syntax_Syntax.lids1 = lids;_}
      ->
      let uu___ =
        let uu___1 =
          FStarC_Class_Hashable.hash FStarC_Class_Hashable.hashable_int
        ...
        let uu___2 =
          FStarC_Class_Hashable.hash
            (FStarC_Class_Hashable.hashable_tuple2
               ...
               (FStarC_Class_Hashable.hashable_list hashable_letbinding)) lbs in
        FStarC_Hash.mix uu___1 uu___2 in
      let uu___1 =
        FStarC_Class_Hashable.hash
          (FStarC_Class_Hashable.hashable_list hashable_lident) lids in
      ...
      let uu___ =
        ...
        let uu___2 =
          FStarC_Class_Hashable.hash
            (FStarC_Class_Hashable.hashable_list hashable_ident) us in
        FStarC_Hash.mix uu___1 uu___2 in
      let uu___1 = FStarC_Class_Hashable.hash hashable_term phi in
      FStarC_Hash.mix uu___ uu___1
