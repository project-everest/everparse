...
let __proj__Mkenv__item__guard_handler (projectee : env) :
...
let push_binder (g : env) (b : FStarC_Syntax_Syntax.binder) : env=
  if
    (b.FStarC_Syntax_Syntax.binder_bv).FStarC_Syntax_Syntax.index <=
      g.max_binder_index
  then
    failwith "Assertion failed: unexpected shadowing in the core environment"
  else
...
let __proj__Success__item___0 (projectee : 'a __result) : 'a=
...
let __proj__Mkhash_entry__item__he_res (projectee : hash_entry) :
  (tot_or_ghost * FStarC_Syntax_Syntax.typ) success=
...
let clear_memo_table (uu___ : unit) : unit=
  FStarC_Syntax_TermHashTable.clear table
let insert (g : env) (e : FStarC_Syntax_Syntax.term)
  (res : (tot_or_ghost * FStarC_Syntax_Syntax.typ) success) : unit=
  let entry =
    {
      he_term = e;
        ...
        let uu___1 =
          let uu___2 ctx01 =
            ...
            | Success (x2, g11) ->
                ...
                (match uu___4 with
                 | Success (y1, g2) ->
                     let uu___5 = let uu___6 = and_pre g11 g2 in (y1, uu___6) in
                     Success uu___5
                 | err -> err)
          ...
          (match uu___2 with
           | (g1, x1, c1) ->
               ...
               let uu___3 =
                 ...
                 (x1, eff, uu___4) in
               (fun uu___4 -> Success (uu___3, FStar_Pervasives_Native.None)))
        else
          (let e_tag =
           ...
           | FStar_Pervasives_Native.None ->
               let uu___3 =
                 let uu___4 =
                   ...
                   let uu___6 =
                     let uu___7 = FStarC_Syntax_Util.comp_effect_name c in
                     FStarC_Class_PP.pp FStarC_Ident.pretty_lident uu___7 in
                ...
                | (g1, x1::[], c1) ->
                    ...
                    (match uu___4 with
                     | (pre, uu___5)::(post, uu___6)::uu___7 ->
                         ...
                         let res_typ =
                           let r =
                             let uu___8 = FStarC_Syntax_Util.comp_result c1 in
...
let check_arg_qual (a : FStarC_Syntax_Syntax.aqual)
  ...
  | uu___ ->
      (match a with
       ...
       | uu___1 -> (fun uu___2 -> Success ((), FStar_Pervasives_Native.None)))
...
let close_guard_with_definition (x : FStarC_Syntax_Syntax.binder)
  ...
  | FStar_Pervasives_Native.Some t' ->
      ...
      FStar_Pervasives_Native.Some uu___
let with_binders (xs : FStarC_Syntax_Syntax.binders)
  ...
  fun ctx ->
    ...
    | Success (t, g) ->
        ...
        Success uu___1
    | err -> err
let with_definition (x : FStarC_Syntax_Syntax.binder)
  (u : FStarC_Syntax_Syntax.universe) (t : FStarC_Syntax_Syntax.term)
  (f : 'a result) : 'a result=
  fun ctx ->
    let uu___ = f ctx in
    ...
    | Success (a1, g) ->
        let uu___1 =
          let uu___2 = close_guard_with_definition x u t g in (a1, uu___2) in
        Success uu___1
    | err -> err
let guard (t : FStarC_Syntax_Syntax.typ) : unit result=
  fun uu___ -> Success ((), (FStar_Pervasives_Native.Some t))
let abs (a : FStarC_Syntax_Syntax.typ)
...
let weaken_subtyping_guard (p : FStarC_Syntax_Syntax.term) (g : precondition)
  : precondition=
  FStarC_Option.map (fun q -> FStarC_Syntax_Util.mk_imp p q) g
let strengthen_subtyping_guard (p : FStarC_Syntax_Syntax.term)
  (g : precondition) : precondition=
  let uu___ =
    let uu___1 =
      FStarC_Option.map (fun q -> FStarC_Syntax_Util.mk_conj p q) g in
    FStarC_Option.dflt p uu___1 in
  FStar_Pervasives_Native.Some uu___
let weaken (p : FStarC_Syntax_Syntax.term) (g : 'a result) (ctx : context) :
  'a success __result=
  let uu___ = g ctx in
  match uu___ with
  | Success (x, q) ->
      let uu___1 = let uu___2 = weaken_subtyping_guard p q in (x, uu___2) in
      Success uu___1
  | err -> err
let weaken_with_guard_formula (p : FStarC_TypeChecker_Common.guard_formula)
  (g : 'a result) : context -> 'a success __result=
  match p with
...
let strengthen (p : FStarC_Syntax_Syntax.term) (g : 'a result)
  (ctx : context) : 'a success __result=
  let uu___ = g ctx in
  match uu___ with
  | Success (x, q) ->
      let uu___1 = let uu___2 = strengthen_subtyping_guard p q in (x, uu___2) in
      Success uu___1
  | err -> err
...
let lookup (g : env) (e : FStarC_Syntax_Syntax.term) :
  ...
  | FStar_Pervasives_Native.Some he ->
      let uu___1 =
        context_included he.he_gamma (g.tcenv).FStarC_TypeChecker_Env.gamma in
      if uu___1
      then
        (record_cache_hit ();
          ...
          let res1 =
            ...
            | ((lbl, ty), prec) ->
                let ty1 =
                  ...
                  replace_all_use_ranges uu___5 ty in
                ((lbl, ty1), prec) in
          fun uu___4 -> Success res1))
  ...
  fun f x l1 l2 ->
    ...
    | (hd1::tl1, hd2::tl2) ->
        ...
        (fun ctx0 ->
           ...
           match uu___1 with
           | Success (x1, g1) ->
               let uu___2 = let uu___3 = fold2 f x1 tl1 tl2 in uu___3 ctx0 in
               (match uu___2 with
                | Success (y, g2) ->
                    let uu___3 = let uu___4 = and_pre g1 g2 in (y, uu___4) in
                    Success uu___3
  ...
  fun xs ys f b1 ->
    ...
    | (x::xs1, y::ys1) ->
        ...
        (fun ctx0 ->
           ...
           match uu___1 with
           | Success (x1, g1) ->
               let uu___2 = let uu___3 = iter2 xs1 ys1 f x1 in uu___3 ctx0 in
               (match uu___2 with
                | Success (y1, g2) ->
...
let rec check_relation (g : env) (rel : relation)
  ...
  (let uu___1 = FStarC_Effect.op_Bang dbg in
   ...
   then
     ...
     let uu___3 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t0 in
     let uu___4 =
       FStarC_Class_Tagged.tag_of FStarC_Syntax_Syntax.tagged_term t1 in
     let uu___5 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t1 in
     FStarC_Format.print5 "check_relation (%s) %s %s (%s) %s\n" uu___2 uu___3
           ...
           let uu___3 =
                   ...
                   let uu___5 =
                     let uu___6 =
                       ...
                       then
                         ...
                         fun uu___8 ->
                           Success (uu___7, FStar_Pervasives_Native.None)
                       else
                         (fun uu___8 ->
                            Success
                              (FStar_Pervasives_Native.None,
             ...
             let fallback t01 t11 =
               ...
               then
                 ...
                 (if uu___4 then emit_guard t01 t11 else err "not equatable")
             ...
             let beta_iota_reduce1 t =
               FStarC_Profiling.profile (fun uu___4 -> beta_iota_reduce t)
                 FStar_Pervasives_Native.None
             ...
             let check_relation1 g2 rel1 t02 t12 ctx =
               ...
               let uu___4 = check_relation g2 rel1 t02 t12 in uu___4 ctx1 in
             let uu___4 = equal_term t01 t11 in
             if uu___4
             then fun uu___5 -> Success ((), FStar_Pervasives_Native.None)
             else
               (match ((t01.FStarC_Syntax_Syntax.n),
                      ...
                      (fun ctx01 ->
                         ...
                         | Success (x2, g11) ->
                             let uu___9 =
                               let uu___10 =
                                 let uu___11 =
                                   universe_of g x0.FStarC_Syntax_Syntax.sort in
                                 fun ctx02 ->
                               ...
                               let uu___11 =
                                 ...
                                 | FStar_Pervasives_Native.Some (t02, t12) ->
                                     ...
                                     let uu___13 =
                                       FStarC_Syntax_Util.flatten_refinement
                                         rhs in
                                     check_relation1 g rel uu___12 uu___13 in
                               uu___11 ctx01 in
                             (match uu___10 with
                              | Success (y, g2) ->
                                  let uu___11 =
                    ...
                    then
                      let uu___8 =
                        ...
                        then
                          ...
                          (fun uu___10 ->
                             Success ((), FStar_Pervasives_Native.None)) in
                      (fun ctx01 ->
                         let uu___9 = uu___8 ctx01 in
                         match uu___9 with
                         | Success (x1, g11) ->
                               ...
                               let uu___12 =
                                 ...
                                 | FStar_Pervasives_Native.Some (t02, t12) ->
                                     ...
                                     let uu___13 =
                                       FStarC_Syntax_Util.flatten_refinement
                                         lhs in
                                     check_relation1 g rel uu___13 t12 in
                               uu___12 ctx01 in
                             (match uu___11 with
                              | Success (y, g2) ->
                                  let uu___12 =
                ...
                | (uu___6, FStarC_Syntax_Syntax.Tm_refine
                   { FStarC_Syntax_Syntax.b = x1;
                    ...
                    if uu___7
                    then
                      let uu___8 = universe_of g x1.FStarC_Syntax_Syntax.sort in
                      (fun ctx01 ->
                         let uu___9 = uu___8 ctx01 in
                         match uu___9 with
                         | Success (x2, g11) ->
                               ...
                               let uu___12 =
                                 ...
                                 | FStar_Pervasives_Native.Some (t02, t12) ->
                                     ...
                                     let uu___13 =
                                       FStarC_Syntax_Util.flatten_refinement
                                         rhs in
                                     check_relation1 g rel t02 uu___13 in
                               uu___12 ctx01 in
                             (match uu___11 with
                              | Success (y, g2) ->
                                  let uu___12 =
                          ...
                          | (head1, args1) ->
                              ...
                              then maybe_unfold_and_retry t01 t11
                                ...
                                (let compare_head_and_args uu___11 =
                                 ...
                                 then
                                   let uu___12 =
                                     let uu___13 = compare_head_and_args () in
                                     no_guard uu___13 in
                                   fun ctx ->
                                     let uu___13 = uu___12 ctx in
                                     match uu___13 with
                                     | Error uu___14 ->
                                         let uu___15 = emit_guard t01 t11 in
                                         uu___15 ctx
                                     | res -> res
                   ...
                   { FStarC_Syntax_Syntax.scrutinee = e1;
                    ...
                    let relate_branch br0 br1 uu___10 =
                      ...
                      | ((p0, FStar_Pervasives_Native.None, body0),
                         (p1, FStar_Pervasives_Native.None, body1)) ->
                          let uu___11 =
                            let uu___12 = FStarC_Syntax_Syntax.eq_pat p0 p1 in
                                ...
                                (p11, uu___15, body11)) ->
                                 ...
                                 (match uu___16 with
                                  | FStar_Pervasives_Native.Some
                                      ...
                                      let uu___18 = check_binders g bs0 in
                                      (fun ctx01 ->
                    ...
                    let uu___10 =
                      ...
                      fun ctx01 ->
                        ...
                        | Success (x1, g11) ->
                            let uu___13 =
                              let uu___14 = iter2 brs0 brs1 relate_branch () in
                              uu___14 ctx01 in
                            (match uu___13 with
                             | Success (y, g2) ->
                                 let uu___14 =
                    ...
                    (fun ctx ->
                       ...
                       match uu___11 with
                       | Error uu___12 ->
                           let uu___13 = fallback t01 t11 in uu___13 ctx
                       | res -> res)
      ...
      (fun uu___ uu___1 uu___2 ->
         ...
         | ((t0, q0), (t1, q1)) ->
             ...
             (fun ctx0 ->
                ...
                | Success (x, g1) ->
                    let uu___5 =
                      let uu___6 = check_relation g rel t0 t1 in uu___6 ctx0 in
                    (match uu___5 with
                     | Success (y, g2) ->
                         let uu___6 =
                           let uu___7 = and_pre g1 g2 in ((), uu___7) in
                         Success uu___6
                     | err -> err)
                | Error err -> Error err)) ()
        ...
        (let ct_eq res0 args0 res1 args1 =
           ...
           fun ctx0 ->
             ...
             | Success (x, g1) ->
                 ...
                 (match uu___6 with
                  | Success (y, g2) ->
                      let uu___7 = let uu___8 = and_pre g1 g2 in ((), uu___8) in
                      Success uu___7
                  | err -> err)
             | Error err -> Error err in
...
and check_subtype (g : env)
  ...
  (t0 : FStarC_Syntax_Syntax.typ) (t1 : FStarC_Syntax_Syntax.typ)
  (ctx : context) : unit success __result=
  FStarC_Profiling.profile
    (fun uu___ ->
  ...
  let check_then_memo g1 e1 ctx =
    ...
    | Success (res, FStar_Pervasives_Native.Some guard1) ->
        (match g1.guard_handler with
         | FStar_Pervasives_Native.None ->
             (insert g1 e1 (res, (FStar_Pervasives_Native.Some guard1)); r)
         | FStar_Pervasives_Native.Some gh ->
             ...
             then
               ...
               (let uu___2 = fail_str "guard handler failed" in uu___2 ctx))
    | uu___ -> r in
  fun ctx ->
      ...
      then
        ...
        (fun ctx0 ->
           ...
           | Success (x, g1) ->
               ...
               (match uu___6 with
                | Success (y, g2) ->
                    let uu___7 = let uu___8 = and_pre g1 g2 in (y, uu___8) in
                    Success uu___7
                | err -> err)
           | Error err -> Error err)
      else
         ...
         | Success (x, g1) ->
             let uu___2 =
               let uu___3 =
                 ...
                 | u::[] ->
                     (fun uu___4 ->
                        Success (([b], [u]), FStar_Pervasives_Native.None)) in
               uu___3 ctx0 in
             (match uu___2 with
              | Success (y, g2) ->
                  let uu___3 = let uu___4 = and_pre g1 g2 in (y, uu___4) in
                                       ...
                                       | ((g2, ss, bs, us1),
                                          {
                                           ...
                                           let uu___13 =
                                             ...
                                             with_binders bs us1 uu___14 in
                                           (fun ctx01 ->
                                              let uu___14 = uu___13 ctx01 in
                                              match uu___14 with
                                              | Success (x1, g11) ->
...
and pattern_branch_condition (g : env)
  ...
  | FStarC_Syntax_Syntax.Pat_var uu___ ->
      (fun uu___1 ->
         Success (FStar_Pervasives_Native.None, FStar_Pervasives_Native.None))
  | FStarC_Syntax_Syntax.Pat_constant c ->
      let const_exp =
        let uu___ =
      ...
      (fun ctx0 ->
         ...
         | Success (x, g1) ->
             ...
             (match uu___2 with
              ...
              | err -> err)
         | Error err -> Error err)
let initial_env (g : FStarC_TypeChecker_Env.env)
  (gh : guard_handler_t FStar_Pervasives_Native.option) : env=
  let max_index =
    FStarC_List.fold_left
      (fun index b ->
         match b with
...
let check_term_top (g : FStarC_TypeChecker_Env.env)
  ...
  (must_tot : Prims.bool)
  (gh : guard_handler_t FStar_Pervasives_Native.option) :
  (tot_or_ghost * FStarC_Syntax_Syntax.typ) result=
  ...
  fun ctx0 ->
    ...
    | Success (x, g11) ->
          ...
          let uu___3 =
            ...
            | FStar_Pervasives_Native.None ->
                ...
                then
                  ...
                  (fun uu___5 -> Success (x, FStar_Pervasives_Native.None))
            | FStar_Pervasives_Native.Some t ->
                let uu___4 =
                     ...
                     (fun ctx01 ->
                        ...
                        | Success (x1, g12) ->
                            let uu___7 =
                              let uu___8 uu___9 =
                                Success
                                  ((eff, t), FStar_Pervasives_Native.None) in
                              uu___8 ctx01 in
                            (match uu___7 with
                             | Success (y, g2) ->
                                 let uu___8 =
                                   let uu___9 = and_pre g12 g2 in (y, uu___9) in
                                 Success uu___8
                             | err -> err)
                        | Error err -> Error err)) in
          uu___3 ctx0 in
        (match uu___2 with
         | Success (y, g2) ->
             let uu___3 = let uu___4 = and_pre g11 g2 in (y, uu___4) in
             Success uu___3
         | err -> err)
    | Error err -> Error err
...
let check_term_top_gh (g : FStarC_TypeChecker_Env.env)
  (e : FStarC_Syntax_Syntax.term)
  (topt : FStarC_Syntax_Syntax.typ FStar_Pervasives_Native.option)
  (must_tot : Prims.bool)
  (gh : guard_handler_t FStar_Pervasives_Native.option) :
  ...
  (let ctx =
   ...
   let res =
     FStarC_Profiling.profile
       (fun uu___4 ->
          let uu___5 =
            let uu___6 = check_term_top g e topt must_tot gh in uu___6 ctx in
          match uu___5 with
                 ...
                 FStarC_Class_Setlike.elems ()
                   (Obj.magic
                      (FStarC_FlatSet.setlike_flat_set
                         FStarC_Syntax_Syntax.ord_bv)) (Obj.magic uu___7) in
               let uu___7 =
                 FStarC_List.tryFind
                   (fun bv ->
                      FStarC_List.for_all
                        (fun binding_env ->
                           ...
                           | uu___8 -> true) g.FStarC_TypeChecker_Env.gamma)
                   guard_names in
               match uu___7 with
               | FStar_Pervasives_Native.Some bv ->
                   let uu___8 =
                     let uu___9 = FStarC_Syntax_Syntax.bv_to_name bv in
                     FStarC_Class_Show.show FStarC_Syntax_Print.showable_term
                       uu___9 in
...
let check_term_at_type (g : FStarC_TypeChecker_Env.env)
  ...
  | Error err -> FStar_Pervasives.Inr err
let compute_term_type_handle_guards (g : FStarC_TypeChecker_Env.env)
  (e : FStarC_Syntax_Syntax.term)
  (gh : FStarC_TypeChecker_Env.env -> FStarC_Syntax_Syntax.typ -> Prims.bool)
  :
  ((tot_or_ghost * FStarC_Syntax_Syntax.typ), error) FStar_Pervasives.either=
  ...
  | Success (uu___1, FStar_Pervasives_Native.Some uu___2) ->
      failwith "Impossible: All guards should have been handled already"
  | Error err -> FStar_Pervasives.Inr err
