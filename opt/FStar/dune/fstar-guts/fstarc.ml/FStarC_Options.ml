...
let snapshot (uu___ : unit) : (Prims.int * unit)=
  FStarC_Common.snapshot push history ()
let rollback (depth1 : Prims.int FStar_Pervasives_Native.option) : unit=
  FStarC_Common.rollback pop history depth1
...
let get_in (uu___ : unit) : Prims.bool= lookup_opt "in" as_bool
...
let get_lsp (uu___ : unit) : Prims.bool= lookup_opt "lsp" as_bool
...
let interactive (uu___ : unit) : Prims.bool=
  ((get_in ()) || (get_ide ())) || (get_lsp ())
...
let legacy_interactive (uu___ : unit) : Prims.bool= get_in ()
let lsp_server (uu___ : unit) : Prims.bool= get_lsp ()
