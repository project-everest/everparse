open Prims
let tc_one_file (remaining : Prims.string Prims.list)
...
let pop (env : FStarC_TypeChecker_Env.env) (msg : Prims.string) : unit=
  (let uu___1 = FStarC_TypeChecker_Tc.pop_context env msg in ());
  FStarC_Options.pop ()
let push_with_kind (env : FStarC_TypeChecker_Env.env) (lax : Prims.bool)
  (restore_cmd_line_options : Prims.bool) (msg : Prims.string) :
  ...
  then (let uu___2 = FStarC_Options.restore_cmd_line_options false in ())
  else ();
  res
let check_frag (env : FStarC_TypeChecker_Env.env)
  (curmod : FStarC_Syntax_Syntax.modul FStar_Pervasives_Native.option)
  (frag : (FStarC_Parser_ParseIt.input_frag * FStarC_Universal.lang_decls_t))
  :
  (FStarC_Syntax_Syntax.modul FStar_Pervasives_Native.option *
    FStarC_TypeChecker_Env.env * Prims.int) FStar_Pervasives_Native.option=
  try
    (fun uu___ ->
       match () with
       | () ->
  ...
  | FStarC_Errors.Error (e, msg, r, ctx) when
      let uu___1 = FStarC_Options.trace_error () in Prims.op_Negation uu___1
      ->
      (FStarC_TypeChecker_Err.add_errors env [(e, msg, r, ctx)];
       FStar_Pervasives_Native.None)
let report_fail (uu___ : unit) : unit=
...
let uu___is_Push (projectee : input_chunks) : Prims.bool=
  match projectee with | Push _0 -> true | uu___ -> false
let __proj__Push__item___0 (projectee : input_chunks) :
  (Prims.bool * Prims.int * Prims.int)= match projectee with | Push _0 -> _0
let uu___is_Pop (projectee : input_chunks) : Prims.bool=
  match projectee with | Pop _0 -> true | uu___ -> false
let __proj__Pop__item___0 (projectee : input_chunks) : Prims.string=
  match projectee with | Pop _0 -> _0
let uu___is_Code (projectee : input_chunks) : Prims.bool=
  match projectee with | Code _0 -> true | uu___ -> false
let __proj__Code__item___0 (projectee : input_chunks) :
  (Prims.string * (Prims.string * Prims.string))=
  match projectee with | Code _0 -> _0
let uu___is_Info (projectee : input_chunks) : Prims.bool=
  match projectee with | Info _0 -> true | uu___ -> false
let __proj__Info__item___0 (projectee : input_chunks) :
  (Prims.string * Prims.bool * (Prims.string * Prims.int * Prims.int)
    FStar_Pervasives_Native.option)=
  match projectee with | Info _0 -> _0
let uu___is_Completions (projectee : input_chunks) : Prims.bool=
  match projectee with | Completions _0 -> true | uu___ -> false
let __proj__Completions__item___0 (projectee : input_chunks) : Prims.string=
  match projectee with | Completions _0 -> _0
...
let __proj__Mkinteractive_state__item__chunk (projectee : interactive_state)
  : FStarC_StringBuffer.t=
  match projectee with | { chunk; stdin; buffer; log;_} -> chunk
let __proj__Mkinteractive_state__item__stdin (projectee : interactive_state)
  :
  FStarC_Util.stream_reader FStar_Pervasives_Native.option FStarC_Effect.ref=
  match projectee with | { chunk; stdin; buffer; log;_} -> stdin
let __proj__Mkinteractive_state__item__buffer (projectee : interactive_state)
  : input_chunks Prims.list FStarC_Effect.ref=
  match projectee with | { chunk; stdin; buffer; log;_} -> buffer
let __proj__Mkinteractive_state__item__log (projectee : interactive_state) :
  FStarC_Util.out_channel FStar_Pervasives_Native.option FStarC_Effect.ref=
...
let rec read_chunk (uu___ : unit) : input_chunks=
  ...
  let log =
    ...
    then
      let transcript =
        ...
        | FStar_Pervasives_Native.None ->
            let transcript1 = FStarC_Util.open_file_for_writing "transcript" in
...
let deps_of_our_file (filename : Prims.string) :
                 ...
                 ((let uu___3 =
                     (let uu___4 = FStarC_Parser_Dep.is_interface intf in
                      Prims.op_Negation uu___4) ||
                       (let uu___4 = FStarC_Parser_Dep.is_implementation impl in
...
let rec tc_deps (m : modul_t) (stack : stack_t)
  (env : FStarC_TypeChecker_Env.env) (remaining : Prims.string Prims.list)
  (ts : m_timestamps) :
...
let update_deps (filename : Prims.string) (m : modul_t) (stk : stack_t)
  (env : env_t) (ts : m_timestamps) : (stack_t * env_t * m_timestamps)=
  let is_stale intf impl intf_t impl_t =
    let impl_mt = FStarC_Parser_ParseIt.get_file_last_modification_time impl in
    (FStarC_Time.is_before impl_t impl_mt) ||
      (match (intf, intf_t) with
       | (FStar_Pervasives_Native.Some intf1, FStar_Pervasives_Native.Some
          intf_t1) ->
           let intf_mt =
             FStarC_Parser_ParseIt.get_file_last_modification_time intf1 in
    ...
    let match_dep depnames1 intf impl =
      ...
      | FStar_Pervasives_Native.None ->
          (match depnames1 with
           | dep::depnames' ->
               if dep = impl then (true, depnames') else (false, depnames1)
      ...
      | uu___::ts3 ->
          (pop env1 "";
           (let uu___2 =
            ...
            match uu___2 with
            | ((env2, uu___3), stack1) -> pop_tc_and_stack env2 stack1 ts3)) in
    match ts1 with
    | ts_elt::ts' ->
        let uu___ = ts_elt in
        (match uu___ with
         | (intf, impl, intf_t, impl_t) ->
             ...
             (match uu___1 with
              | (b, depnames') ->
                  ...
                  then
                    let env1 =
                      pop_tc_and_stack env' (FStarC_List.rev_append st [])
                        ts1 in
                    tc_deps m good_stack env1 depnames good_ts
                  else
                    (let uu___4 =
                       let uu___5 = FStarC_List.hd st in
                       let uu___6 = FStarC_List.tl st in (uu___5, uu___6) in
                     match uu___4 with
                     | (stack_elt, st') ->
...
let format_info (env : FStarC_TypeChecker_Env.env) (name : Prims.string)
  (typ : FStarC_Syntax_Syntax.term) (range : FStarC_Range_Type.range)
  (doc : Prims.string FStar_Pervasives_Native.option) : Prims.string=
  ...
  let uu___2 =
    ...
    | FStar_Pervasives_Native.None -> "" in
  FStarC_Format.fmt4 "(defined at %s) %s: %s%s" uu___ name uu___1 uu___2
let rec go (line_col : (Prims.int * Prims.int)) (filename : Prims.string)
  (stack : stack_t) (curmod : modul_t) (env : env_t) (ts : m_timestamps) :
  unit=
        ...
        | FStar_Pervasives_Native.None ->
            ...
            then FStar_Pervasives_Native.None
              ...
              (let lid =
               ...
               let uu___2 = FStarC_TypeChecker_Env.try_lookup_lid env lid1 in
               FStarC_Option.map
                 (fun uu___3 ->
                    match uu___3 with
                    | ((uu___4, typ), r) ->
                        ((FStar_Pervasives.Inr lid1), typ, r)) uu___2) in
      ((match info_opt with
        | FStar_Pervasives_Native.None ->
  ...
  | Completions search_term ->
      let rec measure_anchored_match search_term1 candidate =
        ...
        | ([], uu___1) -> FStar_Pervasives_Native.Some ([], Prims.int_zero)
      ...
      let str_of_ids ids =
        let uu___1 = FStarC_List.map FStarC_Ident.string_of_id ids in
        FStarC_Util.concat_l "." uu___1 in
      let match_lident_against needle lident =
        let uu___1 =
          let uu___2 = FStarC_Ident.ns_of_lid lident in
          let uu___3 =
            let uu___4 = FStarC_Ident.ident_of_lid lident in [uu___4] in
          FStarC_List.op_At uu___2 uu___3 in
      ...
      let shorten_namespace uu___1 =
        ...
        | (prefix, matched, match_len) ->
            let naked_match =
              match matched with | uu___2::[] -> true | uu___2 -> false in
            let uu___2 =
              FStarC_Syntax_DsEnv.shorten_module_path
                env.FStarC_TypeChecker_Env.dsenv prefix naked_match in
            (match uu___2 with
      ...
      let prepare_candidate uu___1 =
        ...
        | (prefix, matched, stripped_ns, match_len) ->
            ...
            then (matched, stripped_ns, match_len)
              ...
              ((Prims.strcat prefix (Prims.strcat "." matched)), stripped_ns,
                (((FStarC_String.length prefix) + match_len) + Prims.int_one)) in
      ...
      let matches =
        let case_a_find_transitive_includes orig_ns m id =
          ...
          let matched_length =
            FStarC_List.fold_left
              (fun out s -> ((FStarC_String.length s) + out) + Prims.int_one)
              (FStarC_String.length id) orig_ns in
               ...
               then
                 ...
                 FStarC_Option.map
                   (fun fqn ->
                      let uu___2 =
                        let uu___3 =
                          FStarC_List.map FStarC_Ident.id_of_text orig_ns in
                        let uu___4 =
                          let uu___5 = FStarC_Ident.ident_of_lid fqn in
                          [uu___5] in
               ...
               | (ns, id, uu___3) ->
                   ...
                   (match uu___4 with
                    ...
                    | FStar_Pervasives_Native.Some l ->
                        let uu___5 =
                          FStarC_Ident.lid_of_ids (FStarC_List.op_At ns id) in
                        FStarC_Ident.lid_equals l uu___5)) matches1 in
        ...
        | (ns, id) ->
            let matched_ids =
              ...
              | uu___2 ->
                  let l =
                    FStarC_Ident.lid_of_path ns FStarC_Range_Type.dummyRange in
                  let uu___3 =
            ...
            FStarC_List.map
              (fun x ->
                 let uu___2 = shorten_namespace x in prepare_candidate uu___2)
              matched_ids in
      ((let uu___2 =
  ...
  | Push (lax, l, c) ->
      ...
      (match uu___1 with
       | (restore_cmd_line_options, (stack1, env1, ts1)) ->
           ...
           let env2 =
             push_with_kind env1 lax restore_cmd_line_options "#push" in
...
let interactive_mode (filename : Prims.string) : unit=
  ...
  (let uu___1 = deps_of_our_file filename in
   ...
   | (filenames, maybe_intf, dep_graph) ->
       ...
       let uu___2 = tc_deps FStar_Pervasives_Native.None [] env filenames [] in
       (match uu___2 with
        | (stack, env1, ts) ->
            let initial_range =
              let uu___3 =
