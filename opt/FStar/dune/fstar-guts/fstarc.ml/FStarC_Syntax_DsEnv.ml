...
let __proj__Mkenv__item__curmodule (projectee : env) :
  ...
  | { curmodule; curmonad; modules; scope_mods; exported_ids;
      ...
      expect_typ; remaining_iface_decls; syntax_only; ds_hooks; dep_graph;
      no_prelude;_} -> curmodule
...
let __proj__Mkenv__item__trans_exported_ids (projectee : env) :
  ...
  | { curmodule; curmonad; modules; scope_mods; exported_ids;
      trans_exported_ids; includes; sigaccum; sigmap; iface; admitted_iface;
      expect_typ; remaining_iface_decls; syntax_only; ds_hooks; dep_graph;
      no_prelude;_} -> trans_exported_ids
...
let is_open (env1 : env) (lid : FStarC_Ident.lident)
  ...
  FStarC_List.existsb
    (fun uu___ ->
       match uu___ with
       | Open_module_or_namespace (ns, k, FStarC_Syntax_Syntax.Unrestricted)
...
let push_namespace' (env1 : env) (ns : FStarC_Ident.lident)
          ...
          (let uu___5 =
             let uu___6 =
               let uu___7 =
                 ...
                 FStarC_Format.fmt1 "Namespace '%s' cannot be found." uu___8 in
  ...
  | (ns', kd) ->
      ((env1.ds_hooks).ds_push_open_hook env1 (ns', kd, restriction);
       push_scope_mod env1 (Open_module_or_namespace (ns', kd, restriction)))
...
let snapshot (env1 : env) : (Prims.int * env)=
  FStarC_Common.snapshot push stack env1
let rollback (depth : Prims.int FStar_Pervasives_Native.option) : env=
  FStarC_Common.rollback pop stack depth
...
let prepare_module_or_interface (intf : Prims.bool) (admitted : Prims.bool)
  (env1 : env) (mname : FStarC_Ident.lident) (mii : module_inclusion_info) :
  (env * Prims.bool)=
  let prep env2 =
    let filename =
    ...
    let auto_open2 =
      FStarC_List.op_At namespace_of_module (FStarC_List.rev auto_open1) in
                ...
                (match () with
                 | () ->
                     let env' =
                       let uu___4 =
                         FStarC_List.map
                           (fun x -> Open_module_or_namespace x) auto_open2 in
                       {
      ...
      then
        let uu___3 =
          let uu___4 =
            ...
            let uu___6 = FStarC_Class_PP.pp FStarC_Ident.pretty_lident lid in
            FStar_Pprint.prefix (Prims.of_int (2)) Prims.int_one uu___5
              uu___6 in
          let uu___5 =
            let uu___6 =
              let uu___7 = FStarC_Ident.string_of_lid lid in
              let uu___8 = all_local_names env1 in typo_msg uu___7 uu___8 in
            [uu___6] in
          uu___4 :: uu___5 in
