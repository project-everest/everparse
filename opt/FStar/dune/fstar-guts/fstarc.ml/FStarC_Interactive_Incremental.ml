...
type fragment_progress =
  ...
  | FullBufferFinished 
let uu___is_FullBufferStarted (projectee : fragment_progress) : Prims.bool=
  match projectee with | FullBufferStarted -> true | uu___ -> false
let uu___is_FragmentStarted (projectee : fragment_progress) : Prims.bool=
  match projectee with | FragmentStarted _0 -> true | uu___ -> false
let __proj__FragmentStarted__item___0 (projectee : fragment_progress) :
  FStarC_Parser_AST.decl= match projectee with | FragmentStarted _0 -> _0
let uu___is_FragmentSuccess (projectee : fragment_progress) : Prims.bool=
  match projectee with | FragmentSuccess _0 -> true | uu___ -> false
let __proj__FragmentSuccess__item___0 (projectee : fragment_progress) :
  (FStarC_Parser_AST.decl * FStarC_Parser_ParseIt.code_fragment *
    FStarC_Interactive_Ide_Types.push_kind)=
  match projectee with | FragmentSuccess _0 -> _0
let uu___is_FragmentFailed (projectee : fragment_progress) : Prims.bool=
  match projectee with | FragmentFailed _0 -> true | uu___ -> false
let __proj__FragmentFailed__item___0 (projectee : fragment_progress) :
  FStarC_Parser_AST.decl= match projectee with | FragmentFailed _0 -> _0
let uu___is_FragmentError (projectee : fragment_progress) : Prims.bool=
  match projectee with | FragmentError _0 -> true | uu___ -> false
let __proj__FragmentError__item___0 (projectee : fragment_progress) :
  FStarC_Errors.issue Prims.list=
  match projectee with | FragmentError _0 -> _0
let uu___is_FullBufferFinished (projectee : fragment_progress) : Prims.bool=
  match projectee with | FullBufferFinished -> true | uu___ -> false
type qid = (Prims.string * Prims.int)
type 'a qst = qid -> ('a * qid)
let return (x : 'a) : 'a qst= fun q -> (x, q)
let op_let_Bang (f : 'a qst) (g : 'a -> 'b qst) : 'b qst=
  fun q ->
    let uu___ = f q in
    match uu___ with | (x, q') -> let uu___1 = g x in uu___1 q'
let run_qst (f : 'a qst) (q : Prims.string) : 'a=
...
let shift_qid (q : qid) (i : Prims.int) : (Prims.string * Prims.int)=
  ((FStar_Pervasives_Native.fst q), ((FStar_Pervasives_Native.snd q) + i))
let next_qid : qid qst=
  fun q -> let q1 = shift_qid q Prims.int_one in (q1, q1)
let get_qid : qid qst= fun q -> (q, q)
let as_query (q : FStarC_Interactive_Ide_Types.query') :
  ...
  op_let_Bang next_qid
    (fun uu___ ->
       ...
       | (qid_prefix, i) ->
           ...
           return uu___1)
let dump_symbols_for_lid (l : FStarC_Ident.lident) :
  FStarC_Interactive_Ide_Types.query qst=
...
let push_decl (push_kind : FStarC_Interactive_Ide_Types.push_kind)
  (with_symbols : Prims.bool)
  (write_full_buffer_fragment_progress : fragment_progress -> unit)
  (ds : (FStarC_Parser_AST.decl * FStarC_Parser_ParseIt.code_fragment)) :
  FStarC_Interactive_Ide_Types.query Prims.list qst=
  let uu___ = ds in
  match uu___ with
  | (d, s) ->
...
let push_decls (push_kind : FStarC_Interactive_Ide_Types.push_kind)
  (with_symbols : Prims.bool)
  (write_full_buffer_fragment_progress : fragment_progress -> unit)
  (ds :
  ...
  let uu___ =
    map
      (push_decl push_kind with_symbols write_full_buffer_fragment_progress)
      ds in
  op_let_Bang uu___ (fun qs -> return (FStarC_List.flatten qs))
let pop_entries
  (e : FStarC_Interactive_Ide_Types.repl_stack_entry_t Prims.list) :
  FStarC_Interactive_Ide_Types.query Prims.list qst=
  map (fun uu___ -> as_query FStarC_Interactive_Ide_Types.Pop) e
let repl_task (uu___ : ('uuuuu * ('uuuuu1 * 'uuuuu2))) : 'uuuuu1=
  match uu___ with | (uu___1, (p, uu___2)) -> p
let push_kind_geq (pk1 : FStarC_Interactive_Ide_Types.push_kind)
  ...
  (pk1 = pk2) ||
    (match (pk1, pk2) with
     ...
     | uu___ -> false)
let inspect_repl_stack (s : FStarC_Interactive_Ide_Types.repl_stack_t)
  (ds :
    (FStarC_Parser_AST.decl * FStarC_Parser_ParseIt.code_fragment) Prims.list)
  (push_kind : FStarC_Interactive_Ide_Types.push_kind)
  (with_symbols : Prims.bool)
  (write_full_buffer_fragment_progress : fragment_progress -> unit) :
  ...
  let push_decls1 =
    push_decls push_kind with_symbols write_full_buffer_fragment_progress in
  let uu___ =
    FStarC_Util.prefix_until
      (fun uu___1 ->
         match uu___1 with
         | (uu___2,
            (FStarC_Interactive_Ide_Types.PushFragment uu___3, uu___4)) ->
             true
  ...
  | FStar_Pervasives_Native.Some (prefix, first_push, rest) ->
      ...
      let repl_task1 uu___1 = match uu___1 with | (uu___2, (p, uu___3)) -> p in
      let rec matching_prefix accum lookups entries2 ds1 =
                            ...
                            op_let_Bang uu___4
                              (fun lookups' ->
                                 matching_prefix
                                   ...
                                   (FStarC_List.op_At lookups' lookups)
                                   entries3 ds2))
                         else
        ...
        | (es, []) ->
            ...
            op_let_Bang uu___1
              (fun pops -> return ((FStarC_List.op_At lookups pops), accum)) in
...
let parse_code (lang : FStarC_Parser_ParseIt.lang_opts) (code : Prims.string)
  : FStarC_Parser_ParseIt.parse_result=
  let uu___ =
    let uu___1 =
      let uu___2 =
  ...
  | (raw_error, msg, range) ->
      ...
      (match uu___1 with
       | (uu___2, uu___3, num) ->
           ...
           issue)
let run_full_buffer (st : FStarC_Interactive_Ide_Types.repl_state)
  (qid1 : Prims.string) (code : Prims.string)
  (request_type : FStarC_Interactive_Ide_Types.full_buffer_request_kind)
  (with_symbols : Prims.bool)
  (write_full_buffer_fragment_progress : fragment_progress -> unit) :
  (FStarC_Interactive_Ide_Types.query Prims.list * FStarC_Json.json
    Prims.list)=
  ...
  let filter_decls decls =
    ...
    | FStarC_Interactive_Ide_Types.VerifyToPosition (uu___, line, _col) ->
        FStarC_List.filter
          (fun uu___1 ->
             match uu___1 with
             | (d, uu___2) ->
    ...
    | FStarC_Interactive_Ide_Types.LaxToPosition (uu___, line, _col) ->
        FStarC_List.filter
          (fun uu___1 ->
             match uu___1 with
             | (d, uu___2) ->
  ...
  let qs =
    ...
    | FStarC_Parser_ParseIt.IncrementalFragment (decls, uu___, err_opt) ->
        ((let uu___2 =
          ...
          | (FStarC_Interactive_Ide_Types.ReloadDeps, d::uu___2) ->
              let uu___3 =
                ...
                op_let_Bang uu___4
                  (fun queries ->
                     let uu___5 =
                       push_decl FStarC_Interactive_Ide_Types.FullCheck
                         with_symbols write_full_buffer_fragment_progress d in
                     op_let_Bang uu___5
                       (fun push_mod ->
                          return ((FStarC_List.op_At queries push_mod), []))) in
              run_qst uu___3 qid1
          | uu___2 ->
              let decls1 = filter_decls decls in
              let push_kind =
                match request_type with
                | FStarC_Interactive_Ide_Types.LaxToPosition uu___3 ->
                    FStarC_Interactive_Ide_Types.LaxCheck
                | FStarC_Interactive_Ide_Types.Lax ->
                    FStarC_Interactive_Ide_Types.LaxCheck
                | uu___3 -> FStarC_Interactive_Ide_Types.FullCheck in
              let uu___3 =
                let uu___4 =
                  let uu___5 =
                    FStarC_Effect.op_Bang
                      FStarC_Interactive_PushHelper.repl_stack in
                  inspect_repl_stack uu___5 decls1 push_kind with_symbols
        ...
        FStarC_List.fold_left
          (fun uu___1 uu___2 ->
             ...
             | ((out, comments1), (d, uu___3)) ->
                 let uu___4 =
                   FStarC_Parser_ToDocument.decl_with_comments_to_document d
                     comments1 in
                 (match uu___4 with
                  | (doc, comments2) ->
                      let uu___5 =
