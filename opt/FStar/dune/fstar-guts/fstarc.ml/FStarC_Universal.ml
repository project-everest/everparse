...
let parse (env : uenv) (pre_fn : Prims.string FStar_Pervasives_Native.option)
  (fn : Prims.string) : (FStarC_Syntax_Syntax.modul * uenv)=
  ...
  | (ast, uu___1) ->
      ...
      (match uu___2 with
       | (ast1, env1) ->
           let uu___3 = FStarC_ToSyntax_ToSyntax.ast_modul_to_modul ast1 in
...
let tc_one_fragment
  ...
  let fname env1 =
    ...
    else (let uu___2 = FStarC_Options.file_list () in FStarC_List.hd uu___2) in
             ...
             ((let uu___3 =
              ...
              (let uu___3 =
               ...
               | (modul1, env3) ->
                   ...
                   ((FStar_Pervasives_Native.Some modul1), env3, lang_decls)))) in
          ...
          FStarC_Util.fold_map
            (fun env1 a_decl ->
               let uu___1 =
                 ...
                 with_dsenv_of_tcenv env1 uu___2 in
               match uu___1 with | (decls, env2) -> (env2, decls)) env
            ast_decls in
        (match uu___ with
         | (env1, ast_decls_l) ->
             let uu___1 =
              ...
              | (sigelts, env2) ->
                  ...
                  (match uu___2 with
                   | (modul1, uu___3, env3) ->
                       ...
                       ((FStar_Pervasives_Native.Some modul1), env3, uu___4)))) in
  match frag with
...
let tc_one_file (env : uenv)
  (pre_fn : Prims.string FStar_Pervasives_Native.option) (fn : Prims.string)
       ...
       (let restore_opts uu___2 =
        ...
        let tc_source_file uu___2 =
          let uu___3 =
            ...
            FStarC_Profiling.profile (fun uu___5 -> parse env pre_fn fn)
              uu___4 "FStarC.Universal.tc_source_file.parse" in
          match uu___3 with
          | (fmod, env1) ->
              let mii =
                let uu___4 =
                  let uu___5 = FStarC_Extraction_ML_UEnv.tcenv_of_uenv env1 in
                    ...
                    (fun tcenv ->
                       ...
                       (let uu___7 =
                          FStarC_TypeChecker_Tc.check_module tcenv fmod
                            (FStar_Pervasives_Native.uu___is_Some pre_fn) in
                        match uu___7 with
                        | (modul, env3) ->
                            (restore_opts ();
                             (let smt_decls =
                                ...
                                if uu___9
                                then
                                  FStarC_SMTEncoding_Encode.encode_modul env3
                                    modul
                                else ([], []) in
...
let tc_one_file_for_ide (env : FStarC_TypeChecker_Env.env_t)
  (pre_fn : Prims.string FStar_Pervasives_Native.option) (fn : Prims.string)
  (parsing_data : FStarC_Parser_Dep.parsing_data) :
...
let tc_one_file_from_remaining (remaining : Prims.string Prims.list)
  (env : uenv) (deps : FStarC_Parser_Dep.deps) :
  ...
  let uu___ =
    ...
    | intf::impl::remaining1 when needs_interleaving intf impl ->
        let uu___1 =
          ...
          tc_one_file env (FStar_Pervasives_Native.Some intf) impl uu___2 in
        (match uu___1 with
         | (m, mllib, env1) -> (remaining1, (m, mllib, env1)))
    | intf_or_impl::remaining1 ->
        let uu___1 =
          let uu___2 = FStarC_Parser_Dep.parsing_data_of deps intf_or_impl in
          tc_one_file env FStar_Pervasives_Native.None intf_or_impl uu___2 in
        (match uu___1 with
         | (m, mllib, env1) -> (remaining1, (m, mllib, env1)))
    | [] -> failwith "Impossible: Empty remaining modules" in
  match uu___ with
  | (remaining1, (nmods, mllib, env1)) -> (remaining1, nmods, mllib, env1)
let rec tc_fold_interleave (deps : FStarC_Parser_Dep.deps)
  (acc :
  ...
  | uu___ ->
      ...
      (match uu___1 with
       | (mods, mllibs, env_before) ->
           let uu___2 = tc_one_file_from_remaining remaining env_before deps in
           (match uu___2 with
...
let batch_mode_tc (filenames : Prims.string Prims.list)
