...
type pruning_state =
  ...
  assumption_to_triggers: triggers_set FStarC_PSMap.t ;
...
let __proj__Mkpruning_state__item__trigger_to_assumption
  ...
  match projectee with
  | { macro_freenames; trigger_to_assumption; assumption_to_triggers;
      assumption_name_map; ambients; extra_roots;_} -> trigger_to_assumption
let __proj__Mkpruning_state__item__assumption_to_triggers
  (projectee : pruning_state) : triggers_set FStarC_PSMap.t=
  match projectee with
  | { macro_freenames; trigger_to_assumption; assumption_to_triggers;
      assumption_name_map; ambients; extra_roots;_} -> assumption_to_triggers
...
let remove_trigger_for_assumption (p : pruning_state) (trig : Prims.string)
  (aname : Prims.string) : (pruning_state * Prims.bool)=
             ...
             (fun ts ->
                Obj.magic
                  (FStarC_Class_Setlike.remove ()
                     (Obj.magic
                        (FStarC_RBSet.setlike_rbset
                           FStarC_Class_Ord.ord_string)) trig (Obj.magic ts)))
               uu___1) l in
      let eligible =
          ...
          (fun uu___1 ->
             (Obj.magic
                (FStarC_Class_Setlike.is_empty ()
                   (Obj.magic
                      (FStarC_RBSet.setlike_rbset FStarC_Class_Ord.ord_string))))
               uu___1) remaining_triggers in
      let uu___1 =
        let uu___2 =
          FStarC_PSMap.add p.assumption_to_triggers aname remaining_triggers in
        {
          macro_freenames = (p.macro_freenames);
          trigger_to_assumption = (p.trigger_to_assumption);
          assumption_to_triggers = uu___2;
  ...
  | FStarC_SMTEncoding_Term.DefineFun (macro, uu___, uu___1, body, uu___2) ->
      let free_names =
        ...
        FStarC_Class_Setlike.elems ()
          ...
          (Obj.magic uu___3) in
      let p1 =
        let uu___3 = FStarC_PSMap.add p.macro_freenames macro free_names in
        {
       ...
       (FStarC_Class_Monad.op_let_Bang st_monad () () (Obj.magic get)
          (fun uu___ ->
             (fun ctxt1 ->
                let ctxt1 = Obj.magic ctxt1 in
                let uu___ =
                  FStarC_PSMap.try_find (ctxt1.p).trigger_to_assumption x in
                match uu___ with
                | FStar_Pervasives_Native.None ->
                    Obj.magic
...
let remove_trigger_for (uu___1 : sym)
  (uu___ : FStarC_SMTEncoding_Term.assumption) : Prims.bool st=
  (fun trig a ->
     Obj.magic
       (FStarC_Class_Monad.op_let_Bang st_monad () () (Obj.magic get)
          (fun uu___ ->
                            ...
                            (fun uu___2 ->
                               ...
                               Obj.magic
                                 (FStarC_Class_Monad.return st_monad ()
                                    (Obj.magic eligible))) uu___2))) uu___)))
    uu___1 uu___
  ...
  FStarC_Class_Monad.op_let_Bang st_monad () () (Obj.magic get)
    (fun uu___ ->
       (fun ctxt1 ->
          let ctxt1 = Obj.magic ctxt1 in
          let macro_expand s =
            let uu___ = FStarC_PSMap.try_find (ctxt1.p).macro_freenames s in
            match uu___ with
...
let prune (p : pruning_state)
  (roots : FStarC_SMTEncoding_Term.decl Prims.list) :
