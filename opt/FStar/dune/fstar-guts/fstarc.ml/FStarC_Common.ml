open Prims
let snapshot (push : 'a -> 'b) (stackref : 'c Prims.list FStarC_Effect.ref)
  (arg : 'a) : (Prims.int * 'b)=
...
let rollback (pop : unit -> 'a) (stackref : 'c Prims.list FStarC_Effect.ref)
  ...
  let curdepth =
    let uu___ = FStarC_Effect.op_Bang stackref in FStarC_List.length uu___ in
  let n =
    match depth with
