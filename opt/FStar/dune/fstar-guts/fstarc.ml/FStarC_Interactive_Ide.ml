...
let with_captured_errors' (env : FStarC_TypeChecker_Env.env)
  (sigint_handler : FStarC_Util.sigint_handler)
  (f : FStarC_TypeChecker_Env.env -> 'uuuuu FStar_Pervasives_Native.option) :
...
let unpack_interactive_query (json : FStarC_Json.json) :
...
let deserialize_interactive_query (js_query : FStarC_Json.json) :
  FStarC_Interactive_Ide_Types.query=
  try
...
let parse_interactive_query (query_str : Prims.string) :
  FStarC_Interactive_Ide_Types.query=
      ...
      ((let uu___2 = FStarC_Options.ide_id_info_off () in
       ...
       (let frag =
        ...
        let uu___2 =
          run_repl_transaction st1 (FStar_Pervasives_Native.Some push_kind)
            peek_only
            (FStarC_Interactive_Ide_Types.PushFragment (frag, push_kind, [])) in
        match uu___2 with
        | (success, st2) ->
...
let js_repl_eval_js (st : FStarC_Interactive_Ide_Types.repl_state)
  ...
  (FStarC_Json.json Prims.list * (FStarC_Interactive_Ide_Types.repl_state,
    Prims.int) FStar_Pervasives.either)=
  let uu___ = deserialize_interactive_query query_js in js_repl_eval st uu___
let js_repl_eval_str (st : FStarC_Interactive_Ide_Types.repl_state)
  ...
  (Prims.string Prims.list * (FStarC_Interactive_Ide_Types.repl_state,
    Prims.int) FStar_Pervasives.either)=
  let uu___ =
    let uu___1 = parse_interactive_query query_str in js_repl_eval st uu___1 in
  match uu___ with
  | (js_response, st_opt) ->
...
let build_initial_repl_state (filename : Prims.string) :
  ...
  (let uu___1 = FStarC_Util.open_stdin () in
   {
