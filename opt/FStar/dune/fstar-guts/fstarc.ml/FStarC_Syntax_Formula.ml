...
let uu___is_QAll (projectee : connective) : Prims.bool=
  match projectee with | QAll _0 -> true | uu___ -> false
let __proj__QAll__item___0 (projectee : connective) :
  (FStarC_Syntax_Syntax.binders * qpats * FStarC_Syntax_Syntax.typ)=
  match projectee with | QAll _0 -> _0
let uu___is_QEx (projectee : connective) : Prims.bool=
  match projectee with | QEx _0 -> true | uu___ -> false
let __proj__QEx__item___0 (projectee : connective) :
  (FStarC_Syntax_Syntax.binders * qpats * FStarC_Syntax_Syntax.typ)=
  match projectee with | QEx _0 -> _0
let uu___is_BaseConn (projectee : connective) : Prims.bool=
  match projectee with | BaseConn _0 -> true | uu___ -> false
let __proj__BaseConn__item___0 (projectee : connective) :
  (FStarC_Ident.lident * FStarC_Syntax_Syntax.args)=
  match projectee with | BaseConn _0 -> _0
let connective_to_string (c : connective) : Prims.string=
  match c with
  | QAll p ->
      let uu___ =
...
let lookup_arity_lid
  (table :
    (Prims.int * (FStarC_Ident.lident * FStarC_Ident.lident) Prims.list)
      Prims.list)
  (target_lid : FStarC_Ident.lident) (args : FStarC_Syntax_Syntax.args) :
...
let destruct_base_conn (t : FStarC_Syntax_Syntax.term) :
  connective FStar_Pervasives_Native.option=
...
let destruct_sq_base_conn (uu___ : FStarC_Syntax_Syntax.term) :
  connective FStar_Pervasives_Native.option=
  (fun t ->
...
let patterns (t : FStarC_Syntax_Syntax.term) :
  ...
  | uu___ -> ([], t1)
let destruct_q_conn (t : FStarC_Syntax_Syntax.term) :
  connective FStar_Pervasives_Native.option=
  let is_q fa fv =
    if fa
        ...
        (match uu___2 with
         | (bs1, t2) ->
             ...
             (match uu___3 with
              | (pats, body) ->
                  if b
                  then FStar_Pervasives_Native.Some (QAll (bs1, pats, body))
                  else FStar_Pervasives_Native.Some (QEx (bs1, pats, body))))
    | uu___1 -> FStar_Pervasives_Native.None in
  aux FStar_Pervasives_Native.None [] t
let rec destruct_sq_forall (uu___ : FStarC_Syntax_Syntax.term) :
  connective FStar_Pervasives_Native.option=
  (fun t ->
     let uu___ = FStarC_Syntax_Util.un_squash t in
     Obj.magic
                                    ...
                                    | (pats, q2) ->
                                        Obj.magic
                                          (maybe_collect
                                             (FStar_Pervasives_Native.Some
                                                (QEx ([b1], pats, q2))))))
                          | uu___6 -> Obj.magic FStar_Pervasives_Native.None)
                     | uu___3 -> Obj.magic FStar_Pervasives_Native.None))
               uu___1))) uu___
and maybe_collect (f : connective FStar_Pervasives_Native.option) :
  connective FStar_Pervasives_Native.option=
  match f with
  | FStar_Pervasives_Native.Some (QAll (bs, pats, phi)) ->
      ...
      (match uu___ with
       | FStar_Pervasives_Native.Some (QAll (bs', pats', psi)) ->
           FStar_Pervasives_Native.Some
             (QAll
                ((FStarC_List.op_At bs bs'), (FStarC_List.op_At pats pats'),
                  psi))
       | uu___1 -> f)
  | FStar_Pervasives_Native.Some (QEx (bs, pats, phi)) ->
      ...
      (match uu___ with
       ...
       | uu___1 -> f)
  | uu___ -> f
let destruct_typ_as_formula (f : FStarC_Syntax_Syntax.term) :
  connective FStar_Pervasives_Native.option=
