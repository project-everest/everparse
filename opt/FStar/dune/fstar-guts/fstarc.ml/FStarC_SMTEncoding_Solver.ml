  ...
  (let uu___2 = FStarC_Effect.op_Bang dbg_SMTFail in
   ...
   then
     let uu___3 =
       ...
       let uu___5 =
         let uu___6 =
           FStarC_Class_PP.pp FStarC_Syntax_Print.pretty_term qs.query_term in
         [uu___6] in
       uu___4 :: uu___5 in
...
let encode_and_ask (can_split : Prims.bool) (is_retry : Prims.bool)
  ...
  let uu___ = FStarC_SMTEncoding_Solver_Cache.try_find_query_cache tcenv q in
  if uu___
  then
    ([],
      {
  ...
  let ans_opt =
    try
      (fun uu___ ->
         ...
         | () ->
             let uu___1 =
               encode_and_ask can_split is_retry use_env_msg tcenv q in
    ...
    | FStarC_SMTEncoding_Env.Inner_let_rec names ->
        ((let uu___2 =
            let uu___3 =
              let uu___4 =
                let uu___5 =
                  let uu___6 =
                    let uu___7 =
                      FStarC_List.map FStar_Pervasives_Native.fst names in
         ...
         FStar_Pervasives_Native.None) in
  match ans_opt with
  | FStar_Pervasives_Native.Some (default_settings::uu___, ans) when
      Prims.op_Negation ans.ok -> report tcenv default_settings ans
  | FStar_Pervasives_Native.Some (uu___, ans) when ans.ok -> ()
  | FStar_Pervasives_Native.Some ([], ans) when Prims.op_Negation ans.ok ->
...
let split_and_solve (retrying : Prims.bool)
  ...
  (tcenv : FStarC_TypeChecker_Env.env) (q : FStarC_Syntax_Syntax.term) :
  unit=
  (let uu___1 = (FStarC_Debug.any ()) || (FStarC_Options.query_stats ()) in
   if uu___1
   then
     let range =
       let uu___2 =
         ...
         Prims.strcat uu___3 ")" in
       Prims.strcat "(" uu___2 in
     FStarC_Format.print2 "%s\tQuery-stats splitting query because %s\n"
       range
       (if retrying
...
let do_solve_maybe_split
  ...
  then ()
    ...
    (let uu___1 = FStarC_Options.split_queries () in
     ...
     | FStarC_Options.Always -> split_and_solve false use_env_msg tcenv q)
let solve
  (use_env_msg : (unit -> Prims.string) FStar_Pervasives_Native.option)
  (tcenv : FStarC_TypeChecker_Env.env) (q : FStarC_Syntax_Syntax.term) :
  unit=
