...
exception Inner_let_rec of (Prims.string * FStarC_Range_Type.t) Prims.list 
let uu___is_Inner_let_rec (projectee : Prims.exn) : Prims.bool=
  match projectee with | Inner_let_rec uu___ -> true | uu___ -> false
let __proj__Inner_let_rec__item__uu___ (projectee : Prims.exn) :
  (Prims.string * FStarC_Range_Type.t) Prims.list=
  match projectee with | Inner_let_rec uu___ -> uu___
...
let __proj__Mkenv_t__item__global_cache (projectee : env_t) :
  FStarC_SMTEncoding_Term.decls_elt FStarC_SMap.t=
  match projectee with
  | { bvar_bindings; fvar_bindings; depth; tcenv; warn; nolabels;
      use_zfuel_name; encode_non_total_function_typ; current_module_name;
...
let mk_fvb (lid : FStarC_Ident.lident) (fname : Prims.string)
  ...
  (fuel_partial_app :
    (FStarC_SMTEncoding_Term.term * FStarC_SMTEncoding_Term.term)
      FStar_Pervasives_Native.option)
  (thunked : Prims.bool) : fvar_binding=
  let fvb =
    {
      fvar_lid = lid;
      smt_arity = arity;
...
let new_term_constant_and_tok_from_lid_aux (env : env_t)
  (x : FStarC_Ident.lident) (arity : Prims.int) (thunked : Prims.bool) :
  ...
  | (ftok_name, ftok) ->
      let fvb =
        mk_fvb x fname arity ftok FStar_Pervasives_Native.None thunked in
      let uu___1 =
...
let new_term_constant_and_tok_from_lid (env : env_t)
  (x : FStarC_Ident.lident) (arity : Prims.int) :
...
let new_term_constant_and_tok_from_lid_maybe_thunked (env : env_t)
  (x : FStarC_Ident.lident) (arity : Prims.int) (th : Prims.bool) :
  (Prims.string * Prims.string FStar_Pervasives_Native.option * env_t)=
...
let push_free_var_maybe_thunked (env : env_t) (x : FStarC_Ident.lident)
  (arity : Prims.int) (fname : Prims.string)
  (ftok : FStarC_SMTEncoding_Term.term FStar_Pervasives_Native.option)
  (thunked : Prims.bool) : env_t=
  let fvb = mk_fvb x fname arity ftok FStar_Pervasives_Native.None thunked in
...
let push_free_var (env : env_t) (x : FStarC_Ident.lident) (arity : Prims.int)
  (fname : Prims.string)
  (ftok : FStarC_SMTEncoding_Term.term FStar_Pervasives_Native.option) :
  env_t= push_free_var_maybe_thunked env x arity fname ftok false
let push_free_var_thunk (env : env_t) (x : FStarC_Ident.lident)
  (arity : Prims.int) (fname : Prims.string)
  (ftok : FStarC_SMTEncoding_Term.term FStar_Pervasives_Native.option) :
  env_t=
  push_free_var_maybe_thunked env x arity fname ftok (arity = Prims.int_zero)
