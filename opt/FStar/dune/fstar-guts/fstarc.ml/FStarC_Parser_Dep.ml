...
let __proj__Mkdeps__item__interfaces_with_inlining (projectee : deps) :
  ...
  match projectee with
  | { dep_graph; file_system_map; cmd_line_files; all_files;
      interfaces_with_inlining; parse_results;_} -> interfaces_with_inlining
...
let mk_deps (dg : dependence_graph) (fs : files_for_module_name)
  (c : file_name Prims.list) (a : file_name Prims.list)
  (i : module_name Prims.list) (pr : parsing_data FStarC_SMap.t) : deps=
  {
    dep_graph = dg;
    file_system_map = fs;
    cmd_line_files = c;
...
let build_map (filenames : Prims.string Prims.list) : files_for_module_name=
                   ...
                   let uu___7 =
                     ...
                     let uu___9 =
                       let uu___10 =
                         ...
                         let uu___12 =
                           let uu___13 =
                             let uu___14 = FStarC_Errors_Msg.text "in file" in
                             [uu___14;
                 ...
                 let uu___7 =
                   let uu___8 =
                     ...
                     let uu___10 =
                       ...
                       FStar_Pprint.op_Hat_Slash_Hat
                         (FStar_Pprint.dquotes
                            (FStar_Pprint.doc_of_string str)) uu___11 in
...
let peek_past_interfaces (uu___ : unit) : Prims.bool=
  let uu___1 = FStarC_Options_Ext.enabled "dep_minimal" in
  if uu___1
  then false
...
let collect_one (original_map : files_for_module_name)
  ...
  let from_parsing_data pd original_map1 filename1 =
    ...
    let mo_roots =
      let uu___ =
        ((is_interface filename1) && (has_implementation original_map1 mname))
    ...
    let add_dep deps2 d =
      ...
      if uu___
      then
        let uu___1 = let uu___2 = FStarC_Effect.op_Bang deps2 in d :: uu___2 in
    ...
    let record_open_namespace lid implicit_open =
      let key = lowercase_join_longident lid true in
      let r = enter_namespace original_map1 working_map key implicit_open in
    ...
    let add_dep_on_module module_name1 is_friend =
      let uu___ = add_dependence_edge working_map module_name1 is_friend in
      if uu___
      then ()
      else
           ...
           let uu___3 =
             let uu___4 =
               FStarC_Class_Show.show FStarC_Ident.showable_lident
                 module_name1 in
             FStarC_Format.fmt1 "Unbound module reference %s" uu___4 in
  ...
  then
    ...
    (let num_of_toplevelmods = FStarC_Effect.mk_ref Prims.int_zero in
     ...
     let add_to_parsing_data elt =
       let uu___1 =
         let uu___2 =
           let uu___3 = let uu___4 = FStarC_Effect.op_Bang pd in uu___4.elts in
           FStarC_List.existsML (fun e -> parsing_data_elt_eq e elt) uu___3 in
     ...
     and collect_tycon uu___1 =
       ...
       | FStarC_Parser_AST.TyconVariant (uu___2, binders, k, identterms) ->
           (collect_binders binders;
            FStarC_Option.iter collect_term k;
            (let uu___5 =
               FStarC_List.filter_map
     ...
     and collect_binder b =
       ...
       (match b with
        | { FStarC_Parser_AST.b = FStarC_Parser_AST.Annotated (uu___3, t);
     ...
     and collect_term' uu___1 =
       ...
       | FStarC_Parser_AST.Construct (lid, termimps) ->
           (add_to_parsing_data (P_lid lid);
            FStarC_List.iter
              (fun uu___3 ->
                 match uu___3 with | (t, uu___4) -> collect_term t) termimps)
       ...
       | FStarC_Parser_AST.Labeled (t, uu___2, uu___3) -> collect_term t
       | FStarC_Parser_AST.LexList l -> FStarC_List.iter collect_term l
       | FStarC_Parser_AST.WFOrder (t1, t2) ->
           ((let uu___3 =
               let uu___4 =
                 let uu___5 = FStarC_Ident.lid_of_str "FStar.WellFounded" in
                 (false, uu___5) in
               P_dep uu___4 in
             add_to_parsing_data uu___3);
            collect_term t1;
       ...
       | FStarC_Parser_AST.IntroForall (bs, p, e) ->
           ((let uu___3 =
               let uu___4 =
                 let uu___5 = FStarC_Ident.lid_of_str "FStar.Classical.Sugar" in
                 (false, uu___5) in
               P_dep uu___4 in
             add_to_parsing_data uu___3);
            collect_binders bs;
     ...
     and collect_pattern' uu___1 =
       ...
       | FStarC_Parser_AST.PatRecord lidpats ->
           FStarC_List.iter
             (fun uu___2 ->
                match uu___2 with | (uu___3, p) -> collect_pattern p) lidpats
...
let topological_dependences_of' (file_system_map : files_for_module_name)
  ...
  | (friends1, all_files_0) ->
      ((let uu___2 = FStarC_Effect.op_Bang dbg in
        ...
        then
          let uu___3 =
            let uu___4 = FStarC_Util.remove_dups (fun x y -> x = y) friends1 in
       ...
       (let uu___2 = widen_deps friends1 dep_graph file_system_map widened in
        match uu___2 with
        | (widened1, dep_graph1) ->
            let uu___3 =
            ...
            (match uu___3 with
             | (uu___4, all_files) ->
                 ((let uu___6 = FStarC_Effect.op_Bang dbg in
                   if uu___6
                   then
                     FStarC_Format.print1 "Phase2 complete: all_files = %s\n"
...
let collect (all_cmd_line_files : file_name Prims.list)
  ...
  FStarC_Stats.record "Parser.Dep.collect"
    (fun uu___ ->
           ...
           | (parsing_data1, (deps1, mo_roots, needs_interface_inlining)) ->
               (debug_print
                ...
                (let deps2 =
                 ...
                 let dep_node1 =
                   let uu___6 = FStarC_List.unique deps2 in
                 ...
                 (let uu___7 =
                  ...
                  FStarC_List.iter discover_one uu___7)))
         else () in
       profile
       ...
       debug_print
         (fun uu___3 ->
            print_graph FStarC_Util.stdout "stdout" dep_graph file_system_map
              all_cmd_line_files2);
...
let deps_of (deps1 : deps) (f : file_name) : Prims.string Prims.list=
  dependences_of deps1.file_system_map deps1.dep_graph deps1.cmd_line_files f
    ...
    FStarC_Option.map
      (fun uu___2 ->
         ...
         | (intf_opt, impl_opt) ->
             ...
             FStarC_Util.remove_dups (fun x y -> x = y) uu___3) uu___1 in
               ...
               | (iface_fn, iface_deps) ->
                   ...
                   let files1 =
                     ...
                     | FStar_Pervasives_Native.Some iface_deps2 ->
                         ...
                         FStarC_Util.remove_dups (fun x y -> x = y)
