...
let pop_repl (msg : Prims.string)
  ...
  | (depth, (uu___1, st'))::stack_tl ->
      ...
      (FStarC_Effect.op_Colon_Equals repl_stack stack_tl;
       ...
       st')
let tc_one (env : FStarC_TypeChecker_Env.env_t)
  (intf_opt : Prims.string FStar_Pervasives_Native.option)
  (modf : Prims.string) : FStarC_TypeChecker_Env.env_t=
  let parse_data =
...
let run_repl_task (curmod : FStarC_Interactive_Ide_Types.optmod_t)
       ...
       | (events, new_dshooks, new_tchooks) ->
           ...
           (uu___2,
             ((fun env1 ->
                 ...
                 (uu___3, uu___4)))))
let repl_tx (st : FStarC_Interactive_Ide_Types.repl_state)
  ...
  try
    (fun uu___ ->
       ...
       | () ->
           let st1 =
             push_repl "repl_tx" (FStar_Pervasives_Native.Some push_kind)
               task st in
           let uu___1 =
             track_name_changes st1.FStarC_Interactive_Ide_Types.repl_env in
           (match uu___1 with
            | (env, finish_name_tracking) ->
                let uu___2 =
                  run_repl_task st1.FStarC_Interactive_Ide_Types.repl_curmod
                    env task st1.FStarC_Interactive_Ide_Types.repl_lang in
                (match uu___2 with
                 | (curmod, env1, lds) ->
                     let st2 =
                       {
                     ...
                     (match uu___3 with
                      | (env2, name_events) ->
                          let uu___4 = commit_name_tracking st2 name_events in
...
let update_task_timestamps (uu___ : FStarC_Interactive_Ide_Types.repl_task) :
  ...
  | other -> other
let repl_ldtx (st : FStarC_Interactive_Ide_Types.repl_state)
  ...
  let rec aux st1 tasks1 previous =
    ...
    | (task::tasks2, []) ->
        ...
        let uu___ =
          repl_tx st1 FStarC_Interactive_Ide_Types.LaxCheck timestamped_task in
        (match uu___ with
         | (diag, st2) ->
...
let ld_deps (st : FStarC_Interactive_Ide_Types.repl_state) :
...
let full_lax (text : Prims.string)
  (st : FStarC_Interactive_Ide_Types.repl_state) :
  (FStarC_Interactive_JsonHelper.assoct FStar_Pervasives_Native.option *
    FStarC_Interactive_Ide_Types.repl_state)=
  ...
  (let frag =
   ...
   | FStar_Pervasives.Inl (st1, deps) ->
       let names =
         add_module_completions st1.FStarC_Interactive_Ide_Types.repl_fname
           deps st1.FStarC_Interactive_Ide_Types.repl_names in
       repl_tx
         {
           FStarC_Interactive_Ide_Types.repl_line =
             (st1.FStarC_Interactive_Ide_Types.repl_line);
