    ...
    (fun uu___ ->
       ...
       let maybe_add_iface_hash out =
         ...
         | FStar_Pervasives_Native.Some iface ->
             let uu___1 = FStarC_SMap.try_find mcache iface in
             (match uu___1 with
              | FStar_Pervasives_Native.None ->
                  let msg =
                    FStarC_Format.fmt1
                  ...
                  ((let uu___3 = FStarC_Effect.op_Bang dbg in
                    if uu___3 then FStarC_Format.print1 "%s\n" msg else ());
       ...
       let rec hash_deps out uu___1 =
         ...
         | fn2::deps1 ->
             ...
             let digest =
               let uu___2 = FStarC_SMap.try_find mcache cache_fn in
               match uu___2 with
               | FStar_Pervasives_Native.None ->
                   let msg =
                     FStarC_Format.fmt2
                   ...
                   ((let uu___4 = FStarC_Effect.op_Bang dbg in
                     if uu___4 then FStarC_Format.print1 "%s\n" msg else ());
                    FStar_Pervasives.Inl msg)
...
let load_checked_file (fn : Prims.string) (checked_fn : Prims.string) :
  ...
  (let elt = FStarC_SMap.try_find mcache checked_fn in
   if FStar_Pervasives_Native.uu___is_Some elt
   then
     ((let uu___2 = FStarC_Effect.op_Bang dbg in
       if uu___2
       then
         FStarC_Format.print1 "Already loaded checked file %s\n" checked_fn
       else ());
      FStarC_Option.must elt)
   else
     (let add_and_return elt1 = FStarC_SMap.add mcache checked_fn elt1; elt1 in
   ...
   | (Unknown, parsing_data) ->
       ...
       (match uu___1 with
        | FStar_Pervasives.Inl msg ->
            ...
            (FStarC_SMap.add mcache checked_fn elt1; FStar_Pervasives.Inl msg)
            ...
            (match uu___2 with
             | (deps_dig, tc_result1) ->
                 ...
                 then
                   ...
                   (FStarC_SMap.add mcache checked_fn elt1;
                                 ...
                                 | () ->
                                     ...
                                     (match uu___6 with
                                      | FStar_Pervasives_Native.Some
                                          ...
                                          FStarC_SMap.add mcache
                                            iface_checked_fn uu___7
                                      | uu___7 -> ())) ()
                            with | uu___5 -> ()) in
             ...
             then
               (FStarC_Effect.op_Colon_Equals already_failed true;
                (let uu___3 =
                   ...
                   let uu___5 =
                     FStarC_Range_Type.mk_pos Prims.int_zero Prims.int_zero in
                   FStarC_Range_Type.mk_range fn1 uu___4 uu___5 in
                 let uu___4 =
                   let uu___5 =
                     let uu___6 =
                       FStarC_Format.fmt3
                 ...
                 FStarC_Errors.log_issue FStarC_Class_HasRange.hasRange_range
                   uu___3 FStarC_Errors_Codes.Warning_CachedFile ()
...
let store_module_to_cache (env : FStarC_TypeChecker_Env.env)
  (fn : Prims.string) (parsing_data : FStarC_Parser_Dep.parsing_data)
  (tc_result1 : tc_result) : unit=
  let uu___ =
    (FStarC_Options.cache_checked_modules ()) &&
      (let uu___1 = FStarC_Options.cache_off () in Prims.op_Negation uu___1) in
  if uu___
  then
    let cache_file =
      let uu___1 = FStarC_Options.output_to () in
      match uu___1 with
      | FStar_Pervasives_Native.Some fn1 -> fn1
      | FStar_Pervasives_Native.None -> FStarC_Parser_Dep.cache_file_name fn in
    let digest =
      let uu___1 = FStarC_TypeChecker_Env.dep_graph env in
    ...
    | FStar_Pervasives.Inr hashes ->
        ...
        let stage1 =
          let uu___1 = FStarC_Util.digest_of_file fn in
          { version = cache_version_number; digest = uu___1; parsing_data } in
        let stage2 = { deps_dig = hashes; tc_res = tc_result2 } in
        store_values_to_cache cache_file stage1 stage2
    | FStar_Pervasives.Inl msg ->
        let uu___1 =
          let uu___2 = FStarC_Range_Type.mk_pos Prims.int_zero Prims.int_zero in
          let uu___3 = FStarC_Range_Type.mk_pos Prims.int_zero Prims.int_zero in
          FStarC_Range_Type.mk_range fn uu___2 uu___3 in
        let uu___2 =
          let uu___3 =
            let uu___4 =
          ...
          let uu___4 =
            ...
            [uu___5] in
          uu___3 :: uu___4 in
        FStarC_Errors.log_issue FStarC_Class_HasRange.hasRange_range uu___1
...
let unsafe_raw_load_checked_file (checked_fn : Prims.string) :
  ...
  | FStar_Pervasives_Native.Some (s1, s2) ->
      let uu___ =
        let uu___1 = FStarC_List.map FStar_Pervasives_Native.fst s2.deps_dig in
