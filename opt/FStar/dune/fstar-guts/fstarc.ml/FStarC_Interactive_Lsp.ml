open Prims
let unpack_lsp_query (r : (Prims.string * FStarC_Json.json) Prims.list) :
       ...
       | () ->
           ...
           let uu___1 =
             ...
             | "workspace/executeCommand" ->
                 let uu___2 =
                   let uu___3 = FStarC_Interactive_JsonHelper.arg "command" r in
                   FStarC_Interactive_JsonHelper.js_str uu___3 in
             ...
             | "textDocument/didSave" ->
                 let uu___2 =
                   let uu___3 = FStarC_Interactive_JsonHelper.js_txdoc_id r in
                   let uu___4 =
                     let uu___5 = FStarC_Interactive_JsonHelper.arg "text" r in
                     FStarC_Interactive_JsonHelper.js_str uu___5 in
                   (uu___3, uu___4) in
             ...
             | "textDocument/completion" ->
                 let uu___2 =
                   let uu___3 = FStarC_Interactive_JsonHelper.js_txdoc_pos r in
                   let uu___4 =
                     let uu___5 =
             ...
             | "textDocument/rename" -> FStarC_Interactive_JsonHelper.Rename
...
let deserialize_lsp_query (js_query : FStarC_Json.json) :
  FStarC_Interactive_JsonHelper.lsp_query=
  try
    (fun uu___ ->
       match () with
  ...
  with
  | FStarC_Interactive_JsonHelper.UnexpectedJsonType (expected, got) ->
      FStarC_Interactive_JsonHelper.wrap_jsfail FStar_Pervasives_Native.None
        expected got
let parse_lsp_query (query_str : Prims.string) :
  FStarC_Interactive_JsonHelper.lsp_query=
  let uu___1 = FStarC_Json.json_of_string query_str in
  match uu___1 with
  | FStar_Pervasives_Native.None ->
      {
        FStarC_Interactive_JsonHelper.query_id = FStar_Pervasives_Native.None;
        FStarC_Interactive_JsonHelper.q =
          (FStarC_Interactive_JsonHelper.BadProtocolMsg "Json parsing failed")
      }
  | FStar_Pervasives_Native.Some request -> deserialize_lsp_query request
let repl_state_init (fname : Prims.string) :
  ...
  let intial_range =
    ...
    FStarC_Range_Type.mk_range fname uu___ uu___1 in
  ...
  let uu___ = FStarC_Util.open_stdin () in
  {
...
let invoke_full_lax (gst : FStarC_Interactive_Ide_Types.grepl_state)
  (fname : Prims.string) (text : Prims.string) (force : Prims.bool) :
  ...
  let aux uu___ =
    ...
    (let uu___2 =
     ...
     | (diag, st') ->
         let repls =
           FStarC_PSMap.add gst.FStarC_Interactive_Ide_Types.grepl_repls
             fname st' in
         let diag1 =
           if FStar_Pervasives_Native.uu___is_Some diag
           then diag
           else
             (let uu___4 = FStarC_Interactive_JsonHelper.js_diag_clear fname in
  ...
  let uu___ =
    FStarC_PSMap.try_find gst.FStarC_Interactive_Ide_Types.grepl_repls fname in
  match uu___ with
...
let run_query (gst : FStarC_Interactive_Ide_Types.grepl_state)
  (q : FStarC_Interactive_JsonHelper.lquery) :
  (optresponse * either_gst_exit)=
  match q with
  ...
  | FStarC_Interactive_JsonHelper.Exit ->
      (FStar_Pervasives_Native.None, (FStar_Pervasives.Inr Prims.int_zero))
  ...
  | FStarC_Interactive_JsonHelper.Completion (txpos, ctx) ->
      let uu___ =
        FStarC_PSMap.try_find gst.FStarC_Interactive_Ide_Types.grepl_repls
          txpos.FStarC_Interactive_JsonHelper.path in
      (match uu___ with
       | FStar_Pervasives_Native.Some st ->
           let uu___1 = FStarC_Interactive_QueryHelper.complookup st txpos in
...
let rec parse_header_len (stream : FStarC_Util.stream_reader)
  (len : Prims.int) : Prims.int=
  let uu___ = FStarC_Util.read_line stream in
  match uu___ with
  | FStar_Pervasives_Native.Some s ->
      if FStarC_Util.starts_with s "Content-Length: "
      then
        let uu___1 =
          let uu___2 = FStarC_Util.substring_from s (Prims.of_int (16)) in
        ...
        if FStarC_Util.starts_with s "Content-Type: "
        then parse_header_len stream len
        else
          if s = ""
          then len
          else
            FStarC_Effect.raise FStarC_Interactive_JsonHelper.MalformedHeader
  | FStar_Pervasives_Native.None ->
      FStarC_Effect.raise FStarC_Interactive_JsonHelper.InputExhausted
let rec read_lsp_query (stream : FStarC_Util.stream_reader) :
  FStarC_Interactive_JsonHelper.lsp_query=
  try
    (fun uu___ ->
       match () with
       | () ->
           ...
           let uu___1 = FStarC_Util.nread stream n in
           (match uu___1 with
...
let rec go (gst : FStarC_Interactive_Ide_Types.grepl_state) : Prims.int=
...
let start_server (uu___ : unit) : unit=
  let uu___1 =
    let uu___2 =
