open Fstarcompiler
open Prims
let tcenv (uu___ : unit) : FStarC_TypeChecker_Env.env=
  FStarC_Tests_Pars.init ()
let guard_to_string (g : FStarC_TypeChecker_Common.guard_formula) :
  Prims.string=
  match g with
...
let fail (msg : Prims.string) : unit=
  FStarC_Format.print_string msg; FStarC_Effect.op_Colon_Equals success false
let guard_eq (i : Prims.int) (g : FStarC_TypeChecker_Common.guard_formula)
  (g' : FStarC_TypeChecker_Common.guard_formula) : unit=
  let uu___ =
    match (g, g') with
    | (FStarC_TypeChecker_Common.Trivial, FStarC_TypeChecker_Common.Trivial)
        -> (true, g, g')
    | (FStarC_TypeChecker_Common.NonTrivial f,
       FStarC_TypeChecker_Common.NonTrivial f') ->
        let f1 =
...
let unify (i : Prims.int) (bvs : FStarC_Syntax_Syntax.bv Prims.list)
  (x : FStarC_Syntax_Syntax.typ) (y : FStarC_Syntax_Syntax.typ)
  (g' : FStarC_TypeChecker_Common.guard_formula) (check : unit -> unit) :
  unit=
  ...
  (let uu___3 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term x in
   let uu___4 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term y in
  ...
  (let tcenv1 = tcenv () in
   ...
   let g =
     let uu___3 =
       ...
       FStarC_TypeChecker_Rel.solve_deferred_constraints tcenv2 uu___4 in
     FStarC_TypeChecker_Rel.simplify_guard tcenv2 uu___3 in
   guard_eq i g.FStarC_TypeChecker_Common.guard_f g';
   check ();
   FStarC_Options.init ())
let should_fail (x : FStarC_Syntax_Syntax.typ) (y : FStarC_Syntax_Syntax.typ)
  : unit=
  try
    (fun uu___ ->
       match () with
       | () ->
           let g =
             let uu___1 = tcenv () in
             let uu___2 =
               let uu___3 = tcenv () in FStarC_TypeChecker_Rel.teq uu___3 x y in
             FStarC_TypeChecker_Rel.solve_deferred_constraints uu___1 uu___2 in
            ...
            | FStarC_TypeChecker_Common.Trivial ->
                let uu___1 =
                  ...
                  let uu___3 =
                    FStarC_Class_Show.show FStarC_Syntax_Print.showable_term
                      y in
                  FStarC_Format.fmt2 "%s and %s should not be unifiable\n"
                    uu___2 uu___3 in
                fail uu___1
            | FStarC_TypeChecker_Common.NonTrivial f ->
                let uu___1 =
...
let unify' (x : Prims.string) (y : Prims.string) : unit=
  ...
  let uu___1 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term y1 in
  let uu___2 = guard_to_string g.FStarC_TypeChecker_Common.guard_f in
  FStarC_Format.print3 "%s and %s are unifiable with guard %s\n" uu___ uu___1
    uu___2
let norm (t : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  let uu___ = tcenv () in FStarC_TypeChecker_Normalize.normalize [] uu___ t
let check_core (i : Prims.int) (subtyping : Prims.bool)
  (guard_ok : Prims.bool) (x : FStarC_Syntax_Syntax.typ)
  (y : FStarC_Syntax_Syntax.typ) : unit=
  (let uu___1 = FStarC_Options.parse_cmd_line () in ());
  (let env = tcenv () in
   let res =
     if subtyping
     then FStarC_TypeChecker_Core.check_term_subtyping true true env x y
     else FStarC_TypeChecker_Core.check_term_equality true true env x y in
   (match res with
    ...
    | Fstarcompiler.FStar_Pervasives.Inl (FStar_Pervasives_Native.Some g) ->
        ((let uu___3 =
            FStarC_Class_Show.show FStarC_Class_Show.showable_int i in
          let uu___4 =
            FStarC_Class_Show.show FStarC_Syntax_Print.showable_term g in
          FStarC_Format.print2 "%s core check computed guard %s ok\n" uu___3
            uu___4);
   ...
   FStarC_Options.init ())
let check_core_typing (i : Prims.int) (e : FStarC_Syntax_Syntax.term)
  ...
  (let env = tcenv () in
   (let uu___2 = FStarC_TypeChecker_Core.check_term env e t true in
    match uu___2 with
    | Fstarcompiler.FStar_Pervasives.Inl (FStar_Pervasives_Native.None) ->
        let uu___3 = FStarC_Class_Show.show FStarC_Class_Show.showable_int i in
    ...
    then out
      ...
      (let uu___1 =
         ...
         FStarC_TypeChecker_Util.new_implicit_var ""
           FStarC_Range_Type.dummyRange uu___2 FStarC_Syntax_Util.ktype0
           false in
       match uu___1 with
       | (t, uu___2, uu___3) ->
           let uu___4 =
   ...
   (let id = FStarC_Tests_Pars.tc "fun (x:bool) -> x" in
    ...
    (let id1 = FStarC_Tests_Pars.tc "fun (x:bool) -> x" in
     ...
     (let id2 = FStarC_Tests_Pars.tc "fun (x:bool) -> x" in
      ...
      unify1 (Prims.of_int (4)) [] id2 id' FStarC_TypeChecker_Common.Trivial;
      ...
      (let uu___11 = FStarC_Tests_Pars.tc "fun (x:int) (y:int) (z:int) -> y" in
       let uu___12 = FStarC_Tests_Pars.tc "fun (x:int) (y:int) (z:int) -> z" in
       let uu___13 =
         let uu___14 =
                   ...
                   (let tm12 =
                    ...
                    (let uu___18 =
                       ...
                       let tm13 =
                         let uu___19 =
                           let uu___20 =
                             let uu___21 = FStarC_Syntax_Syntax.bv_to_name q in
                             [uu___21] in
                       ...
                       let env =
                         ...
                         let uu___20 =
                           ...
                           let uu___22 =
                             ...
                             [uu___23] in
                           uu___21 :: uu___22 in
                         FStarC_TypeChecker_Env.push_binders uu___19 uu___20 in
                       let uu___19 =
                         FStarC_TypeChecker_Util.new_implicit_var ""
                         ...
                         (unify1 (Prims.of_int (13)) bvs_13 tm13 tm22
                          ...
                          (let uu___20 =
                             ...
                             let x1 =
                               FStarC_Syntax_Syntax.new_bv
                                 FStar_Pervasives_Native.None int_typ in
                             let typ = FStarC_Tests_Pars.tc "pure_post unit" in
                             let l =
                               FStarC_Tests_Pars.tc
