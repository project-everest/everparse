...
let test_lid : FStarC_Ident.lident=
  FStarC_Ident.lid_of_path ["Test"] FStarC_Range_Type.dummyRange
let tcenv_ref :
...
let test_mod_ref :
  FStarC_Syntax_Syntax.modul FStar_Pervasives_Native.option FStarC_Effect.ref=
...
let parse_mod (mod_name : Prims.string) (dsenv : FStarC_Syntax_DsEnv.env) :
  ...
  | FStarC_Parser_ParseIt.ASTFragment
      (Fstarcompiler.FStar_Pervasives.Inl m, uu___1) ->
      let uu___2 =
      ...
      (match uu___2 with
       | (m1, env') ->
           let uu___3 =
             let uu___4 =
               FStarC_Ident.lid_of_path ["Test"] FStarC_Range_Type.dummyRange in
             FStarC_Syntax_DsEnv.prepare_module_or_interface false false
               false env' uu___4 FStarC_Syntax_DsEnv.default_mii in
           (match uu___3 with | (env'1, uu___4) -> (env'1, m1)))
  | FStarC_Parser_ParseIt.ParseError (err, msg, r) ->
      FStarC_Effect.raise (FStarC_Errors.Error (err, msg, r, []))
  | FStarC_Parser_ParseIt.ASTFragment
      (Fstarcompiler.FStar_Pervasives.Inr uu___1, uu___2) ->
...
let add_mods (mod_names : Prims.string Prims.list)
  (dsenv : FStarC_Syntax_DsEnv.env) (env : FStarC_TypeChecker_Env.env) :
    ...
    (fun uu___ mod_name ->
       ...
       | (dsenv1, env1) ->
           ...
           (match uu___1 with
            | (dsenv2, string_mod) ->
                let uu___2 =
                  FStarC_TypeChecker_Tc.check_module env1 string_mod false in
                (match uu___2 with | (_mod, env2) -> (dsenv2, env2))))
    (dsenv, env) mod_names
let do_init (uu___ : unit) : unit=
  let solver = FStarC_SMTEncoding_Solver.dummy in
  let env =
    let uu___1 = FStarC_Parser_Dep.empty_deps [] in
    FStarC_TypeChecker_Env.initial_env uu___1
  ...
  (let prelude_file =
     let uu___2 = FStarC_Find.find_file "Prims.fst" in
     match uu___2 with
     | FStar_Pervasives_Native.Some f -> f
     | FStar_Pervasives_Native.None ->
         failwith "Could not find Prims.fst: is FSTAR_LIB set?" in
   let uu___2 =
     let uu___3 =
       let uu___4 = FStarC_Parser_Dep.empty_deps [] in
       FStarC_Syntax_DsEnv.empty_env uu___4 in
     parse_mod prelude_file uu___3 in
   match uu___2 with
   | (dsenv, prims_mod) ->
       let env1 =
         {
        ...
        | (_prims_mod, env2) ->
            ...
            let env4 =
                  ...
                  FStarC_Syntax_Syntax.Unrestricted in
              {
                ...
                FStarC_TypeChecker_Env.dsenv = uu___4;
                FStarC_TypeChecker_Env.nbe =
            ...
            let env5 =
              FStarC_TypeChecker_Env.set_current_module env4 test_lid in
            FStarC_Effect.op_Colon_Equals tcenv_ref
              (FStar_Pervasives_Native.Some env5)))
let init (uu___ : unit) : FStarC_TypeChecker_Env.env=
  let uu___1 = FStarC_Effect.op_Bang tcenv_ref in
  match uu___1 with
  | FStar_Pervasives_Native.Some f -> f
  | uu___2 ->
      failwith "Should have already been initialized by the top-level effect"
let frag_of_text (s : Prims.string) : FStarC_Parser_ParseIt.input_frag=
  {
...
let pars (s : Prims.string) : FStarC_Syntax_Syntax.term=
  try
    (fun uu___ ->
       match () with
       | () ->
...
let tc' (s : Prims.string) :
  (FStarC_Syntax_Syntax.term * FStarC_TypeChecker_Env.env)=
  let tm = pars s in
  let tcenv = init () in
  let tcenv1 =
    {
      ...
      FStarC_TypeChecker_Env.modules = (tcenv.FStarC_TypeChecker_Env.modules);
      FStarC_TypeChecker_Env.expected_typ =
        (tcenv.FStarC_TypeChecker_Env.expected_typ);
      FStarC_TypeChecker_Env.sigtab = (tcenv.FStarC_TypeChecker_Env.sigtab);
      FStarC_TypeChecker_Env.attrtab = (tcenv.FStarC_TypeChecker_Env.attrtab);
      FStarC_TypeChecker_Env.instantiate_imp =
        (tcenv.FStarC_TypeChecker_Env.instantiate_imp);
      FStarC_TypeChecker_Env.effects = (tcenv.FStarC_TypeChecker_Env.effects);
      FStarC_TypeChecker_Env.generalize =
        (tcenv.FStarC_TypeChecker_Env.generalize);
      FStarC_TypeChecker_Env.letrecs = (tcenv.FStarC_TypeChecker_Env.letrecs);
      ...
      FStarC_TypeChecker_Env.tc_term = (tcenv.FStarC_TypeChecker_Env.tc_term);
      FStarC_TypeChecker_Env.typeof_tot_or_gtot_term =
  ...
  | (tm1, uu___1, g) ->
      (FStarC_TypeChecker_Rel.force_trivial_guard tcenv1 g;
       (let tm2 = FStarC_Syntax_Compress.deep_compress false false tm1 in
        (tm2, tcenv1)))
let tc (s : Prims.string) : FStarC_Syntax_Syntax.term=
  let uu___ = tc' s in match uu___ with | (tm, uu___1) -> tm
let tc_term (tm : FStarC_Syntax_Syntax.term) : FStarC_Syntax_Syntax.term=
  let tcenv = init () in
  let tcenv1 =
    {
      FStarC_TypeChecker_Env.solver = (tcenv.FStarC_TypeChecker_Env.solver);
  ...
  | (tm1, uu___1, g) ->
      (FStarC_TypeChecker_Rel.force_trivial_guard tcenv1 g;
       (let tm2 = FStarC_Syntax_Compress.deep_compress false false tm1 in tm2))
let pars_and_tc_fragment (s : Prims.string) : unit=
  FStarC_Errors.with_ctx (Prims.strcat "pars_and_tc_fragment " s)
    (fun uu___ ->
       let tcenv = init () in
       let frag = frag_of_text s in
       let uu___1 =
         let uu___2 = FStarC_Effect.op_Bang test_mod_ref in
         FStarC_Universal.tc_one_fragment false uu___2 tcenv
           (Fstarcompiler.FStar_Pervasives.Inl (frag, [])) in
       match uu___1 with
       | (test_mod', tcenv', uu___2) ->
           (FStarC_Effect.op_Colon_Equals test_mod_ref test_mod';
            FStarC_Effect.op_Colon_Equals tcenv_ref
              (FStar_Pervasives_Native.Some tcenv')))
let test_hashes (uu___ : unit) : unit=
  ...
  (let test_one_hash n =
     let rec aux n1 =
       ...
       then "U0"
         ...
         (let uu___4 =
            let uu___5 = aux (n1 - Prims.int_one) in Prims.strcat uu___5 ")" in
     ...
     FStarC_Format.print2 "Hash of unary %s is %s\n" uu___3 uu___4 in
   let rec aux n =
     if n = Prims.int_zero
     then ()
     else (test_one_hash n; aux (n - Prims.int_one)) in
   aux (Prims.of_int (100)); FStarC_Options.init ())
let parse_incremental_decls (uu___ : unit) : unit=
  let source0 =
  ...
  let uu___1 =
    ...
    (uu___2, uu___3) in
  match uu___1 with
  | (FStarC_Parser_ParseIt.IncrementalFragment (decls0, uu___2, parse_err0),
     FStarC_Parser_ParseIt.IncrementalFragment (decls1, uu___3, parse_err1))
      ->
      let check_range r l c =
        let p = FStarC_Range_Ops.start_of_range r in
        let uu___4 =
          (let uu___5 = FStarC_Range_Ops.line_of_pos p in uu___5 = l) &&
            (let uu___5 = FStarC_Range_Ops.col_of_pos p in uu___5 = c) in
        if uu___4
        then ()
        else
          (let uu___6 =
             let uu___7 =
               FStarC_Class_Show.show FStarC_Class_Show.showable_int l in
             let uu___8 =
               FStarC_Class_Show.show FStarC_Class_Show.showable_int c in
             let uu___9 =
               let uu___10 = FStarC_Range_Ops.line_of_pos p in
               FStarC_Class_Show.show FStarC_Class_Show.showable_int uu___10 in
             let uu___10 =
               let uu___11 = FStarC_Range_Ops.col_of_pos p in
               FStarC_Class_Show.show FStarC_Class_Show.showable_int uu___11 in
             FStarC_Format.fmt4
            ...
            let uu___5 =
              FStarC_List.forall2
                (fun uu___6 uu___7 ->
                   ...
                   | ((x, uu___8), (y, uu___9)) ->
                       FStarC_Parser_AST_Diff.eq_decl x y) decls0 decls1 in
            if uu___5
            then ()
            else
              failwith
            ...
            let uu___6 =
              let uu___7 =
                FStarC_Class_Show.show FStarC_Class_Show.showable_nat
                  (FStarC_List.length decls0) in
              let uu___8 =
                FStarC_Class_Show.show FStarC_Class_Show.showable_nat
                  (FStarC_List.length decls1) in
              FStarC_Format.fmt2
  ...
  | (FStarC_Parser_ParseIt.ParseError (code, message, range), uu___2) ->
      let msg =
        let uu___3 = FStarC_Range_Ops.string_of_range range in
...
let parse_incremental_decls_use_lang (uu___ : unit) : unit=
  let source0 =
    "module Demo\nlet x = 0\n#lang-somelang\nval f : t\nlet g x = f x\n#restart-solver" in
  ...
  (let input0 =
   ...
   | FStarC_Parser_ParseIt.IncrementalFragment (decls0, uu___3, parse_err0)
       ...
       ((match parse_err0 with
        ...
        (let ds = FStarC_List.map FStar_Pervasives_Native.fst decls0 in
         match ds with
         ...
         | uu___5 ->
             let uu___6 =
               ...
               Prims.strcat "Incremental parsing failed; unexpected decls: "
                 uu___7 in
             failwith uu___6))
