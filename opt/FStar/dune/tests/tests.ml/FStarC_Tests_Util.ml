open Fstarcompiler
open Prims
let always (id : Prims.int) (b : Prims.bool) : unit=
  if b
  then ()
  else
    (let uu___1 =
       let uu___2 = FStarC_Class_Show.show FStarC_Class_Show.showable_int id in
       FStarC_Format.fmt1 "Assertion failed: test %s" uu___2 in
     FStarC_Errors.raise_error0 FStarC_Errors_Codes.Fatal_AssertionFailure ()
...
let m : FStarC_Syntax_Syntax.bv=
  FStarC_Syntax_Syntax.gen_bv "m" FStar_Pervasives_Native.None
    FStarC_Syntax_Syntax.tun
let tm (t : 'uuuuu) : 'uuuuu FStarC_Syntax_Syntax.syntax=
  FStarC_Syntax_Syntax.mk t FStarC_Range_Type.dummyRange
let nm (x1 : FStarC_Syntax_Syntax.bv) : FStarC_Syntax_Syntax.term=
  FStarC_Syntax_Syntax.bv_to_name x1
let app (x1 : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax)
  (ts : FStarC_Syntax_Syntax.term Prims.list) :
  FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax=
  let uu___ =
    let uu___1 =
      let uu___2 = FStarC_List.map FStarC_Syntax_Syntax.as_arg ts in
      { FStarC_Syntax_Syntax.hd = x1; FStarC_Syntax_Syntax.args = uu___2 } in
...
let rec term_eq'
  ...
  (t2 : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax) : Prims.bool=
  let t11 = FStarC_Syntax_Subst.compress t1 in
  let t21 = FStarC_Syntax_Subst.compress t2 in
  let binders_eq xs ys =
    ((FStarC_List.length xs) = (FStarC_List.length ys)) &&
      (FStarC_List.forall2
         (fun x1 y1 ->
            term_eq'
    ...
    ((FStarC_List.length xs) = (FStarC_List.length ys)) &&
      (FStarC_List.forall2
         (fun uu___ uu___1 ->
            ...
            | ((a, imp), (b, imp')) ->
                (term_eq' a b) && (FStarC_Syntax_Util.eq_aqual imp imp')) xs
         ys) in
  let comp_eq c d =
  ...
  | (FStarC_Syntax_Syntax.Tm_uinst (t, uu___), FStarC_Syntax_Syntax.Tm_uinst
     (s, uu___1)) -> term_eq' t s
  | (FStarC_Syntax_Syntax.Tm_constant c1, FStarC_Syntax_Syntax.Tm_constant
     c2) -> FStarC_Const.eq_const c1 c2
  | (FStarC_Syntax_Syntax.Tm_type u, FStarC_Syntax_Syntax.Tm_type v) -> u = v
  | (FStarC_Syntax_Syntax.Tm_abs
     ...
     { FStarC_Syntax_Syntax.bs = ys; FStarC_Syntax_Syntax.body = u;
      ...
      when (FStarC_List.length xs) = (FStarC_List.length ys) ->
  ...
  | (FStarC_Syntax_Syntax.Tm_abs
     ...
     { FStarC_Syntax_Syntax.bs = ys; FStarC_Syntax_Syntax.body = u;
       FStarC_Syntax_Syntax.rc_opt = uu___1;_})
      ->
      if (FStarC_List.length xs) > (FStarC_List.length ys)
      then
        let uu___2 = FStarC_Util.first_N (FStarC_List.length ys) xs in
        (match uu___2 with
         | (xs1, xs') ->
             let t12 =
               let uu___3 =
               ...
               FStarC_Syntax_Syntax.mk uu___3 t11.FStarC_Syntax_Syntax.pos in
             term_eq' t12 t21)
      else
        (let uu___3 = FStarC_Util.first_N (FStarC_List.length xs) ys in
         match uu___3 with
         | (ys1, ys') ->
             let t22 =
               let uu___4 =
               ...
               FStarC_Syntax_Syntax.mk uu___4 t21.FStarC_Syntax_Syntax.pos in
             term_eq' t11 t22)
  ...
  | (FStarC_Syntax_Syntax.Tm_app
     ...
     FStarC_Syntax_Syntax.Tm_app
     {
      ...
      (FStarC_Syntax_Syntax.fv_eq_lid fv_eq_1 FStarC_Parser_Const.eq2_lid) &&
        (FStarC_Syntax_Syntax.fv_eq_lid fv_eq_2 FStarC_Parser_Const.eq2_lid)
  ...
  | (FStarC_Syntax_Syntax.Tm_match
     ...
     { FStarC_Syntax_Syntax.scrutinee = t';
      ...
      ->
      (((FStarC_List.length pats) = (FStarC_List.length pats')) &&
         (FStarC_List.forall2
            (fun uu___2 uu___3 ->
               match (uu___2, uu___3) with
               | ((uu___4, uu___5, e), (uu___6, uu___7, e')) -> term_eq' e e')
            pats pats'))
        && (term_eq' t t')
  | (FStarC_Syntax_Syntax.Tm_ascribed
     { FStarC_Syntax_Syntax.tm = t12;
       FStarC_Syntax_Syntax.asc =
         (Fstarcompiler.FStar_Pervasives.Inl t22, uu___, uu___1);
       FStarC_Syntax_Syntax.eff_opt = uu___2;_},
     FStarC_Syntax_Syntax.Tm_ascribed
     { FStarC_Syntax_Syntax.tm = s1;
       FStarC_Syntax_Syntax.asc =
         (Fstarcompiler.FStar_Pervasives.Inl s2, uu___3, uu___4);
  ...
  | (FStarC_Syntax_Syntax.Tm_let
     ...
     { FStarC_Syntax_Syntax.lbs = (is_rec', lbs');
      ...
      when is_rec = is_rec' ->
      (((FStarC_List.length lbs) = (FStarC_List.length lbs')) &&
         (FStarC_List.forall2
            (fun lb1 lb2 ->
        ...
        && (term_eq' t s)
  | (FStarC_Syntax_Syntax.Tm_uvar (u, uu___), FStarC_Syntax_Syntax.Tm_uvar
     (u', uu___1)) ->
  ...
  | (FStarC_Syntax_Syntax.Tm_meta
     { FStarC_Syntax_Syntax.tm2 = t12; FStarC_Syntax_Syntax.meta = uu___;_},
     uu___1) -> term_eq' t12 t21
  | (uu___, FStarC_Syntax_Syntax.Tm_meta
     { FStarC_Syntax_Syntax.tm2 = t22; FStarC_Syntax_Syntax.meta = uu___1;_})
      -> term_eq' t11 t22
  | (FStarC_Syntax_Syntax.Tm_delayed uu___, uu___1) ->
      let uu___2 =
        ...
        FStarC_Format.fmt2 "Impossible: %s and %s" uu___3 uu___4 in
      failwith uu___2
  ...
  | (FStarC_Syntax_Syntax.Tm_unknown, FStarC_Syntax_Syntax.Tm_unknown) ->
      true
  | uu___ -> false
let term_eq (t1 : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax)
  (t2 : FStarC_Syntax_Syntax.term' FStarC_Syntax_Syntax.syntax) : Prims.bool=
  let b = term_eq' t1 t2 in
  if Prims.op_Negation b
  then
    (let uu___1 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t1 in
     let uu___2 = FStarC_Class_Show.show FStarC_Syntax_Print.showable_term t2 in
     FStarC_Format.print2 ">>>>>>>>>>>Term %s is not equal to %s\n" uu___1
       uu___2)
  else ();
  b
