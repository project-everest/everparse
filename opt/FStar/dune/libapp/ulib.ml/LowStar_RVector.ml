...
let uu___is_Cpy :
  'rst 'a .
    ('rst, 'a) LowStar_Regional.regional ->
      ('rst, 'a, Obj.t) copyable -> Prims.bool
  = fun rg -> fun projectee -> true
let __proj__Cpy__item__copy :
  'rst 'a .
    ('rst, 'a) LowStar_Regional.regional ->
      ('rst, 'a, Obj.t) copyable -> 'rst -> 'a -> 'a -> unit
...
let alloc_empty :
  'a 'rst . ('rst, 'a) LowStar_Regional.regional -> ('a, 'rst, Obj.t) rvector
  =
  fun rg ->
    LowStar_Vector.Vec
...
let alloc_rid :
  'a 'rst .
    ('rst, 'a) LowStar_Regional.regional ->
      LowStar_Vector.uint32_t -> unit -> ('a, 'rst, Obj.t) rvector
  =
  fun rg ->
    fun len ->
      fun rid ->
...
let alloc :
  'a 'rst .
    ('rst, 'a) LowStar_Regional.regional ->
      LowStar_Vector.uint32_t -> ('a, 'rst, Obj.t) rvector
  =
  fun rg -> fun len -> FStar_HyperStack_ST.new_region (); alloc_rid rg len ()
let insert :
  'a 'rst .
    ('rst, 'a) LowStar_Regional.regional ->
      ('a, 'rst, Obj.t) rvector -> 'a -> ('a, 'rst, Obj.t) rvector
  =
  fun rg ->
    fun rv ->
      fun v ->
...
let assign :
  'a 'rst .
    ('rst, 'a) LowStar_Regional.regional ->
      ('a, 'rst, Obj.t) rvector -> LowStar_Vector.uint32_t -> 'a -> unit
  =
  fun rg ->
    fun rv ->
      fun i ->
...
let free :
  'a 'rst .
    ('rst, 'a) LowStar_Regional.regional -> ('a, 'rst, Obj.t) rvector -> unit
  =
  fun rg ->
    fun rv ->
      let hh0 = FStar_HyperStack_ST.get () in
