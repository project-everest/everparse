...
type ('src, 'rrel, 'rel, 'dest) buffer_view =
  ...
  ('src, 'dest) view 
let uu___is_BufferView (projectee : ('src, 'rrel, 'rel, 'dest) buffer_view) :
  Prims.bool= true
let __proj__BufferView__item__buf
  (projectee : ('src, 'rrel, 'rel, 'dest) buffer_view) :
  ('src, 'rrel, 'rel) LowStar_Monotonic_Buffer.mbuffer=
  match projectee with | BufferView (buf, v) -> buf
let __proj__BufferView__item__v
  (projectee : ('src, 'rrel, 'rel, 'dest) buffer_view) : ('src, 'dest) view=
...
let get_view (bv : 'b buffer) : (Obj.t, 'b) view=
  let uu___ = bv in
  match uu___ with
  | FStar_Pervasives.Mkdtuple4
