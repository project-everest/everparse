...
let const_on :
  'k 'v .
    'k FStar_OrdSet.cmp ->
      ('k, Obj.t) FStar_OrdSet.ordset -> 'v -> ('k, 'v, Obj.t) ordmap
  =
  fun f ->
    fun d ->
      fun x ->
...
let select :
  'k 'v .
    'k FStar_OrdSet.cmp ->
      'k -> ('k, 'v, Obj.t) ordmap -> 'v FStar_Pervasives_Native.option
  = fun f -> fun x -> fun m -> __proj__Mk_map__item__m f m x
let insert :
  'a .
    'a FStar_OrdSet.cmp ->
      'a ->
        ('a, Obj.t) FStar_OrdSet.ordset -> ('a, Obj.t) FStar_OrdSet.ordset
  =
  fun f ->
...
let update :
  'k 'v .
    'k FStar_OrdSet.cmp ->
      'k -> 'v -> ('k, 'v, Obj.t) ordmap -> ('k, 'v, Obj.t) ordmap
  =
  fun f ->
    fun x ->
      fun y ->
        fun m ->
          ...
          Mk_map (s', g')
let contains :
  'k 'v . 'k FStar_OrdSet.cmp -> 'k -> ('k, 'v, Obj.t) ordmap -> Prims.bool =
  fun f ->
...
let remove :
  'k 'v .
    'k FStar_OrdSet.cmp ->
      'k -> ('k, 'v, Obj.t) ordmap -> ('k, 'v, Obj.t) ordmap
  =
  fun f ->
    fun x ->
      fun m ->
...
let choose :
  'k 'v .
    'k FStar_OrdSet.cmp ->
      ('k, 'v, Obj.t) ordmap -> ('k * 'v) FStar_Pervasives_Native.option
  =
  fun f ->
    fun m ->
...
let size : 'k 'v . 'k FStar_OrdSet.cmp -> ('k, 'v, Obj.t) ordmap -> Prims.nat
