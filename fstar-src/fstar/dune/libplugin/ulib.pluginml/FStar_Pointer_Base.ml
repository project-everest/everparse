...
let (dfst_struct_field :
  struct_typ ->
    (unit struct_field, Obj.t) Fstarcompiler.Prims.dtuple2 -> Prims.string)
  =
  fun s ->
    fun p ->
      let uu___ = p in
...
type 's struct_literal =
  (unit struct_field, Obj.t) Fstarcompiler.Prims.dtuple2 Prims.list
...
let rec (path_destruct_l :
  typ ->
    typ ->
      (unit, unit) path ->
        (typ,
          ((unit, unit) step, (unit, unit) path) Fstarcompiler.Prims.dtuple2)
