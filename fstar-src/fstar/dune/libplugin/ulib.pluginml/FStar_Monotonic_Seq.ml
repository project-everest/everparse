...
let alloc_mref_iseq :
  'a 'p . unit -> 'a FStar_Seq_Base.seq -> (Obj.t, 'a, 'p) i_seq =
  fun r -> fun init -> FStar_HyperStack_ST.ralloc () init
type ('r, 'a, 'p, 'n, 'x, 'm, 'h) i_at_least = unit
type ('r, 'a, 'p, 'x, 'is, 'h) int_at_most = unit
let i_read : 'a 'p . unit -> (Obj.t, 'a, 'p) i_seq -> 'a FStar_Seq_Base.seq =
  fun r -> fun m -> FStar_HyperStack_ST.op_Bang m
type ('r, 'a, 'p, 'm, 'h) i_contains = unit
let i_write_at_end : 'a 'p . unit -> (Obj.t, 'a, 'p) i_seq -> 'a -> unit =
  fun rgn ->
    fun r ->
      fun x ->
        FStar_HyperStack_ST.recall r;
...
let (itest :
  unit -> (Obj.t, Prims.nat, unit invariant) i_seq -> Prims.nat -> unit) =
  fun r ->
    fun a ->
      fun k ->
        let h0 = FStar_HyperStack_ST.get () in
  ...
  'a .
    unit ->
      Prims.nat ->
        unit ->
          Prims.nat ->
            (Obj.t, 'a) log_t -> (Obj.t, 'a, Obj.t, Obj.t, Obj.t) seqn
  =
  fun l ->
    fun max ->
      fun i ->
...
let increment_seqn :
  'a .
    unit ->
      Prims.nat ->
        unit ->
          (Obj.t, 'a) log_t -> (Obj.t, 'a, Obj.t, Obj.t, Obj.t) seqn -> unit
  =
  fun l ->
    fun max ->
      fun i ->
...
let testify_seqn :
  'a .
    unit ->
      unit ->
        (Obj.t, 'a) log_t ->
          Prims.nat -> (Obj.t, 'a, Obj.t, Obj.t, Obj.t) seqn -> unit
  =
  fun i ->
    fun l ->
      fun log ->
