...
type ('a, 'wp) tac_repr =
  Fstarcompiler.FStarC_Tactics_Types.proofstate ->
    'a Fstarcompiler.FStarC_Tactics_Result.__result
type ('a, 'x, 'ps, 'post) tac_return_wp = 'post
let tac_return : 'a . 'a -> ('a, Obj.t) tac_repr =
  fun x -> fun s -> Fstarcompiler.FStarC_Tactics_Result.Success (x, s)
    ...
    fun r2 ->
      fun t1 ->
        fun t2 ->
          fun ps ->
            let ps1 =
              Fstarcompiler.FStarC_Tactics_Types.set_proofstate_range ps r1 in
            let ps2 = Fstarcompiler.FStarC_Tactics_Types.incr_depth ps1 in
            let r = t1 ps2 in
            match r with
            | Fstarcompiler.FStarC_Tactics_Result.Success (a1, ps') ->
                let ps'1 =
                  Fstarcompiler.FStarC_Tactics_Types.set_proofstate_range ps'
                    r2 in
                (match Fstarcompiler.FStarC_Tactics_Types.tracepoint ps'1
                 with
                 | true ->
                     t2 a1
                       (Fstarcompiler.FStarC_Tactics_Types.decr_depth ps'1))
            | Fstarcompiler.FStarC_Tactics_Result.Failed (e, ps') ->
                Fstarcompiler.FStarC_Tactics_Result.Failed (e, ps')
...
let lift_div_tac : 'a 'wp . (unit -> 'a) -> ('a, 'wp) tac_repr =
  fun f ->
    fun ps ->
      let uu___ = f () in
      Fstarcompiler.FStarC_Tactics_Result.Success (uu___, ps)
let (get :
  unit -> (Fstarcompiler.FStarC_Tactics_Types.proofstate, Obj.t) tac_repr) =
  fun uu___ -> fun ps -> Fstarcompiler.FStarC_Tactics_Result.Success (ps, ps)
let raise : 'a . Prims.exn -> ('a, Obj.t) tac_repr =
  fun e -> fun ps -> Fstarcompiler.FStarC_Tactics_Result.Failed (e, ps)
