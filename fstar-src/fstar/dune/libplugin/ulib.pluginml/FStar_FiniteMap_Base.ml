...
type ('a, 'b) map =
  ('a FStar_FiniteSet_Base.set, ('a, 'b, Obj.t) setfun_t)
    Fstarcompiler.Prims.dtuple2
let domain : 'a 'b . ('a, 'b) map -> 'a FStar_FiniteSet_Base.set =
  fun m ->
    let uu___ = m in
    match uu___ with | Fstarcompiler.Prims.Mkdtuple2 (keys, uu___1) -> keys
let elements : 'a 'b . ('a, 'b) map -> ('a, 'b, Obj.t) setfun_t =
  fun m ->
    let uu___ = m in
    match uu___ with | Fstarcompiler.Prims.Mkdtuple2 (uu___1, f) -> f
...
let glue :
  'a 'b .
    'a FStar_FiniteSet_Base.set -> ('a, 'b, Obj.t) setfun_t -> ('a, 'b) map
  = fun keys -> fun f -> Fstarcompiler.Prims.Mkdtuple2 (keys, f)
  ...
  fun k ->
    fun v ->
      fun m ->
        ...
        Fstarcompiler.Prims.Mkdtuple2 (keys', f')
