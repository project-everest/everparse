...
let (__proj__Mk__item__v : t -> Obj.t FStar_Int.int_t) =
  fun projectee -> match projectee with | Mk v -> v
let (v : t -> Obj.t FStar_Int.int_t) = fun x -> __proj__Mk__item__v x
type 'x fits = unit
let (int_to_t : Obj.t FStar_Int.int_t -> t) = fun x -> Mk x
...
let (ne : t -> t -> Prims.bool) =
  fun a -> fun b -> FStar_Int.ne (Prims.of_int (128)) (v a) (v b)
