...
let empty : 'uuuuu . 'uuuuu cmp -> ('uuuuu, Obj.t) ordset = fun uu___ -> []
let tail : 'a . 'a cmp -> ('a, Obj.t) ordset -> ('a, Obj.t) ordset =
  fun f -> fun s -> Prims.__proj__Cons__item__tl s
let head : 'uuuuu . 'uuuuu cmp -> ('uuuuu, Obj.t) ordset -> 'uuuuu =
  fun uu___ -> fun s -> Prims.__proj__Cons__item__hd s
let mem :
  'uuuuu . 'uuuuu cmp -> 'uuuuu -> ('uuuuu, Obj.t) ordset -> Prims.bool =
  fun uu___ -> fun x -> fun s -> FStar_List_Tot_Base.mem x s
let mem_of : 'a . 'a cmp -> ('a, Obj.t) ordset -> 'a -> Prims.bool =
...
let last_lib : 'a . 'a cmp -> ('a, Obj.t) ordset -> 'a =
  fun f ->
    fun s -> FStar_Pervasives_Native.snd (FStar_List_Tot_Base.unsnoc s)
let last : 'a . 'a cmp -> ('a, Obj.t) ordset -> 'a =
...
let liat_lib : 'a . 'a cmp -> ('a, Obj.t) ordset -> 'a Prims.list =
  fun f ->
    fun s -> FStar_Pervasives_Native.fst (FStar_List_Tot_Base.unsnoc s)
let liat : 'a . 'a cmp -> ('a, Obj.t) ordset -> ('a, Obj.t) ordset =
  fun f -> fun s -> liat_lib f s
let unsnoc : 'a . 'a cmp -> ('a, Obj.t) ordset -> (('a, Obj.t) ordset * 'a) =
  fun f ->
    fun s ->
      let l = FStar_List_Tot_Base.unsnoc s in
      ((FStar_Pervasives_Native.fst l), (FStar_Pervasives_Native.snd l))
let as_list : 'a . 'a cmp -> ('a, Obj.t) ordset -> 'a Prims.list =
  fun f -> fun s -> s
let rec insert' :
  'uuuuu .
...
let distinct : 'a . 'a cmp -> 'a Prims.list -> ('a, Obj.t) ordset =
...
let size' : 'a . 'a cmp -> ('a, Obj.t) ordset -> Prims.nat =
...
let choose :
  'a . 'a cmp -> ('a, Obj.t) ordset -> 'a FStar_Pervasives_Native.option =
  fun f ->
    fun s ->
      match s with
      | [] -> FStar_Pervasives_Native.None
      | x::uu___ -> FStar_Pervasives_Native.Some x
let remove : 'a . 'a cmp -> 'a -> ('a, Obj.t) ordset -> ('a, Obj.t) ordset =
...
let singleton : 'a . 'a cmp -> 'a -> ('a, Obj.t) ordset =
  fun f -> fun x -> [x]
let rec smart_minus :
  'a .
    'a cmp -> ('a, Obj.t) ordset -> ('a, Obj.t) ordset -> ('a, Obj.t) ordset
...
let fold :
  'a 'acc .
    'a cmp -> ('acc -> 'a -> 'acc) -> 'acc -> ('a, Obj.t) ordset -> 'acc
  =
  fun f ->
