...
let uu___is_MonoidMorphism :
  'a 'b .
    ('a -> 'b) ->
      'a monoid ->
        'b monoid ->
          ('a, 'b, Obj.t, Obj.t, Obj.t) monoid_morphism -> Prims.bool
  = fun f -> fun ma -> fun mb -> fun projectee -> true
let intro_monoid_morphism :
  'a 'b .
    ('a -> 'b) ->
      'a monoid -> 'b monoid -> ('a, 'b, Obj.t, Obj.t, Obj.t) monoid_morphism
...
type 'p neg = unit
let (uu___1 : (unit, unit, unit neg, Obj.t, Obj.t) monoid_morphism) =
  Obj.magic
    (intro_monoid_morphism (fun uu___ -> ()) conjunction_monoid
       disjunction_monoid)
let (uu___2 : (unit, unit, unit neg, Obj.t, Obj.t) monoid_morphism) =
  Obj.magic
...
let uu___is_LAct :
  'm . 'm monoid -> unit -> ('m, Obj.t, Obj.t) left_action -> Prims.bool =
  fun mm -> fun a -> fun projectee -> true
let __proj__LAct__item__act :
  'm .
    'm monoid ->
      unit -> ('m, Obj.t, Obj.t) left_action -> 'm -> Obj.t -> Obj.t
  =
  fun mm ->
    fun a ->
      fun projectee ->
