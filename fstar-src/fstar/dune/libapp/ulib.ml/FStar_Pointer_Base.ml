...
type 'l struct_field = unit struct_field'
type 'l union_field = unit struct_field
...
type 't npointer = unit _npointer
let (nullptr : typ -> unit npointer) = fun t -> NullPtr
type 't pointer = unit npointer
type 't buffer_root =
...
type ('l, 'typeuofutyp, 'f) type_of_struct_field' =
  (unit, 'typeuofutyp, unit) type_of_struct_field''
...
let gtdata_create : 'key 'value . 'key -> 'value -> ('key, 'value) gtdata =
  fun k -> fun v -> Prims.Mkdtuple2 (k, v)
...
type 's struct_literal = (unit struct_field, Obj.t) Prims.dtuple2 Prims.list
    ...
    fun l ->
      fun f ->
        ...
        | FStar_Pervasives_Native.Some p ->
            let uu___ = p in
            (match uu___ with | Prims.Mkdtuple2 (uu___1, v) -> v)
...
type ('from, 'dummyV0, 'dummyV1, 'dummyV2, 'dummyV3) path_disjoint_t =
  | PathDisjointStep of typ * typ * typ * (unit, unit) path * (unit, 
  unit) step * (unit, unit) step 
  | PathDisjointIncludes of typ * typ * (unit, unit) path * (unit, unit) path
  * typ * typ * (unit, unit) path * (unit, unit) path * (unit, unit, 
  unit, unit, unit) path_disjoint_t 
...
let rec (path_destruct_l :
  typ ->
    typ ->
      (unit, unit) path ->
        (typ, ((unit, unit) step, (unit, unit) path) Prims.dtuple2)
