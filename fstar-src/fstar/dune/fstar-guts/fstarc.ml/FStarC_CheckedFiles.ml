      ...
      FStarC_Stats.record "hash_dependences"
        (fun uu___ ->
           ...
           let has_interface =
             ...
             FStarC_Option.isSome uu___1 in
           let interface_checked_file_name =
             ...
             then
               let uu___2 =
                 let uu___3 =
                   let uu___4 =
                     FStarC_Parser_Dep.interface_of deps module_name in
                 ...
                 (match uu___1 with
                  | FStar_Pervasives_Native.None ->
                      ...
                      ((let uu___3 = FStarC_Effect.op_Bang dbg in
                        if uu___3 then FStarC_Util.print1 "%s\n" msg else ());
               ...
               | fn2::deps1 ->
                   ...
                   let digest =
                     ...
                     | FStar_Pervasives_Native.None ->
                         let msg =
                           FStarC_Util.format2
                             "For dependency %s, cache file %s is not loaded"
                             fn2 cache_fn in
  ...
  fun fn ->
    fun checked_fn ->
      (let uu___1 = FStarC_Effect.op_Bang dbg in
       if uu___1
       then
         FStarC_Util.print1 "Trying to load checked file result %s\n"
           checked_fn
       else ());
      (let elt = FStarC_SMap.try_find mcache checked_fn in
       if FStarC_Util.is_some elt
       then
         ((let uu___2 = FStarC_Effect.op_Bang dbg in
           if uu___2
           then
             FStarC_Util.print1 "Already loaded checked file %s\n" checked_fn
           else ());
          FStarC_Util.must elt)
       else
         (let add_and_return elt1 =
            FStarC_SMap.add mcache checked_fn elt1; elt1 in
          if Prims.op_Negation (FStarC_Filepath.file_exists checked_fn)
          then
            let msg =
              FStarC_Util.format1 "checked file %s does not exist" checked_fn in
            ...
            (let entry = FStarC_Util.load_value_from_file checked_fn in
             ...
             | FStar_Pervasives_Native.None ->
                 let msg =
                   FStarC_Util.format1 "checked file %s is corrupt"
                     checked_fn in
                 add_and_return ((Invalid msg), (FStar_Pervasives.Inl msg))
                   ...
                   | (deps_dig, tc_result1) ->
                       ...
                       then
                         let elt1 =
                           let uu___3 =
                             let uu___4 =
                               FStarC_Util.digest_of_file checked_fn in
                             Valid uu___4 in
        ...
        (fun uu___ ->
           ...
           let uu___1 =
             (FStarC_Parser_Dep.is_implementation fn) &&
               (FStarC_Util.is_some i_fn_opt) in
           if uu___1
           then
             let i_fn = FStarC_Util.must i_fn_opt in
    ...
    fun stage1 ->
      fun stage2 ->
        FStarC_Errors.with_ctx
          ...
          (fun uu___ ->
             FStarC_Util.save_2values_to_file cache_file stage1 stage2)
          ...
          then
            ...
            | FStar_Pervasives.Inl msg ->
                ...
                let uu___2 =
                  let uu___3 =
                    let uu___4 =
                      FStarC_Util.format1 "Checked file %s was not written."
                        cache_file in
...
let (unsafe_raw_load_checked_file :
  ...
  fun checked_fn ->
    ...
    | FStar_Pervasives_Native.Some (s1, s2) ->
        let uu___ =
          let uu___1 =
            FStarC_List.map FStar_Pervasives_Native.fst s2.deps_dig in
