...
let (arg_of_non_null_binder :
  FStarC_Syntax_Syntax.binder -> FStarC_Syntax_Syntax.arg) =
  fun b ->
    let uu___ =
    ...
    fun u2 ->
      let rec compare_kernel uk1 =
        fun uk2 ->
          ...
          | (FStarC_Syntax_Syntax.U_name u11, FStarC_Syntax_Syntax.U_name
             u21) ->
...
let (comp_effect_name : FStarC_Syntax_Syntax.comp -> FStarC_Ident.lident) =
  fun c ->
...
let (comp_flags :
  FStarC_Syntax_Syntax.comp -> FStarC_Syntax_Syntax.cflag Prims.list) =
  fun c ->
    match c.FStarC_Syntax_Syntax.n with
...
let (is_named_tot : FStarC_Syntax_Syntax.comp -> Prims.bool) =
  fun c ->
    match c.FStarC_Syntax_Syntax.n with
    | FStarC_Syntax_Syntax.Comp c1 ->
...
let (is_total_comp : FStarC_Syntax_Syntax.comp -> Prims.bool) =
  fun c ->
      ...
      (let uu___ = comp_flags c in
       FStarC_Util.for_some
         (fun uu___1 ->
            ...
            | uu___2 -> false) uu___)
let (is_partial_return : FStarC_Syntax_Syntax.comp -> Prims.bool) =
  fun c ->
    let uu___ = comp_flags c in
    FStarC_Util.for_some
      (fun uu___1 ->
         ...
         | uu___2 -> false) uu___
let (is_tot_or_gtot_comp : FStarC_Syntax_Syntax.comp -> Prims.bool) =
  fun c ->
    (is_total_comp c) ||
      (let uu___ = comp_effect_name c in
...
let (is_pure_effect : FStarC_Ident.lident -> Prims.bool) =
  fun l ->
    ((FStarC_Ident.lid_equals l FStarC_Parser_Const.effect_Tot_lid) ||
      ...
      || (FStarC_Ident.lid_equals l FStarC_Parser_Const.effect_Pure_lid)
let (is_pure_comp : FStarC_Syntax_Syntax.comp -> Prims.bool) =
  fun c ->
    match c.FStarC_Syntax_Syntax.n with
    | FStarC_Syntax_Syntax.Total uu___ -> true
    | FStarC_Syntax_Syntax.GTotal uu___ -> false
...
let (is_pure_or_ghost_comp : FStarC_Syntax_Syntax.comp -> Prims.bool) =
  fun c ->
    (is_pure_comp c) ||
      (let uu___ = comp_effect_name c in is_ghost_effect uu___)
let (is_pure_or_ghost_effect : FStarC_Ident.lident -> Prims.bool) =
  fun l -> (is_pure_effect l) || (is_ghost_effect l)
let (is_pure_or_ghost_function : FStarC_Syntax_Syntax.typ -> Prims.bool) =
  fun t ->
    let uu___ =
      let uu___1 = FStarC_Syntax_Subst.compress t in
      uu___1.FStarC_Syntax_Syntax.n in
    ...
    | uu___1 -> true
let (is_lemma_comp : FStarC_Syntax_Syntax.comp -> Prims.bool) =
  fun c ->
    match c.FStarC_Syntax_Syntax.n with
    | FStarC_Syntax_Syntax.Comp ct ->
    ...
    | uu___ -> false
let (is_lemma : FStarC_Syntax_Syntax.typ -> Prims.bool) =
  fun t ->
    let uu___ =
      let uu___1 = FStarC_Syntax_Subst.compress t in
      uu___1.FStarC_Syntax_Syntax.n in
  ...
  fun t ->
    let rec aux t1 =
      fun args ->
        ...
        | FStarC_Syntax_Syntax.Tm_meta
            { FStarC_Syntax_Syntax.tm2 = t0;
              FStarC_Syntax_Syntax.meta = FStarC_Syntax_Syntax.Meta_named
                uu___;_}
            -> aux t0 args
        | FStarC_Syntax_Syntax.Tm_meta
            { FStarC_Syntax_Syntax.tm2 = t0;
...
let (un_uinst : FStarC_Syntax_Syntax.term -> FStarC_Syntax_Syntax.term) =
  fun t ->
    ...
    | uu___ -> t1
let (is_ml_comp : FStarC_Syntax_Syntax.comp -> Prims.bool) =
  fun c ->
    match c.FStarC_Syntax_Syntax.n with
    | FStarC_Syntax_Syntax.Comp c1 ->
    ...
    | uu___ -> false
let (comp_result : FStarC_Syntax_Syntax.comp -> FStarC_Syntax_Syntax.typ) =
  fun c ->
    match c.FStarC_Syntax_Syntax.n with
...
let (is_trivial_wp : FStarC_Syntax_Syntax.comp -> Prims.bool) =
  fun c ->
    let uu___ = comp_flags c in
    FStarC_Util.for_some
      (fun uu___1 ->
...
let (is_primop_lid : FStarC_Ident.lident -> Prims.bool) =
  fun l -> FStarC_Util.for_some (FStarC_Ident.lid_equals l) primops
let (is_primop : FStarC_Syntax_Syntax.term -> Prims.bool) =
  fun f ->
...
let rec (ascribe :
  FStarC_Syntax_Syntax.term ->
    FStarC_Syntax_Syntax.ascription -> FStarC_Syntax_Syntax.term)
  =
  fun t ->
    fun k ->
      match t.FStarC_Syntax_Syntax.n with
...
let (unlazy_emb : FStarC_Syntax_Syntax.term -> FStarC_Syntax_Syntax.term) =
  fun t ->
    ...
    | uu___1 -> t
let unlazy_as_t :
  'a . FStarC_Syntax_Syntax.lazy_kind -> FStarC_Syntax_Syntax.term -> 'a =
  fun k ->
    fun t ->
      let uu___ =
        let uu___1 = FStarC_Syntax_Subst.compress t in
          ...
          then FStarC_Dyn.undyn v
            ...
            (let uu___5 =
               ...
               let uu___7 =
                 FStarC_Class_Show.show
                   FStarC_Syntax_Syntax.showable_lazy_kind k' in
               FStarC_Util.format2 "Expected Tm_lazy of kind %s, got %s"
                 uu___6 uu___7 in
             failwith uu___5)
...
let (canon_app : FStarC_Syntax_Syntax.term -> FStarC_Syntax_Syntax.term) =
  fun t ->
...
let (is_fun : FStarC_Syntax_Syntax.term -> Prims.bool) =
  fun e ->
    ...
    | uu___1 -> false
let (is_function_typ : FStarC_Syntax_Syntax.typ -> Prims.bool) =
  fun t ->
    let uu___ =
      let uu___1 = FStarC_Syntax_Subst.compress t in
      uu___1.FStarC_Syntax_Syntax.n in
    ...
    | uu___1 -> false
let rec (pre_typ : FStarC_Syntax_Syntax.typ -> FStarC_Syntax_Syntax.typ) =
  fun t ->
    let t1 = FStarC_Syntax_Subst.compress t in
    match t1.FStarC_Syntax_Syntax.n with
    | FStarC_Syntax_Syntax.Tm_refine
...
let (range_of_arg : FStarC_Syntax_Syntax.arg -> FStarC_Range_Type.range) =
  ...
  fun args ->
    fun r ->
      FStarC_List.fold_left
        (fun r1 ->
           fun a ->
             let uu___ = range_of_arg a in
             FStarC_Range_Ops.union_ranges r1 uu___) r args
let (mk_app :
  FStarC_Syntax_Syntax.term ->
        ...
        fun abs_body_lcomp ->
          ...
          | FStarC_Syntax_Syntax.Tm_abs
              ...
              then
                ...
                (match uu___1 with
                 | (bs', t3, what1) ->
                     ((FStarC_List.op_At bs bs'), t3, what1))
              else (bs, t2, what)
        ...
        FStarC_Ident.lident ->
          FStarC_Syntax_Syntax.term ->
            FStarC_Syntax_Syntax.term Prims.list ->
              FStarC_Range_Type.range -> FStarC_Syntax_Syntax.letbinding)
  =
  fun lbname ->
    fun univ_vars ->
      fun typ ->
...
let (exp_int : Prims.string -> FStarC_Syntax_Syntax.term) =
  fun s ->
    FStarC_Syntax_Syntax.mk
      ...
      FStarC_Range_Type.dummyRange
let (exp_char : FStar_Char.char -> FStarC_Syntax_Syntax.term) =
  fun c ->
    FStarC_Syntax_Syntax.mk
...
let (exp_string : Prims.string -> FStarC_Syntax_Syntax.term) =
  fun s ->
    FStarC_Syntax_Syntax.mk
      ...
      FStarC_Range_Type.dummyRange
let (fvar_const : FStarC_Ident.lid -> FStarC_Syntax_Syntax.term) =
      ...
      fun phi2 ->
        ...
        let uu___1 =
          FStarC_Range_Ops.union_ranges phi1.FStarC_Syntax_Syntax.pos
            phi2.FStarC_Syntax_Syntax.pos in
        FStarC_Syntax_Syntax.mk uu___ uu___1
let (mk_neg : FStarC_Syntax_Syntax.term -> FStarC_Syntax_Syntax.term) =
  fun phi ->
    let uu___ =
      let uu___1 =
...
let (b2t : FStarC_Syntax_Syntax.term -> FStarC_Syntax_Syntax.term) =
  fun e ->
    let uu___ =
      let uu___1 =
...
let (residual_tot :
  FStarC_Syntax_Syntax.typ -> FStarC_Syntax_Syntax.residual_comp) =
  fun t ->
    {
      FStarC_Syntax_Syntax.residual_effect =
        FStarC_Parser_Const.effect_Tot_lid;
  ...
  fun t ->
    fun lopt ->
      ...
      let uu___ =
        ...
        FStarC_Syntax_Syntax.Tm_app uu___1 in
      FStarC_Syntax_Syntax.mk uu___ t.FStarC_Syntax_Syntax.pos
let (mk_reflect : FStarC_Syntax_Syntax.term -> FStarC_Syntax_Syntax.term) =
  fun t ->
    let reflect_ =
      let uu___ =
        let uu___1 =
    ...
    fun msg ->
      fun cond ->
        ...
        then true
          ...
          (if dbg
           then FStarC_Util.print1 ">>> term_eq failing: %s\n" msg
           else ();
           false)
  ...
  fun dbg ->
    fun a1 ->
      fun a2 ->
        ...
        | uu___ -> false
let (eq_aqual :
  FStarC_Syntax_Syntax.aqual -> FStarC_Syntax_Syntax.aqual -> Prims.bool) =
  fun a1 -> fun a2 -> aqual_eq_dbg false a1 a2
let (eq_bqual :
  FStarC_Syntax_Syntax.bqual -> FStarC_Syntax_Syntax.bqual -> Prims.bool) =
...
let (has_attribute :
  ...
  = fun attrs -> fun attr -> FStarC_Util.for_some (is_fvar attr) attrs
let (get_attribute :
  FStarC_Ident.lident ->
    FStarC_Syntax_Syntax.attribute Prims.list ->
      FStarC_Syntax_Syntax.args FStar_Pervasives_Native.option)
  ...
  fun attr ->
    fun attrs ->
      FStarC_List.filter
        ...
        attrs
let (process_pragma :
  FStarC_Syntax_Syntax.pragma -> FStarC_Range_Type.range -> unit) =
  fun p ->
    fun r ->
        ...
        (match uu___1 with
         ...
         | (FStarC_Syntax_Syntax.Tm_fvar fv,
            uu___2::(hd, uu___3)::(tl, uu___4)::[]) when
             ...
             let uu___5 =
               let uu___6 =
                 let uu___7 = list_elements tl in FStarC_Util.must uu___7 in
               hd :: uu___6 in
...
let (apply_wp_eff_combinators :
  ...
  fun f ->
    fun combs ->
      ...
      let uu___7 = FStarC_Util.map_option f combs.FStarC_Syntax_Syntax.repr in
      let uu___8 =
        FStarC_Util.map_option f combs.FStarC_Syntax_Syntax.return_repr in
      let uu___9 =
        FStarC_Util.map_option f combs.FStarC_Syntax_Syntax.bind_repr in
      {
...
let (apply_layered_eff_combinators :
  ...
  fun f ->
    fun combs ->
      ...
      let uu___5 =
        FStarC_Util.map_option map2 combs.FStarC_Syntax_Syntax.l_close in
      {
...
let (deduplicate_terms :
  FStarC_Syntax_Syntax.term Prims.list ->
    FStarC_Syntax_Syntax.term Prims.list)
  = fun l -> FStarC_List.deduplicate (fun x -> fun y -> term_eq x y) l
let (eq_binding :
