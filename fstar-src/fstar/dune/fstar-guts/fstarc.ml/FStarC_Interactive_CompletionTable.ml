...
let heap_from_list :
  ...
  =
  fun cmp ->
    fun values -> FStarC_List.fold_left (heap_insert cmp) EmptyHeap values
let push_nodup :
  'uuuuu .
    ('uuuuu -> Prims.string) ->
        ...
        fun prefix1 ->
          fun acc ->
            ...
            | StrBranch (k, v, lbt, rbt) ->
                ...
                let matches =
                  if explore_right then aux rbt prefix1 acc else acc in
                let matches1 =
                  if include_middle
                  then
                    ({
          ...
          fun mut_node ->
            fun mut_leaf ->
              let trie1 =
                ...
                FStarC_Util.dflt (trie_empty ()) uu___ in
      ...
      fun query1 ->
        fun acc ->
          ...
          | (ns_search_term, bindings_search_term, query2) ->
              ...
              FStarC_List.rev_map_onto
                (fun uu___1 ->
                   match uu___1 with
                   | (path_el, v) ->
                       ((FStarC_List.rev (path_el :: path_acc)), v))
      ...
      fun path1 ->
        fun mod_query ->
          let ins_ns id =
            fun bindings ->
              fun full_name ->
                fun loaded1 ->
                  let uu___ =
                    let uu___1 = names_find_exact bindings id in
                    (uu___1, loaded1) in
                  match uu___ with
                  | (FStar_Pervasives_Native.None, uu___1) ->
                      names_insert bindings id
                        (Namespace
                           { ns_name = full_name; ns_loaded = loaded1 })
          ...
          let name_of_revq query1 =
            FStarC_String.concat "." (FStarC_List.rev query1) in
          let ins id =
            fun q ->
              fun revq ->
                fun bindings ->
...
let (string_of_path : path -> Prims.string) =
  fun path1 ->
    let uu___ = FStarC_List.map (fun el -> (el.segment).completion) path1 in
  ...
  fun path1 ->
             ...
             | (acc_len, uu___2) ->
                 (match (elem.segment).prefix with
                  | FStar_Pervasives_Native.Some prefix ->
                      let completion_len =
                        FStarC_String.length (elem.segment).completion in
    ...
    | (length, (last_prefix, last_completion_length)) ->
        ((length - Prims.int_one) - last_completion_length) +
          (FStarC_String.length last_prefix)
let (first_import_of_path :
  path -> Prims.string FStar_Pervasives_Native.option) =
  fun path1 ->
    match path1 with
    | [] -> FStar_Pervasives_Native.None
    | { imports; segment = uu___;_}::uu___1 -> FStarC_List.last_opt imports
  ...
  fun annot ->
    fun loaded ->
      fun path1 ->
        ...
        let uu___2 =
          FStarC_Util.format1 (if loaded then " %s " else "(%s)") annot in
        {
...
let (autocomplete_lid : table -> query -> completion_result Prims.list) =
  fun tbl ->
    fun query1 ->
      ...
      FStarC_List.map completion_result_of_lid uu___
    ...
    fun query1 ->
      fun filter ->
        let uu___ =
          ...
          FStarC_List.filter_map filter uu___1 in
        FStarC_List.map completion_result_of_ns_or_mod uu___
