...
let (to_fsteps : FStarC_TypeChecker_Env.step Prims.list -> fsteps) =
  fun s -> FStarC_List.fold_right fstep_add_one s default_steps
type debug_switches =
  {
  gen: Prims.bool ;
  top: Prims.bool ;
...
let (add_steps :
  prim_step_set ->
    FStarC_TypeChecker_Primops_Base.primitive_step Prims.list ->
      prim_step_set)
  = fun m -> fun l -> FStarC_List.fold_right add_step l m
let (prim_from_list :
...
let (dbg_Norm : Prims.bool FStarC_Effect.ref) =
  ...
  fun psteps ->
    fun s ->
      fun e ->
        ...
        let dbg_flag =
          FStarC_List.contains FStarC_TypeChecker_Env.NormDebug s in
        let uu___ =
