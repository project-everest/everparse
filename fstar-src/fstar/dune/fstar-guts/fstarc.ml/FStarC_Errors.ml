...
let (dummy_ide_rng : FStarC_Range_Type.rng) =
...
let (fixup_issue_range :
  ...
  fun rng ->
    let rng1 =
      match rng with
      | FStar_Pervasives_Native.None -> FStarC_Effect.op_Bang fallback_range
      | FStar_Pervasives_Native.Some range ->
          let use_rng = FStarC_Range_Type.use_range range in
          let use_rng' =
            if
              (use_rng <> FStarC_Range_Type.dummy_rng) &&
  ...
  fun print ->
    ...
    let add_one e =
      ...
      (match e.issue_level with
       ...
       | uu___2 when print && (FStarC_Debug.any ()) -> print_issue e
       | uu___2 ->
           let uu___3 =
             let uu___4 = FStarC_Effect.op_Bang issues in e :: uu___4 in
      ...
      fun uu___1 ->
        fun uu___2 ->
          fun msg ->
            ...
            let ctx = get_ctx () in
            add_one
              (mk_issue EInfo rng1 msg2 FStar_Pervasives_Native.None ctx)
let diag :
  'posut .
    'posut FStarC_Class_HasRange.hasRange ->
      'posut ->
        ...
        fun uu___1 ->
          fun uu___2 ->
            fun msg ->
              let rng = FStarC_Class_HasRange.pos uu___ r in
              let uu___3 =
                fixup_issue_range (FStar_Pervasives_Native.Some rng) in
              match uu___3 with
     ...
     let r =
       ...
       | uu___1 ->
           ...
           then (err_exn uu___1; FStar_Pervasives_Native.None)
             ...
             (let uu___2 = finally_restore () in FStarC_Effect.raise uu___1) in
  ...
  fun f ->
    ...
    | (errs, rest, r) ->
        ((let uu___2 =
            ...
            uu___3.eh_add_one in
          let uu___3 =
            FStarC_List.filter (fun i -> i.issue_level = EInfo) rest in
...
let (find_multiset_discrepancy :
  ...
  fun l1 ->
    fun l2 ->
      let sort = FStarC_List.sortWith (fun x -> fun y -> x - y) in
      let rec collect l =
        match l with
        | [] -> []
        | hd::tl ->
