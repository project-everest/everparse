  ...
  fun t ->
    fun s ->
      ...
      | FStarC_Syntax_Syntax.SomeUseRange r ->
          let uu___ =
            let uu___1 =
              FStarC_Range_Type.use_range t.FStarC_Syntax_Syntax.pos in
...
let (mk_range :
  FStarC_Range_Type.range ->
    FStarC_Syntax_Syntax.subst_ts -> FStarC_Range_Type.range)
  =
  fun r ->
    fun s ->
      match FStar_Pervasives_Native.snd s with
  ...
  fun s ->
    fun asc ->
      ...
      | (annot, topt, use_eq) ->
          ...
          let uu___1 = FStarC_Util.map_opt topt (subst' s) in
...
let (shift_subst :
  ...
  = fun n -> fun s -> FStarC_List.map (shift n) s
let shift_subst' :
  'uuuuu .
    Prims.int ->
  ...
  fun n ->
    fun s ->
      let uu___ =
        FStarC_List.map (shift_subst n) (FStar_Pervasives_Native.fst s) in
  ...
  'uuuuu .
    FStarC_Syntax_Syntax.subst_ts ->
      (FStarC_Syntax_Syntax.term * 'uuuuu) Prims.list ->
        (FStarC_Syntax_Syntax.term * 'uuuuu) Prims.list
  = fun s -> FStarC_List.map (subst_arg' s)
let (subst_univs_opt :
  FStarC_Syntax_Syntax.subst_elt Prims.list Prims.list ->
      ...
      let rec aux n =
        fun p1 ->
          ...
          | FStarC_Syntax_Syntax.Pat_dot_term eopt ->
              let s1 = shift_subst' n s in
              let eopt1 = FStarC_Util.map_option (subst' s1) eopt in
              ({
                 FStarC_Syntax_Syntax.v =
              ...
              FStarC_List.map
                (fun lb ->
                   ...
                   let lbd =
                     let uu___ =
                       is_rec &&
                         (FStarC_Util.is_left lb.FStarC_Syntax_Syntax.lbname) in
                     if uu___
        ...
        | b::bs' ->
            let x' =
                  ...
                  (b.FStarC_Syntax_Syntax.binder_bv).FStarC_Syntax_Syntax.sort in
              {
                ...
                FStarC_Syntax_Syntax.index =
                  (uu___.FStarC_Syntax_Syntax.index);
  ...
  fun p ->
    let rec open_pat_aux sub =
      fun p1 ->
        ...
        | FStarC_Syntax_Syntax.Pat_dot_term eopt ->
            let eopt1 = FStarC_Util.map_option (subst sub) eopt in
            ({
               FStarC_Syntax_Syntax.v =
                 (FStarC_Syntax_Syntax.Pat_dot_term eopt1);
               FStarC_Syntax_Syntax.p = (p1.FStarC_Syntax_Syntax.p)
...
let (univ_var_closing :
  ...
  fun us ->
    let n = (FStarC_List.length us) - Prims.int_one in
    FStarC_List.mapi (fun i -> fun u -> FStarC_Syntax_Syntax.UD (u, (n - i)))
      us
let (open_univ_vars :
  FStarC_Syntax_Syntax.univ_names ->
    FStarC_Syntax_Syntax.term ->
