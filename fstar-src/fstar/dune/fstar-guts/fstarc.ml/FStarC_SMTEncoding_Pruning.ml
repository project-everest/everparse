...
type pruning_state =
  {
  macro_freenames: Prims.string Prims.list FStarC_Util.psmap ;
  trigger_to_assumption:
    FStarC_SMTEncoding_Term.assumption Prims.list FStarC_Util.psmap ;
  assumption_to_triggers: triggers_set FStarC_Util.psmap ;
  assumption_name_map: FStarC_SMTEncoding_Term.decl FStarC_Util.psmap ;
...
let (__proj__Mkpruning_state__item__macro_freenames :
  pruning_state -> Prims.string Prims.list FStarC_Util.psmap) =
  fun projectee ->
...
let (__proj__Mkpruning_state__item__assumption_to_triggers :
  pruning_state -> triggers_set FStarC_Util.psmap) =
  fun projectee ->
    match projectee with
    | { macro_freenames; trigger_to_assumption; assumption_to_triggers;
        assumption_name_map; ambients; extra_roots;_} ->
...
let (__proj__Mkpruning_state__item__assumption_name_map :
  pruning_state -> FStarC_SMTEncoding_Term.decl FStarC_Util.psmap) =
  fun projectee ->
    match projectee with
    | { macro_freenames; trigger_to_assumption; assumption_to_triggers;
...
let (print_pruning_state : pruning_state -> Prims.string) =
  fun p ->
    let t_to_a =
      FStarC_Util.psmap_fold p.trigger_to_assumption
        (fun k -> fun v -> fun acc -> (k, (FStarC_List.length v)) :: acc) [] in
    let t_to_a1 =
    ...
    let a_to_t =
      FStarC_Util.psmap_fold p.assumption_to_triggers
        (fun k ->
           fun v ->
             fun acc ->
               let uu___ =
               ...
               uu___ :: acc) [] in
    let macros =
      FStarC_Util.psmap_fold p.macro_freenames
        (fun k ->
           fun v ->
             fun acc ->
               let uu___ =
...
let (add_trigger_to_assumption :
  ...
  fun a ->
    fun p ->
      fun trig ->
        let uu___ = FStarC_Util.psmap_try_find p.trigger_to_assumption trig in
        match uu___ with
        | FStar_Pervasives_Native.None ->
            let uu___1 =
              FStarC_Util.psmap_add p.trigger_to_assumption trig [a] in
            {
        ...
        | FStar_Pervasives_Native.Some l ->
            let uu___1 =
              FStarC_Util.psmap_add p.trigger_to_assumption trig (a :: l) in
            {
              macro_freenames = (p.macro_freenames);
              trigger_to_assumption = uu___1;
              assumption_to_triggers = (p.assumption_to_triggers);
...
let (free_top_level_names :
  FStarC_SMTEncoding_Term.term -> Prims.string FStarC_RBSet.t) =
  fun uu___ ->
    (fun t ->
...
let (assumption_free_names :
  FStarC_SMTEncoding_Term.assumption -> Prims.string FStarC_RBSet.t) =
  fun uu___ ->
    (fun a ->
       Obj.magic
         (FStarC_Class_Setlike.diff ()
  ...
  fun a ->
    fun p ->
      let add_assumption_with_triggers triggers1 =
        let p1 =
          let uu___ =
            FStarC_Util.psmap_add p.assumption_to_triggers
  ...
  fun a ->
    fun p ->
      fun trigs ->
        let p1 =
          let uu___ =
            FStarC_Util.psmap_add p.assumption_name_map
              a.FStarC_SMTEncoding_Term.assumption_name
        ...
        | uu___ ->
            let uu___1 =
              FStarC_Util.psmap_add p1.assumption_to_triggers
...
let (trigger_reached : pruning_state -> Prims.string -> pruning_state) =
  fun p ->
    fun trig ->
      let uu___ = FStarC_Util.psmap_remove p.trigger_to_assumption trig in
      {
        macro_freenames = (p.macro_freenames);
...
let (remove_trigger_for_assumption :
  ...
  fun p ->
    fun trig ->
      fun aname ->
        let uu___ = FStarC_Util.psmap_try_find p.assumption_to_triggers aname in
        match uu___ with
...
let rec (add_decl :
  ...
  fun d ->
    fun p ->
      ...
      | FStarC_SMTEncoding_Term.Module (uu___, ds) ->
          FStarC_List.fold_left (fun p1 -> fun d1 -> add_decl d1 p1) p ds
      | FStarC_SMTEncoding_Term.DefineFun
          ...
          let p1 =
            let uu___3 =
              FStarC_Util.psmap_add p.macro_freenames macro free_names in
            {
...
let (add_decls :
  ...
  =
  fun ds ->
    fun p -> FStarC_List.fold_left (fun p1 -> fun d -> add_decl d p1) p ds
      ...
      (fun uu___ ->
         (fun ctxt1 ->
            ...
            let p =
              ...
              let uu___1 =
                FStarC_Util.psmap_remove (ctxt1.p).assumption_to_triggers
                  aname in
              {
    ...
    FStarC_Class_Monad.op_let_Bang st_monad () () (Obj.magic get)
      (fun uu___ ->
         (fun ctxt1 ->
            ...
            let macro_expand s =
              let uu___ =
                FStarC_Util.psmap_try_find (ctxt1.p).macro_freenames s in
              match uu___ with
...
let (prune :
  ...
  fun p ->
    fun roots ->
      let roots1 = FStarC_List.collect assumptions_of_decl roots in
      let init1 =
        let uu___ =
          Obj.magic
            (FStarC_Class_Setlike.empty ()
      ...
      | (uu___1, ctxt1) ->
          ...
          let reached_assumptions =
            FStarC_List.collect
              (fun name ->
                 let uu___2 =
                   FStarC_Util.psmap_try_find (ctxt1.p).assumption_name_map
                     name in
                 match uu___2 with
                 | FStar_Pervasives_Native.None -> []
