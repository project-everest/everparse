...
let (status_string_and_errors :
  ...
  fun s ->
    ...
    | SAT (errs, msg) ->
        let uu___ =
          FStarC_Util.format2 "%s%s" (status_tag s)
            (match msg with
...
let (query_logging : query_log) =
    ...
    let file_name =
      ...
      | FStar_Pervasives_Native.Some (uu___2, k) ->
          ((let uu___4 =
           ...
           (let uu___4 = FStarC_Util.string_of_int (k + Prims.int_one) in
            FStarC_Util.format2 "%s-%s" n uu___4)) in
    FStarC_Util.format1 "queries-%s.smt2" file_name in
  ...
  let write_to_log fresh =
    fun str -> if fresh then write_to_new_log str else append_to_log str in
  let close_log uu___ =
    let uu___1 = FStarC_Effect.op_Bang log_file_opt in
    match uu___1 with
    | FStar_Pervasives_Native.None -> ()
          ...
          let uu___3 =
            let uu___4 =
              let uu___5 =
                ...
                FStarC_Util.string_of_int uu___6 in
              FStarC_Util.format1 "smt.random_seed=%s" uu___5 in
            [uu___4] in
          "-in" :: uu___3 in
        "-smt2" :: uu___2 in
  ...
  fun suf ->
    fun s ->
      ...
      FStarC_Errors.log_issue0 FStarC_Errors_Codes.Warning_UnexpectedZ3Output
        () (Obj.magic FStarC_Errors_Msg.is_error_message_list_doc)
        (Obj.magic uu___)
let (check_z3version : FStarC_Util.proc -> unit) =
  fun p ->
    let getinfo arg =
      let s =
        let uu___ =
          FStarC_Util.format1 "(get-info :%s)\n(echo \"Done!\")\n" arg in
        FStarC_Util.ask_process p uu___ (fun uu___1 -> "Killed")
          (warn_handler []) in
      if FStarC_Util.starts_with s (Prims.strcat "(:" arg)
      then
        let ss = FStarC_String.split [34] s in
        FStarC_List.nth ss Prims.int_one
      else
        (warn_handler [] s;
         (let uu___2 =
            let uu___3 = FStarC_Util.proc_prog p in
            FStarC_Util.format1 "Could not run Z3 from `%s'" uu___3 in
          FStarC_Errors.raise_error0
            FStarC_Errors_Codes.Error_Z3InvocationError ()
  ...
  fun cmd_and_args ->
    let p =
      let uu___ =
        ...
        FStarC_Util.format1 "z3-bg-%s" uu___1 in
  ...
  let make_new_z3_proc cmd_and_args =
    (let uu___1 = FStarC_Options.hint_info () in
     ...
     then
       ...
       FStarC_Util.print2 "Creating new z3proc (cmd=[%s], version=[%s])\n"
         uu___2 uu___3
     else ());
    (let uu___2 =
...
let (smt_output_sections :
  Prims.string FStar_Pervasives_Native.option ->
    FStarC_Range_Type.range -> Prims.string Prims.list -> smt_output)
  =
  fun log_file ->
    fun r ->
      fun lines ->
                           ...
                           (match uu___4 with
                            | (labels, lines5) ->
                                ...
                                ((match remaining with
                                 ...
                                 (let uu___6 = FStarC_Util.must result_opt in
                                  {
                                    smt_result = uu___6;
...
let reading_solver_state :
  ...
  fun f -> let ss = FStarC_Effect.op_Bang bg_z3_proc in f ss.ctxt
          ...
          fun label_messages ->
            fun queryid ->
              let parse z3out =
                ...
                let unsat_core =
                  ...
                  | FStar_Pervasives_Native.Some s ->
                      let s1 =
                        FStarC_Util.trim_string (FStarC_String.concat " " s) in
                      let s2 =
                ...
                let statistics =
                  ...
                  | FStar_Pervasives_Native.Some lines1 ->
                      let parse_line line =
                        let pline =
                          FStarC_Util.split (FStarC_Util.trim_string line)
                            ":" in
                        match pline with
                        | "("::entry::[] ->
                ...
                let reason_unknown =
                  FStarC_Util.map_opt smt_output1.smt_reason_unknown
                    (fun x ->
                ...
                let status =
                  (let uu___1 = FStarC_Debug.any () in
                   ...
                   then
                     let uu___2 =
                       FStarC_Util.format1 "Z3 says: %s\n"
                         (FStarC_String.concat "\n" smt_output1.smt_result) in
                     FStarC_Util.print_string uu___2
                   else ());
              ...
              then
                ...
                let out =
                  FStarC_Util.ask_process proc input kill_handler
                    (warn_handler []) in
                ...
                (let out =
                   let uu___1 = FStarC_Effect.op_Bang bg_z3_proc in
                   uu___1.ask input in
                 let r1 = parse (FStarC_Util.trim_string out) in
                 log_result
                   (fun _fname ->
...
let (context_profile : FStarC_SMTEncoding_Term.decl Prims.list -> unit) =
  fun theory ->
    ...
    | (modules, total_decls) ->
        ...
        (if modules1 <> []
         then
           (let uu___2 = FStarC_Util.string_of_int total_decls in
            FStarC_Util.print1
