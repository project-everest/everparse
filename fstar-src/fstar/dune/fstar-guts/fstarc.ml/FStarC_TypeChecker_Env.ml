...
type cached_elt =
  (((FStarC_Syntax_Syntax.universes * FStarC_Syntax_Syntax.typ),
    ...
    FStar_Pervasives.either * FStarC_Range_Type.range)
...
let (__proj__Mkenv__item__range : env -> FStarC_Range_Type.range) =
  fun projectee ->
    match projectee with
    | { solver; range; curmodule; gamma; gamma_sig; gamma_cache; modules;
        expected_typ; sigtab; attrtab; instantiate_imp; effects = effects1;
...
let (__proj__Mkenv__item__gamma_cache : env -> cached_elt FStarC_SMap.t) =
  fun projectee ->
    match projectee with
    | { solver; range; curmodule; gamma; gamma_sig; gamma_cache; modules;
        expected_typ; sigtab; attrtab; instantiate_imp; effects = effects1;
...
let (__proj__Mkenv__item__sigtab :
  env -> FStarC_Syntax_Syntax.sigelt FStarC_SMap.t) =
  fun projectee ->
    match projectee with
    | { solver; range; curmodule; gamma; gamma_sig; gamma_cache; modules;
        expected_typ; sigtab; attrtab; instantiate_imp; effects = effects1;
...
let (__proj__Mkenv__item__attrtab :
  env -> FStarC_Syntax_Syntax.sigelt Prims.list FStarC_SMap.t) =
  fun projectee ->
    match projectee with
    | { solver; range; curmodule; gamma; gamma_sig; gamma_cache; modules;
        expected_typ; sigtab; attrtab; instantiate_imp; effects = effects1;
...
let (__proj__Mkenv__item__normalized_eff_names :
  env -> FStarC_Ident.lident FStarC_SMap.t) =
  fun projectee ->
    match projectee with
    | { solver; range; curmodule; gamma; gamma_sig; gamma_cache; modules;
        expected_typ; sigtab; attrtab; instantiate_imp; effects = effects1;
...
let (__proj__Mkenv__item__fv_delta_depths :
  env -> FStarC_Syntax_Syntax.delta_depth FStarC_SMap.t) =
  fun projectee ->
    match projectee with
    | { solver; range; curmodule; gamma; gamma_sig; gamma_cache; modules;
        expected_typ; sigtab; attrtab; instantiate_imp; effects = effects1;
...
let (__proj__Mkenv__item__identifier_info :
  env -> FStarC_TypeChecker_Common.id_info_table FStarC_Effect.ref) =
  fun projectee ->
    match projectee with
    | { solver; range; curmodule; gamma; gamma_sig; gamma_cache; modules;
        expected_typ; sigtab; attrtab; instantiate_imp; effects = effects1;
...
let (__proj__Mkenv__item__strict_args_tab :
  env -> Prims.int Prims.list FStar_Pervasives_Native.option FStarC_SMap.t) =
  fun projectee ->
    match projectee with
    | { solver; range; curmodule; gamma; gamma_sig; gamma_cache; modules;
        expected_typ; sigtab; attrtab; instantiate_imp; effects = effects1;
...
let (__proj__Mkenv__item__missing_decl :
  env -> FStarC_Ident.lident FStarC_RBSet.t) =
  fun projectee ->
    match projectee with
    | { solver; range; curmodule; gamma; gamma_sig; gamma_cache; modules;
        expected_typ; sigtab; attrtab; instantiate_imp; effects = effects1;
...
let new_sigtab : 'uuuuu . unit -> 'uuuuu FStarC_SMap.t =
  fun uu___ -> FStarC_SMap.create default_table_size
let new_gamma_cache : 'uuuuu . unit -> 'uuuuu FStarC_SMap.t =
...
let (sigtab : env -> FStarC_Syntax_Syntax.sigelt FStarC_SMap.t) =
  fun env1 -> env1.sigtab
let (attrtab : env -> FStarC_Syntax_Syntax.sigelt Prims.list FStarC_SMap.t) =
  fun env1 -> env1.attrtab
let (gamma_cache : env -> cached_elt FStarC_SMap.t) =
  ...
  fun uu___ ->
    ...
    | uu___2 ->
        let uu___3 =
          ...
          let uu___5 = FStarC_Effect.op_Bang query_indices in uu___4 ::
            uu___5 in
...
let (add_query_index : (FStarC_Ident.lident * Prims.int) -> unit) =
  fun uu___ ->
    ...
    | (l, n) ->
        ...
        (match uu___1 with
         | hd::tl ->
             FStarC_Effect.op_Colon_Equals query_indices (((l, n) :: hd) ::
               tl)
                       ...
                       (rollback_query_indices query_indices_depth;
                        (match () with
                         | () ->
                             ...
                             ((let uu___5 =
                                 FStarC_Util.physical_equality tcenv.dsenv
                                   dsenv1 in
...
let (set_range : env -> FStarC_Range_Type.range -> env) =
  fun e ->
    fun r ->
...
let (get_range : env -> FStarC_Range_Type.range) = fun e -> e.range
...
let (check_effect_is_not_a_template :
  FStarC_Syntax_Syntax.eff_decl -> FStarC_Range_Type.range -> unit) =
  fun ed ->
    fun rng ->
      if
          ...
          | (us, t) ->
              (check_effect_is_not_a_template ed env1.range;
               ...
               then
                 (let uu___3 =
                    let uu___4 =
                      FStarC_Util.string_of_int (FStarC_List.length us) in
                    let uu___5 =
                      FStarC_Util.string_of_int (FStarC_List.length insts) in
                    let uu___6 =
                      FStarC_Class_Show.show FStarC_Ident.showable_lident
                        ed.FStarC_Syntax_Syntax.mname in
                    let uu___7 =
...
and (add_sigelts :
  ...
  fun force ->
    fun env1 -> fun ses -> FStarC_List.iter (add_sigelt force env1) ses
let (try_lookup_bv :
  env ->
    ...
    fun bv ->
      ...
      (let uu___1 = FStarC_Debug.extreme () in
       ...
       then
         ...
         FStarC_Util.print2 "lookup_bv %s -> %s\n" uu___2 uu___3
       else ());
      r
    ...
    fun env1 ->
      fun lid ->
        ...
        let uu___ =
          ...
          FStarC_Util.bind_opt uu___1 mapper in
        match uu___ with
    ...
    fun bv ->
      ...
      | FStar_Pervasives_Native.None ->
          let uu___1 =
            let uu___2 =
              FStarC_Class_Show.show FStarC_Syntax_Print.showable_bv bv in
            FStarC_Util.format1 "Variable \"%s\" not found" uu___2 in
...
let name_not_found : 'a . FStarC_Ident.lid -> 'a =
  fun l ->
    let uu___ =
      ...
      FStarC_Util.format1 "Name \"%s\" not found" uu___1 in
    FStarC_Errors.raise_error FStarC_Ident.hasrange_lident l
    ...
    fun lid ->
      ...
      | uu___1 ->
          let uu___2 =
            ...
            FStarC_Util.format1 "Not a datacon: %s" uu___3 in
          failwith uu___2
...
let (visible_with :
  ...
  fun delta_levels ->
    fun quals ->
      FStarC_Util.for_some
        (fun dl -> FStarC_Util.for_some (visible_at dl) quals) delta_levels
    ...
    fun fv ->
      let f uu___ =
        ...
        | FStar_Pervasives_Native.Some attrs1 ->
            ...
            let uu___1 = FStarC_Util.map_opt res FStar_Pervasives_Native.fst in
            (true, uu___1) in
      ...
      fun lid0 ->
        ...
        | FStar_Pervasives_Native.Some
            ...
            then FStar_Pervasives_Native.None
              ...
              (let insts =
                 if
                   (FStarC_List.length univ_insts) =
                     (FStarC_List.length univs)
                 then univ_insts
                 else
                   (let uu___11 =
                      let uu___12 =
                   ...
                   let uu___14 =
                     let uu___15 =
                       FStarC_Class_Show.show FStarC_Ident.showable_lident
                         lid1 in
                     let uu___16 =
                       FStarC_Util.string_of_int (FStarC_List.length univs) in
                     FStarC_Util.format2
...
let (num_effect_indices :
  env -> FStarC_Ident.lident -> FStarC_Range_Type.range -> Prims.int) =
  fun env1 ->
    fun name ->
      fun r ->
        let sig_t =
            ...
            let uu___1 =
              ...
              let uu___3 =
                FStarC_Class_Show.show FStarC_Syntax_Print.showable_term
                  sig_t in
              FStarC_Util.format2 "Signature for %s not an arrow (%s)" uu___2
                uu___3 in
...
let (identity_mlift : mlift) =
  {
    ...
    mlift_term =
      (FStar_Pervasives_Native.Some
         (fun uu___ -> fun uu___1 -> fun e -> FStarC_Util.return_all e))
  }
let (join_opt :
  env ->
    FStarC_Ident.lident ->
      ...
      fun l2 ->
        ...
        | FStar_Pervasives_Native.None ->
            let uu___1 =
              ...
              FStarC_Util.format2 "Effects %s and %s cannot be composed"
                uu___2 uu___3 in
...
let effect_repr_aux :
  ...
  fun only_reifiable ->
    fun env1 ->
      fun c ->
        fun u_res ->
          let check_partial_application eff_name =
            fun args ->
              ...
              | (given, expected) ->
                  ...
                  then ()
                    ...
                    (let message =
                       ...
                       let uu___3 = FStarC_Util.string_of_int given in
                       let uu___4 = FStarC_Util.string_of_int expected in
                       FStarC_Util.format3
      ...
      fun u_c ->
        ...
        (let uu___1 =
         ...
         then
           let uu___2 =
             let uu___3 = FStarC_Ident.string_of_lid l in
             FStarC_Util.format1 "Effect %s cannot be reified" uu___3 in
           FStarC_Errors.raise_error hasRange_env env1
...
let (push_sigelt' : Prims.bool -> env -> FStarC_Syntax_Syntax.sigelt -> env)
  ...
  fun force ->
    fun env1 ->
      fun s ->
        let sb =
          let uu___ = FStarC_Syntax_Util.lids_of_sigelt s in (uu___, s) in
        let env2 =
          {
            solver = (env1.solver);
            range = (env1.range);
         ...
         | { msource = src; mtarget = tgt; mlift = uu___2; mpath = path;_} ->
             ...
             (match uu___3 with
              ...
              | FStar_Pervasives_Native.None ->
                  ...
                  FStarC_SMap.add m uu___4 uu___5)) (env1.effects).order;
    FStarC_List.iter
      (fun uu___2 ->
         match uu___2 with
         | (m, n, p, uu___3) ->
             let key =
               ...
               FStarC_Util.format3 "%s, %s |> %s" uu___4 uu___5 uu___6 in
      ...
      (fun uu___3 ->
         ...
         | (m, n, uu___4, uu___5) ->
             let key =
               ...
               FStarC_Util.format2 "%s <: %s" uu___6 uu___7 in
                  ...
                  FStarC_SMap.fold m
                    (fun tgt ->
                       fun path ->
                         fun s1 ->
                           let uu___5 =
                             FStarC_Util.format3 "%s -> %s [label=\"%s\"]"
                               src tgt path in
                           uu___5 :: s1) s) [] in
         ...
         FStarC_SMap.fold pbinds
           (fun k ->
              fun uu___6 ->
                fun s ->
                  let uu___7 =
                    FStarC_Util.format1 "\"%s\" [shape=\"plaintext\"]" k in
                  uu___7 :: s) [] in
       FStarC_String.concat "\n" uu___5 in
     let uu___5 =
       let uu___6 =
          ...
          let compose_edges e1 =
            fun e2 ->
                ...
                { mlift_wp; mlift_term } in
              {
                ...
                mlift = composed_lift;
                mpath =
                  (FStarC_List.op_At e1.mpath
                     (FStarC_List.op_At [e1.mtarget] e2.mpath))
              } in
          let edge1 =
                   ...
                   fun ik ->
                     fun jk ->
                       ...
                       let v =
                         ...
                         match uu___1 with
                         | FStar_Pervasives_Native.Some ubs1 ->
                             (i, j, k, ik, jk) :: ubs1
                         | FStar_Pervasives_Native.None ->
                             [(i, j, k, ik, jk)] in
                                 ...
                                 FStarC_List.for_all
                                   (fun uu___3 ->
                                      ...
                                      | (uu___4, uu___5, k', uu___6, uu___7)
                                          ...
                                          FStarC_Util.is_some uu___8) l) l in
...
let (push_bvs : env -> FStarC_Syntax_Syntax.bv Prims.list -> env) =
  fun env1 ->
    fun bvs ->
      FStarC_List.fold_left (fun env2 -> fun bv -> push_bv env2 bv) env1 bvs
let (pop_bv :
...
let (univ_vars : env -> FStarC_Syntax_Syntax.universe_uvar FStarC_FlatSet.t)
  =
  fun env1 ->
    let no_univs =
      Obj.magic
...
let (univnames : env -> FStarC_Syntax_Syntax.univ_name FStarC_FlatSet.t) =
  fun env1 ->
    let no_univ_names =
      Obj.magic
        (FStarC_Class_Setlike.empty ()
...
let (string_of_proof_ns : env -> Prims.string) =
  fun env1 ->
    ...
    let uu___ =
      let uu___1 = FStarC_List.map aux env1.proof_ns in
              ...
              FStarC_Syntax_Syntax.ctx_uvar_meta_t
                ...
                Prims.bool ->
                  (FStarC_Syntax_Syntax.term * (FStarC_Syntax_Syntax.ctx_uvar
                    * FStarC_Range_Type.range) * guard_t))
  =
  fun reason ->
    fun r ->
      fun env1 ->
...
let (get_letrec_arity :
  ...
  fun env1 ->
    fun lbname ->
      let compare_either f1 =
        fun f2 ->
          fun e1 ->
            fun e2 ->
              match (e1, e2) with
              | (FStar_Pervasives.Inl v1, FStar_Pervasives.Inl v2) ->
                  f1 v1 v2
              | (FStar_Pervasives.Inr v1, FStar_Pervasives.Inr v2) ->
                  f2 v1 v2
              | uu___ -> false in
      let uu___ =
        FStarC_Util.find_opt
