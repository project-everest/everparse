...
let (cur_state : ext_state FStarC_Effect.ref) =
  let uu___ = let uu___1 = FStarC_Util.psmap_empty () in E uu___1 in
...
let (reset : unit -> unit) =
  fun uu___ ->
    let uu___1 = let uu___2 = FStarC_Util.psmap_empty () in E uu___2 in
    FStarC_Effect.op_Colon_Equals cur_state uu___1