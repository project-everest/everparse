...
let (get_env : unit -> FStarC_TypeChecker_Env.env) =
  fun uu___ ->
    let uu___1 =
      FStarC_Effect.op_Bang FStarC_TypeChecker_Normalize.reflection_env_hook in
    match uu___1 with
  ...
  fun p ->
    ...
    | FStarC_Syntax_Syntax.Pat_var bv ->
        ...
        FStarC_Reflection_V2_Data.Pat_Var
          ((FStarC_Sealed.seal bv.FStarC_Syntax_Syntax.sort), uu___)
  ...
  fun c ->
    ...
    | FStarC_Syntax_Syntax.Comp ct ->
        let uopt =
          ...
          else FStarC_List.hd ct.FStarC_Syntax_Syntax.comp_univs in
        let uu___ =
...
let (pack_comp :
  ...
  fun cv ->
    ...
    | FStarC_Reflection_V2_Data.C_Eff (us, ef, res, args, decrs) ->
        ...
        let flags =
          if (FStarC_List.length decrs) = Prims.int_zero
          then []
          else
...
let rec (pack_pat :
  ...
  fun p ->
    ...
    | FStarC_Reflection_V2_Data.Pat_Var (sort, ppname) ->
        let bv =
          FStarC_Syntax_Syntax.gen_bv (FStarC_Sealed.unseal ppname)
            FStar_Pervasives_Native.None (FStarC_Sealed.unseal sort) in
    ...
    | FStarC_Reflection_V2_Data.Tv_Let (isrec, attrs, b, t1, t2) ->
        ...
        let lb =
          FStarC_Syntax_Util.mk_letbinding (FStar_Pervasives.Inl bv) []
            ...
            t1 attrs FStarC_Range_Type.dummyRange in
        FStarC_Syntax_Syntax.mk
          (FStarC_Syntax_Syntax.Tm_let
             {
               FStarC_Syntax_Syntax.lbs = (isrec, [lb]);
    ...
    | FStarC_Reflection_V2_Data.Tv_Unsupp ->
        (FStarC_Errors.log_issue0 FStarC_Errors_Codes.Warning_CantInspect ()
         ...
         FStarC_Syntax_Syntax.mk FStarC_Syntax_Syntax.Tm_unknown
           FStarC_Range_Type.dummyRange)
let (compare_bv :
  FStarC_Syntax_Syntax.bv -> FStarC_Syntax_Syntax.bv -> FStarC_Order.order) =
  fun x ->
    fun y ->
      let n = FStarC_Syntax_Syntax.order_bv x y in
      if n < Prims.int_zero
      then FStarC_Order.Lt
      else if n = Prims.int_zero then FStarC_Order.Eq else FStarC_Order.Gt
let (compare_namedv :
  FStarC_Reflection_V2_Data.namedv ->
    FStarC_Reflection_V2_Data.namedv -> FStarC_Order.order)
  =
  fun x ->
    fun y ->
      let n = FStarC_Syntax_Syntax.order_bv x y in
...
let (rd_to_syntax_qual :
  ...
  fun uu___ ->
    ...
    | FStarC_Reflection_V2_Data.Reflectable l ->
        let uu___1 = FStarC_Ident.lid_of_path l FStarC_Range_Type.dummyRange in
    ...
    | FStarC_Reflection_V2_Data.Projector (l, i) ->
        let uu___1 =
          let uu___2 =
            FStarC_Ident.lid_of_path l FStarC_Range_Type.dummyRange in
          (uu___2, i) in
  ...
  fun vcfg ->
    let uu___ =
      FStarC_Syntax_Embeddings_Base.embed FStarC_Syntax_Embeddings.e_vconfig
        vcfg in
    uu___ FStarC_Range_Type.dummyRange FStar_Pervasives_Native.None
...
let (inspect_sigelt :
  ...
  fun se ->
    ...
    | FStarC_Syntax_Syntax.Sig_inductive_typ
        ...
        let uu___3 =
          let uu___4 = FStarC_List.map inspect_ctor c_lids in
...
let (pack_sigelt :
  ...
  fun sv ->
    ...
    | FStarC_Reflection_V2_Data.Sg_Let (r, lbs) ->
        ...
        let lbs1 = FStarC_List.map FStar_Pervasives_Native.snd packed in
        let lids = FStarC_List.map FStar_Pervasives_Native.fst packed in
    ...
    | FStarC_Reflection_V2_Data.Sg_Inductive
        ...
        let ind_lid =
          FStarC_Ident.lid_of_path nm FStarC_Range_Type.dummyRange in
        (check_lid ind_lid;
         (let nparam = FStarC_List.length param_bs in
          ...
          let pack_ctor c =
            ...
            | (nm1, ty1) ->
                let lid =
                  FStarC_Ident.lid_of_path nm1 FStarC_Range_Type.dummyRange in
                FStarC_Syntax_Syntax.mk_sigelt
  ...
  fun bview ->
    ...
    | (pqual, attrs) ->
        let uu___1 =
          let uu___2 =
            FStarC_Ident.mk_ident
              ((FStarC_Sealed.unseal bview.FStarC_Reflection_V2_Data.ppname2),
                FStarC_Range_Type.dummyRange) in
          {
    ...
    FStarC_List.map
      (fun uu___1 ->
         ...
         | (l, m) ->
             ...
             FStarC_List.map FStarC_Ident.string_of_id uu___2) uu___
let (bv_to_binding :
...
let (vars_of_env :
  ...
  fun e ->
    ...
    FStarC_List.map (fun b -> bv_to_binding b.FStarC_Syntax_Syntax.binder_bv)
      uu___
let eqopt :
  'uuuuu .
    unit ->
