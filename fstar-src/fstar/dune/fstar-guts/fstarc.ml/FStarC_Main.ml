...
let (cleanup : unit -> unit) = fun uu___ -> FStarC_Util.kill_all ()
...
let (load_native_tactics : unit -> unit) =
  fun uu___ ->
          ...
          then
            ...
            (let uu___3 =
             ...
             | FStar_Pervasives_Native.Some ml ->
                 let dir = FStarC_Util.dirname ml in
    ...
    (let uu___2 = FStarC_Debug.any () in
     if uu___2
     then
       FStarC_Util.print1 "Will try to load cmxs files: [%s]\n"
         (FStarC_String.concat ", " cmxs_files)
     else ());
    FStarC_Plugins.load_plugins cmxs_files;
    (let uu___4 = FStarC_Options.use_native_tactics () in
     FStarC_Util.iter_opt uu___4 FStarC_Plugins.load_plugins_dir)
let (fstar_files :
         ...
         (match res with
          ...
          | FStarC_Getopt.Success when
              ...
              let path =
                let uu___3 = FStarC_Options.read_krml_file () in
                FStar_Pervasives_Native.__proj__Some__item__v uu___3 in
              let uu___3 = FStarC_Util.load_value_from_file path in
              (match uu___3 with
               | FStar_Pervasives_Native.None ->
                   let uu___4 =
                     let uu___5 =
              ...
              (check_no_filenames "--locate_file";
               (let f =
                ...
                | FStar_Pervasives_Native.Some fn ->
                    ((let uu___6 = FStarC_Util.normalize_file_path fn in
                      FStarC_Util.print1 "%s\n" uu___6);
                     FStarC_Effect.exit Prims.int_zero)))
          | FStarC_Getopt.Success when
          ...
          | FStarC_Getopt.Success ->
              (FStarC_Effect.op_Colon_Equals fstar_files
               ...
               (let uu___5 = FStarC_Debug.any () in
                if uu___5
                then
                  (FStarC_Util.print1 "- F* executable: %s\n"
                     FStarC_Util.exec_name;
                   (let uu___8 =
                      let uu___9 = FStarC_Find.lib_root () in
                      FStarC_Util.dflt "<none>" uu___9 in
  ...
  fun e ->
    ...
    (let uu___2 = FStarC_Options.trace_error () in
     ...
     then
       ...
       FStarC_Util.print2_error "Unexpected error\n%s\n%s\n" uu___3 uu___4
     else
       (let uu___4 =
          let uu___5 = FStarC_Errors.handleable e in Prims.op_Negation uu___5 in
        if uu___4
        then
          let uu___5 = FStarC_Util.message_of_exn e in
          FStarC_Util.print1_error
      ...
      (fun uu___1 ->
         ...
         | () ->
             (FStarC_Hooks.setup_hooks ();
              (let uu___3 = FStarC_Util.record_time_ms go in
               match uu___3 with
               | (uu___4, time) ->
