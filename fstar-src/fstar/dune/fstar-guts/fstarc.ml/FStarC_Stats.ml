...
type stat =
  ...
  ns_sub: Prims.int ;
  ncalls: Prims.int }
let (__proj__Mkstat__item__ns_tree : stat -> Prims.int) =
  fun projectee ->
    match projectee with | { ns_tree; ns_exn; ns_sub; ncalls;_} -> ns_tree
let (__proj__Mkstat__item__ns_exn : stat -> Prims.int) =
  fun projectee ->
    match projectee with | { ns_tree; ns_exn; ns_sub; ncalls;_} -> ns_exn
let (__proj__Mkstat__item__ns_sub : stat -> Prims.int) =
  fun projectee ->
    match projectee with | { ns_tree; ns_exn; ns_sub; ncalls;_} -> ns_sub
let (__proj__Mkstat__item__ncalls : stat -> Prims.int) =
  fun projectee ->
...
let (r_running : Prims.string -> Prims.bool FStarC_Effect.ref) =
  fun k ->
...
let (add : Prims.string -> stat -> unit) =
  fun k ->
    fun s1 ->
      let uu___ =
      ...
      | (r, s0) ->
          let uu___1 =
            let uu___2 = FStarC_Class_Monoid.mplus uu___0 s0 s1 in
            (r, uu___2) in
          FStarC_SMap.add st k uu___1
let do_record : 'a . Prims.string -> (unit -> 'a) -> 'a =
  fun key ->
    fun f ->
...
let record : 'a . Prims.string -> (unit -> 'a) -> 'a =
  fun key ->
    fun f ->
      let uu___ = FStarC_Effect.op_Bang enabled in
      if uu___ then do_record key f else f ()
let (lpad : Prims.int -> Prims.string -> Prims.string) =
  fun len ->
    fun s ->
      ...
      then s
        ...
        (let uu___1 = FStarC_String.make (len - l) 32 in
         Prims.strcat uu___1 s)
let (max : Prims.int -> Prims.int -> Prims.int) =
  fun x -> fun y -> if x > y then x else y
let (print_all : unit -> Prims.string) =
  fun uu___ ->
    ...
    let longest_key =
      FStarC_List.fold_left
        (fun acc ->
           fun uu___1 ->
             match uu___1 with
             | (k, uu___2) -> max acc (FStarC_String.length k))
    ...
    let pr1 p =
      ...
      | (k, st1) ->
          ...
          let uu___3 =
            lpad (Prims.of_int (8)) (Prims.string_of_int st1.ncalls) in
          let uu___4 =
            lpad (Prims.of_int (6))
              (Prims.string_of_int
          ...
          let uu___6 =
            lpad (Prims.of_int (6))
              (Prims.string_of_int (st1.ns_exn / (Prims.parse_int "1000000"))) in
          FStarC_Util.format5 "  %s  %s %s ms %s ms %s ms" uu___2 uu___3
    ...
    let uu___1 =
      ...
      FStarC_Util.format5 "  %s  %s %s %s %s" uu___2 uu___3 uu___4 uu___5
        uu___6 in
    let uu___2 =
      let uu___3 =
