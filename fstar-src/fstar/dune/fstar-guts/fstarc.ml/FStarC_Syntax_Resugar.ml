...
let map_opt :
  ...
  = fun uu___ -> FStarC_List.filter_map
let (bv_as_unique_ident : FStarC_Syntax_Syntax.bv -> FStarC_Ident.ident) =
  fun x ->
    let unique_name =
      ...
      then
        let uu___1 = FStarC_Ident.string_of_id x.FStarC_Syntax_Syntax.ppname in
        let uu___2 = FStarC_Util.string_of_int x.FStarC_Syntax_Syntax.index in
...
let (universe_to_string : FStarC_Ident.ident Prims.list -> Prims.string) =
  fun univs ->
    ...
    then
      let uu___1 =
        FStarC_List.map (fun x -> FStarC_Ident.string_of_id x) univs in
      FStarC_String.concat ", " uu___1
    else ""
let rec (resugar_universe :
  FStarC_Syntax_Syntax.universe ->
    FStarC_Range_Type.range -> FStarC_Parser_AST.term)
  =
  fun u ->
    fun r ->
      let mk a =
        fun r1 -> FStarC_Parser_AST.mk_term a r1 FStarC_Parser_AST.Un in
                ...
                | uu___2 ->
                    let e1 =
                      let uu___3 =
                        let uu___4 =
                          let uu___5 =
                            let uu___6 = FStarC_Util.string_of_int n in
  ...
  fun uu___ ->
    ...
    | (s, w) ->
        ...
        let su =
          ...
          | FStarC_Const.Signed -> "" in
        let uu___1 =
          FStarC_Util.format3 "FStar.%sInt%s.__%sint_to_t" sU sW su in
        let uu___2 =
          let uu___3 =
            FStarC_Util.format3 "FStar.%sInt%s.%sint_to_t" sU sW su in
          [uu___3] in
        uu___1 :: uu___2
...
let (can_resugar_machine_integer_fv : FStarC_Syntax_Syntax.fv -> Prims.bool)
  =
  fun fv ->
    let uu___ = parse_machine_integer_desc fv in FStarC_Option.isSome uu___
let (resugar_machine_integer :
  FStarC_Syntax_Syntax.fv ->
    Prims.string -> FStarC_Range_Type.range -> FStarC_Parser_AST.term)
  =
  fun fv ->
    fun i ->
      fun pos ->
                   ...
                   | {
                       ...
                       FStarC_Parser_AST.mk_term
                         (FStarC_Parser_AST.Construct
                            (hd, (FStarC_List.op_At previous_args args2))) r
                         l
                   | e2 ->
                       FStarC_List.fold_left
                         (fun acc ->
                            fun uu___4 ->
                              match uu___4 with
                              | (x, qual) ->
                                  mk (FStarC_Parser_AST.App (acc, x, qual)))
                         e2 args2 in
               let args1 = filter_imp_args args in
               let is_projector t2 =
                 let uu___3 =
                                       ...
                                       fun t2 ->
                                         fun flavor_matches ->
                                           ...
                                           | FStarC_Parser_AST.QExists
                                               (xs', (uu___11, pats'), body)
                                               when flavor_matches t2 ->
                                               uncurry
                                                 (FStarC_List.op_At xs xs')
  ...
  fun env ->
    fun uu___ ->
      ...
      | (asc, tac_opt, b) ->
          ...
          let uu___2 = FStarC_Util.map_opt tac_opt (resugar_term' env) in
          (uu___1, uu___2, b)
and (resugar_calc :
  FStarC_Syntax_DsEnv.env ->
    FStarC_Syntax_Syntax.term ->
                      ...
                      (match uu___3 with
                       | FStarC_Syntax_Syntax.Tm_app
                           { FStarC_Syntax_Syntax.hd = e;
                             FStarC_Syntax_Syntax.args = args;_}
                           when
                           (FStarC_List.length args) >= (Prims.of_int (2)) ->
...
and (resugar_binder' :
  FStarC_Syntax_DsEnv.env ->
    FStarC_Syntax_Syntax.binder ->
      FStarC_Range_Type.range -> FStarC_Parser_AST.binder)
  =
  fun env ->
    fun b ->
      fun r ->
...
and (resugar_bv_as_pat :
  FStarC_Syntax_DsEnv.env ->
    FStarC_Syntax_Syntax.bv ->
      FStarC_Syntax_Syntax.binder_qualifier FStar_Pervasives_Native.option ->
        FStarC_Syntax_Syntax.bv FStarC_FlatSet.t -> FStarC_Parser_AST.pattern)
  =
  fun env ->
    fun x ->
      fun qual ->
      ...
      fun branch_bv ->
        ...
        let resugar_plain_pat_cons' fv =
          fun args ->
            ...
            mk uu___ in
        let rec resugar_plain_pat_cons fv =
          fun args ->
            let args1 =
              let uu___ =
            ...
            let args2 =
              FStarC_List.map
                (fun uu___ ->
                   match uu___ with
                   | (p1, b) -> aux p1 (FStar_Pervasives_Native.Some b))
                args1 in
        ...
        and aux p1 =
          fun imp_opt ->
            ...
            | FStarC_Syntax_Syntax.Pat_cons (fv, uu___, args) when
                ...
                (match uu___1 with
                 | (hd, false)::(tl, false)::[] ->
                     ...
                     (match uu___2 with
                      | {
                          FStarC_Parser_AST.pat = FStarC_Parser_AST.PatList
                            tl';
            ...
            | FStarC_Syntax_Syntax.Pat_cons
                ...
                let fields1 =
                  let uu___2 =
                    FStarC_List.map (fun f -> FStarC_Ident.lid_of_ids [f])
                      fields in
                ...
                let args2 =
                  let uu___2 = map2 fields1 args1 in FStarC_List.rev uu___2 in
                 ...
                 let tyc =
                   ...
                   then
                     ...
                     (let resugar_datacon constructors =
                      ...
                      let constructors =
                        FStarC_List.fold_left resugar_datacon []
                          current_datacons in
                      let uu___6 =
              ...
              (match uu___1 with
               | (bs1, action_typ) ->
                   ...
                   then
                     ...
                     let t =
                       FStarC_Parser_AST.mk_term a r FStarC_Parser_AST.Un in
                     let uu___2 =
                       let uu___3 =
                         let uu___4 =
                           let uu___5 =
        ...
        | FStarC_Syntax_Syntax.Sig_bundle
            ...
            (match uu___1 with
             | (decl_typ_ses, datacon_ses) ->
                 let retrieve_datacons_and_resugar uu___2 =
                   fun se1 ->
                     match uu___2 with
                     | (datacon_ses1, tycons) ->
        ...
        | FStarC_Syntax_Syntax.Sig_splice
            ...
            let uu___ =
              let uu___1 =
                let uu___2 =
                  let uu___3 =
                    FStarC_List.map (fun l -> FStarC_Ident.ident_of_lid l)
                      ids in
                  let uu___4 = resugar_term' env t in
