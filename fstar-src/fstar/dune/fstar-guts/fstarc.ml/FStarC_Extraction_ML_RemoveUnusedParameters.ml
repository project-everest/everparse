  ...
  fun env ->
    fun mlty ->
      ...
      | FStarC_Extraction_ML_Syntax.MLTY_Named (args, name) ->
          let args1 = FStarC_List.map (elim_mlty env) args in
    ...
    fun e ->
      ...
      | FStarC_Extraction_ML_Syntax.MLE_App (e1, es) ->
          let uu___ =
            ...
            let uu___2 = FStarC_List.map (elim_mlexpr env) es in
            (uu___1, uu___2) in
          FStarC_Extraction_ML_Syntax.MLE_App uu___
      | FStarC_Extraction_ML_Syntax.MLE_TApp (e1, tys) ->
          let uu___ =
            let uu___1 = FStarC_List.map (elim_mlty env) tys in (e1, uu___1) in
            ...
            let uu___1 =
              FStarC_List.map
                (fun b ->
                   ...
                   let uu___3 =
                     FStarC_List.map (elim_mlexpr env)
      ...
      | FStarC_Extraction_ML_Syntax.MLE_Match (e1, branches) ->
          let uu___ =
            let uu___1 = elim_mlexpr env e1 in
            let uu___2 = FStarC_List.map (elim_branch env) branches in
            (uu___1, uu___2) in
      ...
      | FStarC_Extraction_ML_Syntax.MLE_CTor (l, es) ->
          let uu___ =
            let uu___1 = FStarC_List.map (elim_mlexpr env) es in (l, uu___1) in
          FStarC_Extraction_ML_Syntax.MLE_CTor uu___
      | FStarC_Extraction_ML_Syntax.MLE_Seq es ->
          let uu___ = FStarC_List.map (elim_mlexpr env) es in
      ...
      | FStarC_Extraction_ML_Syntax.MLE_Raise (p, es) ->
          let uu___ =
            let uu___1 = FStarC_List.map (elim_mlexpr env) es in (p, uu___1) in
        ...
        fun parameters ->
          fun mlty ->
                              ...
                              then
                                ...
                                ((let uu___7 =
                                  ...
                                  FStarC_Errors.log_issue
                                    ...
                                    (Obj.magic uu___7));
                                 FStarC_Effect.raise Drop_tydef)
                              else
            ...
            | (uu___1, parameters1, entry1) ->
                let uu___2 = extend_env env name (FStarC_List.rev entry1) in
                (uu___2,
                  (name, metadata, (FStarC_List.rev parameters1), mlty1))
      ...
      | (name, metadata, FStar_Pervasives.Inr arity) ->
          ...
          (match remove_typars_list with
           ...
           | FStar_Pervasives_Native.Some
               (FStarC_Extraction_ML_Syntax.RemoveUnusedTypeParameters
               (l, r)) ->
               let must_eliminate i = FStarC_List.contains i l in
               let rec aux i =
                 if i = arity
                 then []
                 else
    ...
    fun tds ->
      ...
      then (env, tds)
        ...
        (let uu___2 =
         ...
         match uu___2 with | (env1, tds1) -> (env1, (FStarC_List.rev tds1)))
...
let (elim_modulebody :
  ...
  fun env ->
    fun m ->
      ...
      match uu___ with | (env1, m1) -> (env1, (FStarC_List.rev m1))
let (set_current_module :
  env_t -> FStarC_Extraction_ML_Syntax.mlpath -> env_t) =
  fun e ->
    fun n ->
      let curmod =
        FStarC_List.op_At (FStar_Pervasives_Native.fst n)
