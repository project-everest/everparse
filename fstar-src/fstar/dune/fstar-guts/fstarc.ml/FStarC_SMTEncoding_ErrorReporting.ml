...
let (remove_dups :
  labels ->
    (FStarC_SMTEncoding_Term.fv * FStarC_Errors_Msg.error_message *
      FStarC_Range_Type.range) Prims.list)
  =
  fun l ->
    FStarC_Util.remove_dups
      ...
      l
type msg = (Prims.string * FStarC_Range_Type.range)
type ranges =
...
let (fresh_label :
  ...
  fun message ->
    fun range ->
      fun t ->
        let l =
          FStarC_Util.incr __ctr;
          (let uu___1 =
             let uu___2 = FStarC_Effect.op_Bang __ctr in
             FStarC_Util.string_of_int uu___2 in
           FStarC_Util.format1 "label_%s" uu___1) in
        let lvar =
...
let (label_goals :
  ...
  fun use_env_msg ->
    fun r ->
      fun q ->
        let rec is_a_post_condition post_name_opt =
          fun tm ->
        ...
        | (flag, msg_prefix) ->
            ...
            let rec aux default_msg =
              fun ropt ->
                fun post_name_opt ->
                  fun labels1 ->
                    fun q1 ->
                      match q1.FStarC_SMTEncoding_Term.tm with
                      | FStarC_SMTEncoding_Term.BoundV uu___1 ->
                          (labels1, q1)
                      | FStarC_SMTEncoding_Term.Integer uu___1 ->
                          (labels1, q1)
                      ...
                      | FStarC_SMTEncoding_Term.Labeled
                          ...
                          (try
                             (fun uu___1 ->
                                match () with
                                | () ->
                                    (match arg.FStarC_SMTEncoding_Term.tm
                                     with
                                     | FStarC_SMTEncoding_Term.Quant
                                         (FStarC_SMTEncoding_Term.Forall,
                      ...
                      | FStarC_SMTEncoding_Term.Labeled (arg, reason, r1) ->
                          aux reason (FStar_Pervasives_Native.Some r1)
                            post_name_opt labels1 arg
                      | FStarC_SMTEncoding_Term.Quant
                          (FStarC_SMTEncoding_Term.Forall, [],
                          ...
                          (match uu___2 with
                           | (sorts', post) ->
                               ...
                               let uu___3 =
                                 let uu___4 =
                                   FStarC_SMTEncoding_Term.inst instantiation
                                     lhs in
                                 let uu___5 =
                                   FStarC_SMTEncoding_Term.inst instantiation
                                     rhs in
                                ...
                                | (lhs1, rhs1) ->
                                    let uu___4 =
                                      FStarC_Util.fold_map
                                        (fun labels2 ->
                                           fun tm ->
                                             match tm.FStarC_SMTEncoding_Term.tm
                                             with
                                             | FStarC_SMTEncoding_Term.Quant
                                                 (FStarC_SMTEncoding_Term.Forall,
                                                  ({
                      ...
                      | FStarC_SMTEncoding_Term.App
                          ...
                          (match uu___1 with
                           | (labels2, rhs1) ->
                               let uu___2 =
                                 FStarC_SMTEncoding_Util.mkImp (lhs, rhs1) in
                               (labels2, uu___2))
                      | FStarC_SMTEncoding_Term.App
                          (FStarC_SMTEncoding_Term.And, conjuncts1) ->
                          let uu___1 =
                            FStarC_Util.fold_map
                              (aux default_msg ropt post_name_opt) labels1
                              conjuncts1 in
                          (match uu___1 with
                           | (labels2, conjuncts2) ->
                               let uu___2 =
                      ...
                      | FStarC_SMTEncoding_Term.App
                          ...
                          (match uu___1 with
                           | (labels2, q12) ->
                               let uu___2 =
                                 aux default_msg ropt post_name_opt labels2
                                   q2 in
                               (match uu___2 with
                                | (labels3, q21) ->
                                    let uu___3 =
                                    ...
                                    (labels3, uu___3)))
                      | FStarC_SMTEncoding_Term.Quant
                          (FStarC_SMTEncoding_Term.Exists, uu___1, uu___2,
                           uu___3, uu___4)
                          ->
                          let uu___5 =
                      ...
                      | FStarC_SMTEncoding_Term.App
                          ...
                          is_a_post_condition post_name_opt q1 ->
                          (labels1, q1)
                          ...
                          (match uu___1 with
                           | (labels2, body1) ->
                               let uu___2 =
                                 FStarC_SMTEncoding_Term.mk
                                   (FStarC_SMTEncoding_Term.Quant
                                      (FStarC_SMTEncoding_Term.Forall, pats,
                                        iopt, sorts, body1))
                                   q1.FStarC_SMTEncoding_Term.rng in
                               (labels2, uu___2))
        ...
        fun askZ3 ->
          let print_banner uu___ =
            let msg1 =
              ...
              let uu___2 = FStarC_Util.string_of_int (Prims.of_int (5)) in
              let uu___3 =
                FStarC_Util.string_of_int (FStarC_List.length all_labels) in
              FStarC_Util.format4
                "Detailed %s report follows for %s\nTaking %s seconds per proof obligation (%s proofs in total)\n"
                (if hint_replay then "hint replay" else "error") uu___1
                uu___2 uu___3 in
            FStarC_Util.print_error msg1 in
          ...
          let rec linear_check eliminated =
            fun errors ->
              fun active ->
                ...
                (match active with
                 | [] ->
                     let results =
                       let uu___1 =
                         FStarC_List.map (fun x -> (x, true)) eliminated in
                       let uu___2 =
                         FStarC_List.map (fun x -> (x, false)) errors in
                       FStarC_List.op_At uu___1 uu___2 in
                     sort_labels results
                 | hd::tl ->
                     ((let uu___2 =
                         FStarC_Util.string_of_int
                           (FStarC_List.length active) in
                       FStarC_Util.print1 "%s, " uu___2);
                      (let decls =
                         elim
                           (FStarC_List.op_At eliminated
                              (FStarC_List.op_At errors tl)) in
                       let result = askZ3 decls in
                       match result.FStarC_SMTEncoding_Z3.z3result_status
                       with
          ...
          (let res = linear_check [] [] all_labels in
           ...
           (let uu___4 = FStarC_Util.for_all FStar_Pervasives_Native.snd res in
            if uu___4
            then
              FStarC_Util.print_string
