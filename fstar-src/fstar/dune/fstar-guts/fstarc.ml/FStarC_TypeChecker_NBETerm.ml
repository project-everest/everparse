...
type constant =
  ...
  | String of (Prims.string * FStarC_Range_Type.range) 
...
let (__proj__String__item___0 :
  constant -> (Prims.string * FStarC_Range_Type.range)) =
...
let (__proj__Range__item___0 : constant -> FStarC_Range_Type.range) =
...
let (__proj__Mkt__item__nbe_r : t -> FStarC_Range_Type.range) =
...
let (mk_rt : FStarC_Range_Type.range -> t' -> t) =
  fun r -> fun t1 -> { nbe_t = t1; nbe_r = r }
let (mk_t : t' -> t) = fun t1 -> mk_rt FStarC_Range_Type.dummyRange t1
...
let (constant_to_string : constant -> Prims.string) =
  fun c ->
    ...
    | Char c1 -> FStarC_Util.format1 "'%s'" (FStarC_Util.string_of_char c1)
    | String (s, uu___) -> FStarC_Util.format1 "\"%s\"" s
    | Range r ->
        let uu___ = FStarC_Range_Ops.string_of_range r in
        FStarC_Util.format1 "Range %s" uu___
    | SConst s -> FStarC_Class_Show.show FStarC_Syntax_Print.showable_const s
    | Real s -> FStarC_Util.format1 "Real %s" s
let rec (t_to_string : t -> Prims.string) =
  fun x ->
    match x.nbe_t with
    | Lam { interp = b; shape = uu___; arity;_} ->
        let uu___1 = FStarC_Util.string_of_int arity in
        FStarC_Util.format1 "Lam (_, %s args)" uu___1
    | Accu (a, l) ->
        let uu___ =
    ...
    | TopLevelLet (lb, uu___, uu___1) ->
        let uu___2 =
          let uu___3 =
            let uu___4 = FStarC_Util.right lb.FStarC_Syntax_Syntax.lbname in
    ...
    | TopLevelRec (lb, uu___, uu___1, uu___2) ->
        let uu___3 =
          let uu___4 =
            let uu___5 = FStarC_Util.right lb.FStarC_Syntax_Syntax.lbname in
              ...
              (let a1 = FStarC_Dyn.undyn b in
               (let uu___3 =
                ...
                then
                  ...
                  FStarC_Util.print1 "Unembed cancelled for %s\n" uu___4
                else ());
            ...
            ((let uu___2 =
              ...
              then
                let uu___3 =
                  ...
                  FStarC_Class_Show.show
                    FStarC_Syntax_Syntax.showable_emb_typ uu___4 in
                FStarC_Util.print1 "Unembedding:\n\temb_typ=%s\n" uu___3
              else ());
             aopt)
...
let e_sealed : 'a . 'a embedding -> 'a FStarC_Sealed.sealed embedding =
  fun ea ->
    let etyp uu___ =
      let uu___1 =
        let uu___2 =
