  ...
  fun st ->
    fun search_term ->
      fun is_reset ->
        ...
        | ("--", trimmed_name) ->
            let matcher opt =
              FStarC_Util.starts_with opt.opt_name trimmed_name in
...
let (sc_fvars :
  FStarC_TypeChecker_Env.env ->
    search_candidate -> FStarC_Ident.lident FStarC_RBSet.t)
  =
  fun tcenv ->
    fun sc ->
        ...
        try
          (fun uu___ ->
             ...
             | () ->
                 ...
                 let js =
                   FStarC_List.map (json_of_search_result tcenv) sorted in
                 (match results1 with
                  | [] ->
                      let kwds =
...
let (interactive_error_handler : FStarC_Errors.error_handler) =
  ...
  let add_one e =
    let uu___ = let uu___1 = FStarC_Effect.op_Bang issues in e :: uu___1 in
...
let interactive_mode' :
  ...
  fun init_st ->
    ...
    (let exit_code =
       ...
       then
         let uu___2 =
           let uu___3 = FStarC_Options.file_list () in FStarC_List.hd uu___3 in
