...
let __proj__Mkproblem__item__loc : 'a . 'a problem -> FStarC_Range_Type.range
  =
  fun projectee ->
    match projectee with
    | { pid; lhs; relation; rhs; element; logical_guard; logical_guard_uvar;
...
let (__proj__Mkidentifier_info__item__identifier_range :
  identifier_info -> FStarC_Range_Type.range) =
  fun projectee ->
    match projectee with
      ...
      fun info ->
        ...
        | FStar_Pervasives_Native.Some id_ty1 ->
            ...
            let uu___ =
              ...
              let uu___2 = FStarC_Range_Ops.col_of_pos start in
              (uu___1, uu___2) in
            (match uu___ with
             | (row, col) ->
                 let rows =
...
let (id_info_insert :
  id_info_table ->
    (FStarC_Syntax_Syntax.bv, FStarC_Syntax_Syntax.fv)
      FStar_Pervasives.either ->
      FStarC_Syntax_Syntax.typ -> FStarC_Range_Type.range -> id_info_table)
  =
  fun table ->
    fun id ->
      fun ty ->
...
let (__proj__Mkimplicit__item__imp_range :
  implicit -> FStarC_Range_Type.range) =
  fun projectee ->
    match projectee with
...
let (conj_guards : guard_t Prims.list -> guard_t) =
  fun gs -> FStarC_List.fold_left conj_guard trivial_guard gs
