...
BOOLEAN BoundedSumConstCheckBoundedSum(uint8_t *base, uint32_t len) {
	...
	uint64_t result;
	frame.filled = FALSE;
	result = BoundedSumConstValidateBoundedSum( (uint8_t*)&frame, &DefaultErrorHandler, base, len, 0);
	if (EverParseIsError(result))
	{
		if (frame.filled)
		{
			BoundedSumConstEverParseError(frame.typename_s, frame.fieldname, frame.reason);
		}
		return FALSE;
	}
	return TRUE;
