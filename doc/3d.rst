...
2. Give MyFile.3d to EverParse:

   * with the Windows binary package: ``everparse.bat MyFile.3d``

   * with the Linux binary package: ``everparse.sh MyFile.3d``

   * with the sources: ``src/3d/3d --batch MyFile.3d``

Then, EverParse/3d produces:

* ``MyFile.fsti``, ``MyFile.fst``: the verified F* specification and
  Low* implementation of parsers and validators for your data format
...
not require any POSIX emulation layer such as Cygwin (contrary to the
sources) or WSL.
...
.. note::
   ...
   If, along with some ``MyFile.3d``, you provide a file
   ``MyFile.3d.copyright.txt`` that contains syntactically correct C
   comments (with ``//`` or ``/* ... */``), then EverParse will
   prepend ``MyFile.c``, ``MyFileWrapper.c`` and their corresponding
   ``.h`` with these comments.
