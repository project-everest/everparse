...
: Lemma
  ( 
    perm_gsub (perm_gsub p offset1 length1) offset2 length2 == LP.coerce (perm (B.gsub (B.gsub b offset1 length1) offset2 length2)) (perm_gsub p (offset1 `U32.add` offset2) length2)
  )
...
: Lemma
  (
    ...
    perm_gsub p 0ul (B.len b) == LP.coerce (perm (B.gsub b 0ul (B.len b))) p
  )
...
: HST.Stack t
  ...
  (ensures (fun h res h' ->
    B.modifies (loc_perm_from_to slp pos (LP.get_valid_pos p h sl pos)) h h' /\
