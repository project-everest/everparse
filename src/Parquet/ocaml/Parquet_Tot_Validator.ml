...
let (__proj__Mkfile_crypto_meta_data__item__key_metadata :
  ...
  fun projectee ->
    ...
    | { encryption_algorithm1; key_metadata1 = key_metadata;_} ->
        key_metadata
let (check : Prims.bool -> Prims.string -> unit) =
  fun b -> fun msg -> if Prims.op_Negation b then failwith msg else ()
...
let rec (ranges_in_bound : range Prims.list -> Prims.int -> unit) =
  fun rs ->
    fun max_len ->
      match rs with
      | [] -> ()
      | r::rest ->
          ...
          (check (end_ <= max_len)
             (Prims.strcat
                (Prims.strcat "Range end "
                   (Prims.strcat (Prims.string_of_int end_) " exceeds bound "))
                (Prims.strcat (Prims.string_of_int max_len) ""));
           ranges_in_bound rest max_len)
let rec (ranges_disjoint : range Prims.list -> unit) =
  fun rs ->
    match rs with
    | [] -> ()
    | uu___::[] -> ()
    | x::xs ->
        (check (FStar_List_Tot_Base.for_all (disjoint x) xs)
           (Prims.strcat
              (Prims.strcat "Ranges "
                 (Prims.strcat (Prims.string_of_int x.start) "-"))
              (Prims.strcat (Prims.string_of_int (range_end x))
...
let (validate_column_chunk : column_chunk -> unit) =
  fun cc ->
    let offsets_ok =
          ...
          (match ((cmd.dictionary_page_offset), (cmd.index_page_offset)) with
           ...
           | (FStar_Pervasives_Native.Some dict_off,
              FStar_Pervasives_Native.None) ->
               (check ((FStar_Int64.v dict_off) < data_off)
                  (Prims.strcat
                     (Prims.strcat
                        "Dictionary page offset must be less than data page offset, got "
                        (Prims.strcat
                           (Prims.string_of_int (FStar_Int64.v dict_off))
                           " >= "))
                     (Prims.strcat (Prims.string_of_int data_off) ""));
                  ...
                  (Prims.strcat
                     (Prims.strcat
                        ...
                        (Prims.strcat
                           ...
                           " >= "))
                     (Prims.strcat (Prims.string_of_int data_off) ""));
                check ((FStar_Int64.v dict_off) < (FStar_Int64.v idx_off))
                  (Prims.strcat
                     (Prims.strcat
                        "Dictionary page offset must be less than index page offset, got "
                        (Prims.strcat
                           (Prims.string_of_int (FStar_Int64.v dict_off))
                           " >= "))
                     (Prims.strcat
                        (Prims.string_of_int (FStar_Int64.v idx_off)) ""));
                ((FStar_Int64.v dict_off) < data_off) &&
    ...
    let idx_ok =
      ...
      | (FStar_Pervasives_Native.None, FStar_Pervasives_Native.None,
         FStar_Pervasives_Native.Some uu___, uu___1) ->
          (check false "ColumnIndex is present without OffsetIndex"; false)
      | (FStar_Pervasives_Native.None, FStar_Pervasives_Native.None, uu___,
         FStar_Pervasives_Native.Some uu___1) ->
          (check false "ColumnIndex is present without OffsetIndex"; false)
      | (FStar_Pervasives_Native.Some uu___, FStar_Pervasives_Native.None,
         uu___1, uu___2) ->
          (check false "OffsetIndex is present without length"; false)
      | (FStar_Pervasives_Native.None, FStar_Pervasives_Native.Some uu___,
         uu___1, uu___2) ->
          (check false "OffsetIndex length is present without offset"; false)
      | (uu___, uu___1, FStar_Pervasives_Native.Some uu___2,
         FStar_Pervasives_Native.None) ->
          (check false "ColumnIndex offset is present without length"; false)
      | (uu___, uu___1, FStar_Pervasives_Native.None,
         FStar_Pervasives_Native.Some uu___2) ->
          (check false "ColumnIndex length is present without offset"; false) in
    ()
type schema_tree =
...
let rec (columns_are_contiguous :
  column_chunk -> column_chunk Prims.list -> unit) =
  fun prev ->
    fun cols ->
      match cols with
      | [] -> ()
          ...
          (match ((prev.meta_data), (cc.meta_data)) with
           | (FStar_Pervasives_Native.Some prev_md,
              FStar_Pervasives_Native.Some cc_md) ->
               ...
               (check ((prev_offset + prev_size) = cc_offset)
                  (Prims.strcat
                     (Prims.strcat
                        (Prims.strcat
                           "Non-contiguous column chunks: previous offset "
                           (Prims.strcat (Prims.string_of_int prev_offset)
                              " + size "))
                        (Prims.strcat (Prims.string_of_int prev_size)
                           " != current offset "))
                     (Prims.strcat (Prims.string_of_int cc_offset) ""));
                columns_are_contiguous cc rest)
           | uu___ -> ())
let (validate_row_group : row_group -> unit) =
  fun rg ->
    let col_offset =
      ...
      | FStar_Pervasives_Native.Some off ->
          (match col_offset with
           ...
           | FStar_Pervasives_Native.Some col_off ->
               (check ((FStar_Int64.v off) = (FStar_Int64.v col_off))
                  (Prims.strcat
                     (Prims.strcat "Row group file offset "
                        (Prims.strcat
                           (Prims.string_of_int (FStar_Int64.v off))
                           " does not match column chunk offset "))
                     (Prims.strcat
                        (Prims.string_of_int (FStar_Int64.v col_off)) ""));
      ...
      | FStar_Pervasives_Native.Some sz ->
          ...
          (match total_size with
           ...
           | FStar_Pervasives_Native.Some total_sz ->
               (check ((FStar_Int64.v sz) = total_sz)
                  (Prims.strcat
                     (Prims.strcat "Row group total compressed size "
                        (Prims.strcat
                           (Prims.string_of_int (FStar_Int64.v sz))
                           " does not match sum of column chunk sizes "))
                     (Prims.strcat (Prims.string_of_int total_sz) ""));
                total_sz = (FStar_Int64.v sz))) in
    (match rg.columns with
     | [] -> ()
     | first::rest -> columns_are_contiguous first rest);
    FStar_List.iter (fun cc -> validate_column_chunk cc) rg.columns
...
let rec (page_offsets_are_distinct_and_inbound :
  int64 Prims.list -> int64 -> unit) =
  fun page_offs ->
    fun bound ->
      match page_offs with
      | [] -> ()
      | x::xs ->
          (check (Prims.op_Negation (FStar_List_Tot_Base.contains x xs))
             (Prims.strcat "Duplicate page offset found: "
                (Prims.strcat (Prims.string_of_int (FStar_Int64.v x)) ""));
           check ((FStar_Int64.v x) < (FStar_Int64.v bound))
             (Prims.strcat
                (Prims.strcat "Page offset "
                   (Prims.strcat (Prims.string_of_int (FStar_Int64.v x))
                      " is out of bounds, must be less than "))
                (Prims.strcat (Prims.string_of_int (FStar_Int64.v bound)) ""));
           page_offsets_are_distinct_and_inbound xs bound)
let (validate_file_meta_data : file_meta_data -> int64 -> unit) =
  fun fmd ->
    fun footer_start ->
      ...
      FStar_List.iter (fun rg -> validate_row_group rg) fmd.row_groups
let rec (page_offsets_are_contiguous :
  page_location -> page_location Prims.list -> unit) =
  fun prev ->
    fun locs ->
      match locs with
      | [] -> ()
      | loc::rest ->
          (check
             ...
             (Prims.strcat
                (Prims.strcat
                   (Prims.strcat "Page offset is not contiguous: "
                      (Prims.strcat
                         (Prims.string_of_int (FStar_Int64.v loc.offset))
                         " != "))
                   (Prims.strcat
                      (Prims.string_of_int (FStar_Int64.v prev.offset)) " + "))
                (Prims.strcat
                   (Prims.string_of_int
                      (FStar_Int32.v prev.compressed_page_size1)) ""));
           page_offsets_are_contiguous loc rest)
let (validate_offset_index : offset_index -> column_chunk -> unit) =
  fun oi ->
    fun cc ->
      ...
      let first_loc_ok =
        ...
        | (FStar_Pervasives_Native.Some loc, FStar_Pervasives_Native.Some
           off) ->
            (check ((FStar_Int64.v loc.offset) = off)
               (Prims.strcat
                  (Prims.strcat "First page location offset "
                     (Prims.strcat
                        (Prims.string_of_int (FStar_Int64.v loc.offset))
                        " does not match column chunk offset "))
                  (Prims.strcat (Prims.string_of_int off) ""));
             (FStar_Int64.v loc.offset) = off)
        | uu___ -> true in
      let all_page_offsets =
        FStar_List_Tot_Base.map (fun pl -> pl.offset) locs in
      let contains off = FStar_List_Tot_Base.contains off all_page_offsets in
      (match cc.meta_data with
       | FStar_Pervasives_Native.None -> ()
       | FStar_Pervasives_Native.Some cmd ->
           ((match ((cmd.dictionary_page_offset), (cmd.index_page_offset))
             ...
             | (FStar_Pervasives_Native.Some dict_off,
                FStar_Pervasives_Native.Some idx_off) ->
                 (check (contains dict_off)
                    (Prims.strcat "Dictionary page offset "
                       (Prims.strcat
                          (Prims.string_of_int (FStar_Int64.v dict_off))
                          " is not listed in page locations"));
                  check (contains idx_off)
                    (Prims.strcat "Index page offset "
                       (Prims.strcat
                          (Prims.string_of_int (FStar_Int64.v idx_off))
                          " is not listed in page locations")))
             | (FStar_Pervasives_Native.Some dict_off,
             ...
             | (FStar_Pervasives_Native.None, FStar_Pervasives_Native.None)
                 -> ());
            check (contains cmd.data_page_offset)
              (Prims.strcat "Data page offset "
                 (Prims.strcat
                    (Prims.string_of_int (FStar_Int64.v cmd.data_page_offset))
                    " is not listed in page locations"))));
      (let col_size_ok =
         match cc.meta_data with
         | FStar_Pervasives_Native.None -> true
         | FStar_Pervasives_Native.Some md ->
             ...
             (check ((FStar_Int64.v md.total_compressed_size) = s)
                (Prims.strcat
                   (Prims.strcat "Total compressed size "
                      (Prims.strcat
                         (Prims.string_of_int
                            (FStar_Int64.v md.total_compressed_size))
                         " does not match sum of page sizes "))
                   (Prims.strcat (Prims.string_of_int s) ""));
