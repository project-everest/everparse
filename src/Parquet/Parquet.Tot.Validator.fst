...
(** Helpers ----------------------------------------------------------- *)
let check (b:bool) (msg:string) : ML unit =
...
let rec ranges_in_bound (rs: list range) (max_len: int) : ML unit =
  match rs with
  | [] -> ()
  | r :: rest ->
    let end_ = range_end r in
    check (end_ <= max_len) (Printf.sprintf
      "Range end %d exceeds bound %d" end_ max_len);
...
let rec ranges_disjoint (rs: list range) : ML unit =
  match rs with
  | [] | [_] -> ()
  | x :: xs -> 
    check (List.Tot.for_all (disjoint x) xs) (Printf.sprintf
      "Ranges %d-%d overlap with another range" x.start (range_end x));
    ranges_disjoint xs
  // List.Tot.for_all (disjoint x) xs && ranges_disjoint xs
...
val validate_column_chunk: column_chunk -> ML unit
...
let validate_column_chunk cc =
  let offsets_ok =
    ...
    | Some cmd ->
      ...
      | None, _ -> true
      | Some dict_off, None -> 
        check (I64.v dict_off < data_off) (Printf.sprintf
          "Dictionary page offset must be less than data page offset, got %d >= %d" (I64.v dict_off) data_off);
        I64.v dict_off < data_off
      | Some dict_off, Some idx_off -> 
        check (I64.v dict_off < data_off) (Printf.sprintf
          "Dictionary page offset must be less than data page offset, got %d >= %d" (I64.v dict_off) data_off);
        check (I64.v dict_off < I64.v idx_off) (Printf.sprintf
          "Dictionary page offset must be less than index page offset, got %d >= %d" (I64.v dict_off) (I64.v idx_off));
        I64.v dict_off < data_off && I64.v dict_off < I64.v idx_off
  in
  ...
  bool =
    ...
    | None, None, Some _, _ | None, None, _, Some _ ->
      check false "ColumnIndex is present without OffsetIndex";
      false
    | Some _, None, _, _ ->
      check false "OffsetIndex is present without length";
      false
    | None, Some _, _, _ ->
      check false "OffsetIndex length is present without offset";
      false
    | _, _, Some _, None ->
      check false "ColumnIndex offset is present without length";
      false
    | _, _, None, Some _ ->
      check false "ColumnIndex length is present without offset";
      false
  in
  ()
  // offsets_ok && idx_ok
...
let rec columns_are_contiguous (prev: column_chunk) (cols: list column_chunk)
  ...
  | cc :: rest ->
    ...
    | Some (prev_md: column_meta_data), Some cc_md ->
      ...
      check (prev_offset + prev_size = cc_offset) (Printf.sprintf
        "Non-contiguous column chunks: previous offset %d + size %d != current offset %d"
...
val validate_row_group: row_group -> ML unit
...
let validate_row_group rg =
  ...
  let offset_ok =
    ...
    | Some off ->
      ...
      | None -> true
      | Some col_off -> 
        check (I64.v off = I64.v col_off) (Printf.sprintf
          "Row group file offset %d does not match column chunk offset %d" (I64.v off) (I64.v col_off));
        I64.v off = I64.v col_off
  in
  let rg_size_ok =
    ...
    | Some sz ->
      ...
      | None -> true
      | Some total_sz -> 
        check (I64.v sz = total_sz) (Printf.sprintf
          "Row group total compressed size %d does not match sum of column chunk sizes %d" (I64.v sz) total_sz);
        total_sz = I64.v sz
  in
  let contiguous_ok =
  ...
  in
  let cols_ok =
    List.iter (fun cc -> validate_column_chunk cc) rg.columns (* each column chunk passes *)
  in
  ()
...
let rec page_offsets_are_distinct_and_inbound (page_offs: list int64) (bound: int64)
    : ML unit (decreases page_offs) =
  match page_offs with
  | [] -> ()
  | x :: xs ->
    check (not (List.Tot.contains x xs)) (Printf.sprintf
      "Duplicate page offset found: %d" (I64.v x));
    check (I64.v x < I64.v bound) (Printf.sprintf
      "Page offset %d is out of bounds, must be less than %d" (I64.v x) (I64.v bound));
...
val validate_file_meta_data: file_meta_data -> int64 -> ML unit
...
let validate_file_meta_data fmd footer_start =
  ...
  ranges_in_bound ranges (I64.v footer_start);
  List.iter (fun rg -> validate_row_group rg) fmd.row_groups
...
let rec page_offsets_are_contiguous (prev: page_location) (locs: list page_location)
    : ML unit (decreases locs) =
  match locs with
  | [] -> ()
  | loc :: rest ->
    check (I64.v loc.offset = I64.v prev.offset + I32.v prev.compressed_page_size) (Printf.sprintf
      "Page offset is not contiguous: %d != %d + %d"
...
val validate_offset_index (oi: offset_index) (cc: column_chunk) : ML unit

let validate_offset_index (oi: offset_index) (cc: column_chunk) : ML unit =
  ...
  let first_loc_ok =
    ...
    | Some loc, Some off -> 
      check (I64.v loc.offset = off) (Printf.sprintf
        "First page location offset %d does not match column chunk offset %d" (I64.v loc.offset) off);
      I64.v loc.offset = off
    | _ -> true
  in
  ...
  let cc_page_offsets_ok =
    ...
    | Some cmd -> begin
      (match cmd.dictionary_page_offset, cmd.index_page_offset with
        | Some dict_off, Some idx_off ->
          check (contains dict_off) (Printf.sprintf
            "Dictionary page offset %d is not listed in page locations" (I64.v dict_off));
          check (contains idx_off) (Printf.sprintf
            "Index page offset %d is not listed in page locations" (I64.v idx_off));
            ()
        | Some dict_off, None ->
          check (contains dict_off) (Printf.sprintf
            "Dictionary page offset %d is not listed in page locations" (I64.v dict_off));
      ...
      check (contains cmd.data_page_offset) (Printf.sprintf
        "Data page offset %d is not listed in page locations" (I64.v cmd.data_page_offset));
      ()
      end 
  in
  let col_size_ok =
    ...
    | Some md ->
      ...
      check (I64.v md.total_compressed_size = s) (Printf.sprintf
        "Total compressed size %d does not match sum of page sizes %d"
        (I64.v md.total_compressed_size) s);
