...
# not in any released F*.  The COSE snapshots in ../c and ../rust/src are
# Custard's output, but the .fst sources must keep building with a released
# F*, so the extraction backend is chosen from the compiler at hand rather
# than hardcoded.  Set CUSTARD explicitly to override.
#
