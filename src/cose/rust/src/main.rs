...
fn read_cose_key(path: PathBuf, buf: &mut Vec<u8>) -> Result<coseformat::evercddl_COSE_Key_OKP, Box<dyn std::error::Error>> {
    File::open(path)?.read_to_end(buf)?;
    match coseformat::validate_and_parse_COSE_Key_OKP(buf.as_slice()) {
        coseformat::option__·COSE_Format_evercddl_COSE_Key_OKP···Pulse_Lib_Slice_slice·uint8_t·::None =>
            Err(Box::new(NotACOSEKey{})),
        coseformat::option__·COSE_Format_evercddl_COSE_Key_OKP···Pulse_Lib_Slice_slice·uint8_t·::Some {v: (k, _)} =>
            Ok(k),
    }
}
    ...
    match cli.command {
        ...
        Some(Commands::Sign { payload, privkey, out_msg }) => {
            ...
            let privkey = match read_cose_key(privkey, &mut privkeybuf)? {
                coseformat::evercddl_COSE_Key_OKP { intkeyneg4: coseformat::option__COSE_Format_evercddl_bstr::Some {v: privkey}, .. } =>
                    privkey,
        ...
        Some(Commands::Verify { msg, pubkey }) => {
            ...
            let pubkey = match read_cose_key(pubkey, &mut pubkeybuf)? {
                coseformat::evercddl_COSE_Key_OKP { intkeyneg2: coseformat::option__COSE_Format_evercddl_bstr::Some {v: pubkey}, .. } =>
                    pubkey,
                _ => panic!("wrong key format"),
            };
