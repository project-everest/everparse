...
= let prf
  ...
  : Lemma
    (array_group3_concat a1 (array_group3_concat a2 a3) l ==
      array_group3_concat (array_group3_concat a1 a2) a3 l)
  = match a1 l with
    | None -> ()
    | Some (l1, lr1) ->
...
let array_group3_concat_unique_strong'_equiv
  #b (a1 a3: array_group3 b)
: Lemma
...
let array_group3_zero_or_more_equiv #b
...
= assert (array_group3_equiv a1 a2);
  let rec phi
    (l: list Cbor.raw_data_item { opt_precedes_list l b })
  : Lemma
    (ensures (array_group3_zero_or_more a1 l == array_group3_zero_or_more a2 l))
    (decreases (List.Tot.length l))
  = assert (a1 l == a2 l);
    match a1 l with
    | None -> ()
...
let array_group3_concat_unique_weak_intro
  #b (a1 a3: array_group3 b)
  (prf1:
...
let array_group3_concat_unique_strong'_concat_left
  (#b: _)
...
= let a1 = array_group3_concat g1 g2 in
  let a3 = g3 in
  let prf
    (l1 l2:
      (l: list Cbor.raw_data_item { opt_precedes_list l b })
...
= array_group3_concat_unique_strong'_concat_left g1 g2 g3

#restart-solver
let array_group3_concat_unique_strong_zero_or_one_left
  #b (a1 a2: array_group3 b)
: Lemma
  (requires (
    array_group3_disjoint a1 a2 /\
...
let array_group3_concat_unique_strong_one_or_more_left
  #b (a1 a2: array_group3 b)
: Lemma
  (requires (
    array_group3_disjoint a1 a2 /\
...
= array_group3_concat_unique_weak_zero_or_more_right a1 a1;
  array_group3_concat_unique_strong_zero_or_more_left a1 a2;
  array_group3_concat_unique_strong_concat_left a1 (array_group3_zero_or_more a1) a2

#pop-options

#restart-solver
let array_group3_concat_unique_weak_zero_or_one_left
  #b (a1 a2: array_group3 b)
: Lemma
  (requires (
    array_group3_disjoint a1 a2 /\
...
#restart-solver
let array_group3_concat_unique_weak_one_or_more_left
  #b (a1 a2: array_group3 b)
: Lemma
  (requires (
    array_group3_disjoint a1 a2 /\
...
= array_group3_concat_unique_weak_concat_zero_or_more_right a1 a1 a2;
  array_group3_concat_unique_weak_zero_or_more_left a1 a2;
  array_group3_concat_unique_weak_intro (array_group3_one_or_more a1) a2
      (fun l ->
        let Some (l1, lr) = a1 l in
        assert (array_group3_concat (array_group3_zero_or_more a1) a2 lr == Some (lr, []));
        let Some (l1s, l2) = array_group3_zero_or_more a1 lr in
        assert (array_group3_zero_or_more a1 l1s == Some (l1s, []));
        assert (a2 l2 == Some (l2, []));
        ()
      )
      (fun l1 l2 ->
        assert (Some? (a1 l1));
        assert (array_group3_zero_or_more a1 l1 == Some (l1, []));
        let l = l1 `List.Tot.append` l2 in
        assert (array_group3_concat (array_group3_zero_or_more a1) a2 l == Some (l, []));
        assert (Some? (a1 l));
        ()
      )
...
: Lemma
  (array_group3_equiv
    (close_array_group (array_group3_concat a1 a2))
    (array_group3_concat a1 (close_array_group a2))
  )
= ()
