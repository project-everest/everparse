...
= 
  ...
  assert (
      ...
      sp.mg_serializable m1' /\
      cbor_map_union l (sp.mg_serializer (map_of_list_snoc key_eq m1 k v)) == cbor_map_union (cbor_map_union l (sp.mg_serializer m1)) (cbor_map_singleton (sp1.serializer k) (sp2.serializer v)) /\
      cbor_map_length (sp.mg_serializer mkv) == 1 /\
...
(*
  ...
  Trade.refl (rel_map_iterator cbor_map_iterator_match (Iterator.mk_spec r1) c0 v);
  while (
    let res = !pres;
    if (res) {
...
(*
let map_of_list_append_op
  (#key #value: Type)
  (key_eq: EqTest.eq_test key)
  (m1 m2: Map.t key (list value))
  (accu: Map.t key (list value))
  (x: key)
: Tot (Map.t key (list value))
= match Map.get accu x with
  | Some _ -> accu
  | None ->
    begin match Map.get m1 x, Map.get m2 x with
    | None, None -> accu
    | Some y, None
    | None, Some y -> Map.union accu (Map.singleton x (key_eq x) y)
    | Some y1, Some y2 -> Map.union accu (Map.singleton x (key_eq x) (List.Tot.append y1 y2))
    end

let map_of_list_append_op_comm
  (#key #value: Type)
  (key_eq: EqTest.eq_test key)
  (m1 m2: Map.t key (list value))
  (accu: Map.t key (list value))
  (x1 x2: key)
: Lemma
  (map_of_list_append_op key_eq m1 m2 (map_of_list_append_op key_eq m1 m2 accu x1) x2 ==
    map_of_list_append_op key_eq m1 m2 (map_of_list_append_op key_eq m1 m2 accu x2) x1
  )
= assert (map_of_list_append_op key_eq m1 m2 (map_of_list_append_op key_eq m1 m2 accu x1) x2 `Map.equal`
    map_of_list_append_op key_eq m1 m2 (map_of_list_append_op key_eq m1 m2 accu x2) x1
  )
...
#lang-fstar
let set_fold #target #source #sp = Set.fold #target #source #sp
#lang-pulse

let map_of_list_append
  (#key #value: Type)
  (#tkey: typ)
  (key_eq: EqTest.eq_test key)
  (m1 m2: Map.t key (list value))
  (sp: (spec tkey key true) {
    (forall (x: key) . (Map.defined x m1 \/ Map.defined x m2) ==> sp.serializable x)
  })
: Tot (Map.t key (list value))
= set_fold (map_of_list_append_op key_eq m1 m2) (Map.empty _ _) (Set.union (Map.key_set sp m1) (Map.key_set sp m2))

let map_of_list_append_mem
  (#key #value: Type)
  (#tkey: typ)
  (key_eq: EqTest.eq_test key)
  (m1 m2: Map.t key (list value))
...
: Lemma
  (ensures (
    Map.get (map_of_list_append key_eq m1 m2 sp) x == begin match Map.get m1 x, Map.get m2 x with
    | None, None -> None
    | Some y1, None
  ...
  [SMTPat (Map.get (map_of_list_append key_eq m1 m2 sp) x)]
= 
  Classical.forall_intro_3 (map_of_list_append_op_comm key_eq m1 m2);
  let inv (s: Set.t sp) (accu: Map.t key (list value)) : Tot prop =
      Map.get accu x == begin if Set.mem x s then match Map.get m1 x, Map.get m2 x with
      | None, None -> None
      | Some y1, None
      | None, Some y1 -> Some y1
      | Some y1, Some y2 -> Some (List.Tot.append y1 y2)
  ...
  in
  let s = Set.union (Map.key_set sp m1) (Map.key_set sp m2) in
  Set.fold_inv
    inv
    (map_of_list_append_op key_eq m1 m2)
    (fun accu k s ->
      ()
    )
    (Map.empty _ _)
