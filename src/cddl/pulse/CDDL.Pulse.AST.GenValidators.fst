...
let "^wf^" = compute_wf_typ "^env^" (T.pull_name "^source^") (_ by (FStar.Tactics.trefl ())) (T.pull_type "^source^") (_ by (T.solve_mk_wf_typ_fuel_for ()))
let _ : unit = _ by (FStar.Tactics.print (\"wf'\"); FStar.Tactics.exact (`()))
...
let "^env'^" : wf_ast_env =
  wf_ast_env_extend_typ_with_weak "^env^" (T.pull_name "^source^") (T.pull_type "^source^") "^wf^"
let _ : unit = _ by (FStar.Tactics.print (\"venv'\"); FStar.Tactics.exact (`()))
[@@noextract_to "^krml^"; sem_attr] noextract
let v"^env'^" : Impl.validator_env Det.cbor_det_match "^env'^".e_sem_env =
  Impl.extend_validator_env_with_typ_weak v"^env^" (T.pull_name "^source^") () _ "^validator^"
...
[@@noextract_to "^krml^"; sem_attr] noextract
let "^env'^" : wf_ast_env =
  wf_ast_env_extend_group "^env^" (T.pull_name "^source^") (T.pull_group "^source^") (_ by (FStar.Tactics.norm [delta; iota; zeta; primops]; FStar.Tactics.trefl ())) (_ by (FStar.Tactics.norm [delta; iota; zeta; primops]; FStar.Tactics.trefl ()))
let _ : unit = _ by (FStar.Tactics.print (\"venv'\"); FStar.Tactics.exact (`()))
[@@noextract_to "^krml^"; sem_attr] noextract
let v"^env'^" : Impl.validator_env Det.cbor_det_match "^env'^".e_sem_env =
