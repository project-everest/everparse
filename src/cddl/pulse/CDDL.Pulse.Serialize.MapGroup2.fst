module CDDL.Pulse.Serialize.MapGroup2
#lang-pulse
...
= let sp = mg_zero_or_more_match_item sp1 sp2 except in
  ...
  map_of_list_is_append_serializable_disjoint sp1 sp2 except m1 m2 m;
...
= 
  ...
  map_of_list_is_append_serializable_disjoint sp1 sp2 except m1 mkv m1';
  if
        sp1.serializable k &&
        sp2.serializable v &&
...
#push-options "--z3rlimit 128 --print_implicits --split_queries always"

#restart-solver
let impl_serialize_map_group_valid_map_zero_or_more_snoc1
  (#key #value: Type)
  (#tkey: typ)
  (sp1: spec tkey key true)
  (key_eq: EqTest.eq_test key)
...
: Tot (squash (
    ...
    sp.mg_serializable m1 /\
      ...
      sp2.serializable v /\
      (~ (except (sp1.serializer k, sp2.serializer v))) /\
      sp.mg_serializable m2 /\ (~ (Map.defined k m2)) /\
      sp.mg_serializable m2' /\
...
= impl_serialize_map_group_valid_map_zero_or_more_snoc_aux sp1 key_eq sp2 except l m1 k v m2 len;
  ...
  map_of_list_is_append_serializable_intro sp1 sp2 except m1 mkv m1';
  map_of_list_is_append_serializable_disjoint sp1 sp2 except m1 mkv m1';
  assert (sp1.serializable k);
  assert (sp2.serializable v);
  assert (~ (except (sp1.serializer k, sp2.serializer v)));
  assert (sp.mg_serializable m2);
  assert (~ (Map.defined k m2));
  assert (sp.mg_serializable m2');
  ...
  then begin
    assert (~ (cbor_map_disjoint (cbor_map_union l (sp.mg_serializer m1)) (sp.mg_serializer m2')));
    assert False // (~ (impl_serialize_map_group_valid (cbor_map_union l (sp.mg_serializer m1)) sp m2' len))
  end else begin
//  assert (FStar.UInt.fits (cbor_map_length l + cbor_map_length (sp.mg_serializer m1) + 1) 64);
    ()
  end

#restart-solver
let impl_serialize_map_group_valid_map_zero_or_more_snoc2
  (#key #value: Type)
  (#tkey: typ)
  (sp1: spec tkey key true)
  (key_eq: EqTest.eq_test key)
...
= impl_serialize_map_group_valid_map_zero_or_more_snoc_aux sp1 key_eq sp2 except l m1 k v m2 len;
  impl_serialize_map_group_valid_map_zero_or_more_snoc1 sp1 key_eq sp2 except l m1 k v m2 len ();
  let sp = mg_zero_or_more_match_item sp1 sp2 except in
  let m2' = map_of_list_cons key_eq k v m2 in
  ...
  assert (~ (cbor_map_defined (sp1.serializer k) (sp.mg_serializer m1)));
  map_of_list_is_append_snoc key_eq m1 k v;
  map_of_list_is_append_serializable_intro sp1 sp2 except m1 mkv m1';
  map_of_list_is_append_serializable_disjoint sp1 sp2 except m1 mkv m1';
  map_of_list_is_append_serializable_singleton sp1 sp2 except k (key_eq k) v;
  map_of_list_is_append_serializable_elim sp1 sp2 except mkv m2 m2';
  map_of_list_is_append_serializable_disjoint sp1 sp2 except mkv m2 m2';
    ...
    then begin
      ...
      then ()
        ...
        then begin
          ...
          assert (cbor_map_disjoint (sp.mg_serializer m1) (sp.mg_serializer m2') <==> cbor_map_disjoint (sp.mg_serializer m1) (sp.mg_serializer m2));
          map_of_list_is_append_snoc key_eq m1 k v;
          assume (cbor_map_disjoint (sp.mg_serializer m1') (sp.mg_serializer m2) <==> cbor_map_disjoint (sp.mg_serializer m1) (sp.mg_serializer m2));
          assert (cbor_map_disjoint (cbor_map_union l (sp.mg_serializer m1)) (sp.mg_serializer m2') <==> cbor_map_disjoint (cbor_map_union l (sp.mg_serializer m1')) (sp.mg_serializer m2));
          ...
          then begin
            ...
            let lm2 = cbor_map_length (sp.mg_serializer m2) in
            impl_serialize_map_group_valid_map_zero_or_more_snoc_length ll lm1 1 lm2;
            assume (cbor_map_length (cbor_map_union l (sp.mg_serializer m1)) + cbor_map_length (sp.mg_serializer m2') == cbor_map_length (cbor_map_union l (sp.mg_serializer m1')) + cbor_map_length (sp.mg_serializer m2));
            assert (cbor_map_length (cbor_map_union l (sp.mg_serializer m1)) + cbor_map_length (sp.mg_serializer m2') >= cbor_map_length (cbor_map_union l (sp.mg_serializer m1)) + 1);
            let sl = Seq.length (Cbor.cbor_det_serialize_map l) in
            let sm1 = Seq.length (Cbor.cbor_det_serialize_map (sp.mg_serializer m1)) in
            let sk = Seq.length (Cbor.cbor_det_serialize (sp1.serializer k)) in
...
//        assert (pure (w' == Seq.append (Seq.slice w 0 (SZ.v size0)) (Seq.append (Seq.slice w1 0 (SZ.v sz1)) w2)));
        ...
        if (sz2 = 0sz) {
          ...
          assert (pure (Seq.equal vl (Seq.append (CBOR.Spec.API.Format.cbor_det_serialize (sp1.serializer ke)) Seq.empty)));
