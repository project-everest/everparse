...
let fold_of_unique
  (#key: Type)
  (dom: (key -> bool))
  (dom_ex: squash (nonempty (enum_of dom)))
  (#accu: Type)
  (f1 f2: fold_of dom accu)
: Lemma
  (ensures (f1 == f2))
= let l : enum_of dom = nonempty_elim (enum_of dom) in
...
let key_set #key #key_s spec_key #value m =
  ...
  let l = Ghost.hide (nonempty_elim (enum_of m.dom)) in
...
let union #key #value m1 m2 =
  ...
  let l1 : Ghost.erased (enum_of m1.dom) = nonempty_elim (enum_of m1.dom) in
  let l2 : Ghost.erased (enum_of m2.dom) = nonempty_elim (enum_of m2.dom) in
  Util.union_as_list l1 l2 m2.dom;
  let l : Ghost.erased (enum_of dom) = List.Tot.append (List.Tot.filter (Util.notp m2.dom) l1) l2 in
  {
    dom = dom;
    dom_ex = nonempty_intro (Ghost.reveal l);
...
let filter #key #value f m =
  ...
  let l : Ghost.erased (enum_of m.dom) = nonempty_elim (enum_of m.dom) in
