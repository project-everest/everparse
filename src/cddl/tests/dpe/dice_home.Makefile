...
dice.done:
	test -d "$(dice_fstar)" || { rm -rf "$(dice_fstar)".tmp && git clone --no-checkout --filter=blob:none $(FStar_repo) "$(dice_fstar)".tmp && cd "$(dice_fstar)".tmp && git sparse-checkout init --no-cone && git sparse-checkout set --no-cone '!/*' '!/.*' && git sparse-checkout add pulse/share/pulse/examples/dice && git checkout $(FStar_hash) && cd .. && mv "$(dice_fstar)".tmp "$(dice_fstar)"; }
	touch $@
