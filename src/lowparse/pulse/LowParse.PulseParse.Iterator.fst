...
ensures
  (if res then
    exists* (i': iterator t) (pm': perm) (suffix: Ghost.erased (list u))
            (i_spare': Ghost.erased (iterator t)) (pm_sing: perm) .
      R.pts_to pi i' **
      iterator_match vmatch p pm' i' (Ghost.reveal v :: Ghost.reveal suffix) **
      Trade.trade
        (iterator_match vmatch p pm' i' (Ghost.reveal v :: Ghost.reveal suffix))
        (iterator_match vmatch p pm i l **
         R.pts_to spare i_spare' **
         base_iterator_match vmatch p pm_sing (Singleton (1.0R /. pm_sing) (pm_v /. pm_sing) elem_ref) [Ghost.reveal v]) **
      pure (List.Tot.sorted lt_ord (Ghost.reveal v :: Ghost.reveal suffix))
  else
    R.pts_to pi i **
    iterator_match vmatch p pm i l **
    R.pts_to spare i_spare **
...
{
  // Read pi to get concrete iterator value
  let saved_i = !pi;
  rewrite (iterator_match vmatch p pm i l)
  ...
  if is_empty {
    ...
    rewrite (iterator_match vmatch p pm saved_i l)
         as (iterator_match vmatch p pm i l);
    iterator_insert_before vmatch vmatch_share vmatch_gather p pm pi spare elem_ref pm_v;
    true
  } else {
    rewrite (iterator_match vmatch p pm saved_i l)
         as (iterator_match vmatch p pm i l);

    // Advance: get head element
    let head = iterator_next vmatch vmatch_share vmatch_gather p sq j rd pm pi;

    // Read elem_ref to get concrete value (elem is ghost, can't pass to stt)
    ...
    // Compare: is elem < head?
    let b_lt = lt_impl elem_val head;
    let b_gt = lt_impl head elem_val;
    if b_lt {
      // elem < head: insert before the original position
      // Elim advance trade to restore original iterator_match
      Trade.elim
        (iterator_match vmatch p pm_next i_next (List.Tot.tl l) **
         vmatch pm_v_next head (List.Tot.hd l))
        (iterator_match vmatch p pm i l);

      // Restore pi to original value
      R.forget_init pi;
      pi := saved_i;
      ...
      if b_gt {
        // head < elem: recurse on tail
        let res = iterator_insert_sorted vmatch vmatch_share vmatch_gather p sq j rd
          lt_ord lt_impl pm_next pi spare elem_ref pm_v;
        if res {
          // Recursive call succeeded: compose trades
          with i_rec pm_rec suffix_rec i_spare_rec pm_sing_rec . assert (
            R.pts_to pi i_rec **
            iterator_match vmatch p pm_rec i_rec (Ghost.reveal v :: Ghost.reveal suffix_rec) **
            Trade.trade
              (iterator_match vmatch p pm_rec i_rec (Ghost.reveal v :: Ghost.reveal suffix_rec))
              (iterator_match vmatch p pm_next i_next (List.Tot.tl l) **
               R.pts_to spare i_spare_rec **
               base_iterator_match vmatch p pm_sing_rec (Singleton (1.0R /. pm_sing_rec) (pm_v /. pm_sing_rec) elem_ref) [Ghost.reveal v])
          );

          // Compose the inner trade (from recursive insert) with the advance trade
          intro (iterator_match vmatch p pm_rec i_rec (Ghost.reveal v :: Ghost.reveal suffix_rec) @==>
                 iterator_match vmatch p pm i l **
                 R.pts_to spare i_spare_rec **
                 base_iterator_match vmatch p pm_sing_rec (Singleton (1.0R /. pm_sing_rec) (pm_v /. pm_sing_rec) elem_ref) [Ghost.reveal v])
            #(Trade.trade
              ...
              Trade.trade
                ...
                (iterator_match vmatch p pm i l))
            fn _ {
              // Elim inner trade: get iter(tail) ** spare ** singleton
              Trade.elim
                (iterator_match vmatch p pm_rec i_rec (Ghost.reveal v :: Ghost.reveal suffix_rec))
                (iterator_match vmatch p pm_next i_next (List.Tot.tl l) **
                 R.pts_to spare i_spare_rec **
              ...
              // Elim advance trade: iter(tail) ** vmatch(head) -> iter(l)
              Trade.elim
                (iterator_match vmatch p pm_next i_next (List.Tot.tl l) **
                 vmatch pm_v_next head (List.Tot.hd l))
                (iterator_match vmatch p pm i l) ;
          ...
          // Recursive call returned None (duplicate found)
          // Elim advance trade
          Trade.elim
            (iterator_match vmatch p pm_next i_next (List.Tot.tl l) **
             vmatch pm_v_next head (List.Tot.hd l))
        ...
        // Neither lt v head nor lt head v: duplicate
        // Elim advance trade
        Trade.elim
          (iterator_match vmatch p pm_next i_next (List.Tot.tl l) **
           vmatch pm_v_next head (List.Tot.hd l))
