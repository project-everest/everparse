...
```pulse
ghost
fn slice_append_split_trade_aux
...
noextract
```pulse
fn slice_append_split_trade (#t: Type) (mutb: bool) (input: S.slice t) (#p: perm) (#v1 #v2: Ghost.erased (Seq.seq t)) (i: SZ.t)
    requires S.pts_to input #p (v1 `Seq.append` v2) ** pure (slice_append_split_precond mutb p v1 i)
    returns res: S.slice_pair t
    ensures slice_append_split_trade_post input p v1 v2 i res
{
  ...
  match res {
    S.SlicePair input1 input2 -> {
      ...
      intro_trade _ _ _ (slice_append_split_trade_aux input p v1 v2 i input1 input2);
      fold (slice_append_split_trade_post' input p v1 v2 i input1 input2);
      fold (slice_append_split_trade_post input p v1 v2 i (S.SlicePair input1 input2));
      (S.SlicePair input1 input2)
    }
  }
}
...
noextract
```pulse
fn slice_split_trade (#t: Type) (mutb: bool) (s: S.slice t) (#p: perm) (#v: Ghost.erased (Seq.seq t)) (i: SZ.t)
    requires S.pts_to s #p v ** pure (S.split_precond mutb p v i)
    returns res: S.slice_pair t
    ensures slice_split_trade_post s p v i res
{
  ...
  match res { S.SlicePair s1 s2 -> {
    ...
    intro_trade _ _ _ (slice_split_trade_aux s p v i s1 s2 v1 v2 ());
    fold (slice_split_trade_post' s p v i s1 s2);
    fold (slice_split_trade_post s p v i (S.SlicePair s1 s2));
    (S.SlicePair s1 s2)
  }}
}
```
