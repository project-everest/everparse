...
= fun #rrel #rel input pos ->
    ...
    then
      ...
      then v
        ...
        let z = read_from_valid_slice read_u8 input pos in
        if z `U8.lt` 128uy
        then validator_error_generic
        else v
    else
      ...
      then
        ...
        then v
          ...
          let y = read_from_valid_slice (read_bounded_integer_2 ()) input pos in
          if y `U32.lt `256ul
          then validator_error_generic
          else v
      else if len = 3uy
      then
        ...
        then v
          ...
          let y = read_from_valid_slice (read_bounded_integer_3 ()) input pos in
          if y `U32.lt `65536ul
          then validator_error_generic
          else v
      else
        ...
        then v
          ...
          let y = read_from_valid_slice (read_bounded_integer_4 ()) input pos in
...
= fun #rrel #rel input pos ->
    let h = HST.get () in
    [@inline_let] let _ =
      bvalid_facts (parse_der_length_payload32 x) h input pos;
      parse_der_length_payload32_unfold x (bytes_of_buffer_from h input pos);
      assert_norm (pow2 (8 * 1) == 256);
      assert_norm (pow2 (8 * 2) == 65536);
      assert_norm (pow2 (8 * 3) == 16777216);
      assert_norm (pow2 (8 * 4) == 4294967296)
    ...
    then pos
      ...
      let len = x `U8.sub` 128uy in
      [@inline_let] let _ =
        bvalid_facts parse_u8 h input pos;
        parser_kind_prop_equiv parse_u8_kind parse_u8;
        bvalid_facts (parse_bounded_integer (U8.v len)) h input pos;
...
= fun #rrel #rel input pos ->
    ...
    then
      [@inline_let] let _ = bvalid_facts parse_u8 h input pos in
      ...
      let len = x `U8.sub` 128uy in
      [@inline_let] let _ = bvalid_facts (parse_bounded_integer (U8.v len)) h input pos in
      if len = 2uy
      then
...
= fun #rrel #rel input pos ->
    ...
    then v
      ...
      let x = read_from_valid_slice read_u8 input pos in
      ...
      then validator_error_generic
        ...
        then v2
          ...
          let y = read_from_valid_slice (read_der_length_payload32 x) input v in
...
= fun #rrel #rel input pos ->
    ...
    [@inline_let]
    let _ =
      bvalid_facts (parse_bounded_der_length32 (vmin) (vmax)) h input pos;
      parse_bounded_der_length32_unfold (vmin) (vmax) (bytes_of_buffer_from h input pos);
      bvalid_facts parse_u8 h input pos
    in
    let v = jump_u8 input pos in
    let x = read_u8 input pos in
    let len = der_length_payload_size_of_tag8 x in
    [@inline_let] let _ = bvalid_facts (parse_der_length_payload32 x) h input v in
