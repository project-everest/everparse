...
module LowParse.Writers.NoHoare.Effect
include LowParse.Writers.Parser
open LowParse.Writers.Effect
...
| ValidSynth:
  ...
  (v: valid_rewrite_t p1 p2 (fun _ -> True) f) ->
...
let tvalid_rewrite_of_evalid_rewrite
  ...
  (v: valid_rewrite_t p1 p2 precond f { forall (x: Parser?.t p1) . precond x })
...
let evalid_rewrite_of_tvalid_rewrite
  ...
  (#p2: parser)
  (v: squash (valid_rewrite_prop p1 p2))
: Tot (valid_rewrite_t p1 p2 (fun _ -> True) (evalid_rewrite_of_tvalid_rewrite_f v))
