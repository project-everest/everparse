...
abstract let bytes_equal (b1 b2: bytes) : GTot Type0 =
...
abstract let bytes_equal_intro (b1 b2: bytes) : Lemma
  (requires (
    Seq.length b1 == Seq.length b2 /\
...
abstract let bytes_equal_refl (b1 b2: bytes) : Lemma
  (requires (b1 == b2))
  (ensures (b1 `bytes_equal` b2))
  [SMTPat (bytes_equal b1 b2)]
= ()

abstract let bytes_equal_elim (b1 b2: bytes) : Lemma
