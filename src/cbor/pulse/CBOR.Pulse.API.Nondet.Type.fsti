...
(* Abstract type of the scratch cells the application must pre-allocate
   (as references) to call cbor_nondet_map_entry_insert. A fixed number (two)
   of such references is required per insertion (plus one
   cbor_nondet_map_entry_t reference). The type is kept abstract; use
   CBOR.Pulse.API.Nondet.C.dummy_cbor_nondet_map_entry_insert_cell to
   initialize the references. *)
