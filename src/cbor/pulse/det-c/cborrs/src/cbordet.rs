...
#[derive(PartialEq)] struct slice_pair__uint8_t <'a> { pub fst: &'a [u8], pub snd: &'a [u8] }
...
fn read_header(input: &[u8]) -> header
{
    ...
    let _letpattern: slice_pair__uint8_t =
        {
            let actual_pair: (&[u8], &[u8]) = input.split_at(i);
            slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
        };
    let s1: &[u8] = _letpattern.fst;
    let s2: &[u8] = _letpattern.snd;
...
fn validate_header(input: &[u8], poffset: &mut [usize]) -> bool
{
    ...
    let is_valid1: bool =
        if is_valid
        {
            ...
            let _letpattern: slice_pair__uint8_t =
                {
                    let actual_pair: (&[u8], &[u8]) = input.split_at(offset2);
                    slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                };
            let s1: &[u8] = _letpattern.fst;
            let s2: &[u8] = _letpattern.snd;
            ...
            let s1s2: slice_pair__uint8_t =
                {
                    let actual_pair: (&[u8], &[u8]) = input23.split_at(consumed);
                    slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                };
            let s10: &[u8] = s1s2.fst;
            let s20: &[u8] = s1s2.snd;
    ...
    if is_valid1
    {
        ...
        let _letpattern: slice_pair__uint8_t =
            {
                let actual_pair: (&[u8], &[u8]) = input.split_at(offset1);
                slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
            };
        let s1: &[u8] = _letpattern.fst;
        let s2: &[u8] = _letpattern.snd;
            ...
            if x0.major_type == cbor_major_type_simple_value
            {
                ...
                if is_valid0
                {
                    ...
                    let _letpattern0: slice_pair__uint8_t =
                        {
                            let actual_pair: (&[u8], &[u8]) = input.split_at(offset20);
                            slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                        };
                    let s11: &[u8] = _letpattern0.fst;
                    let s21: &[u8] = _letpattern0.snd;
                    ...
                    let s1s20: slice_pair__uint8_t =
                        {
                            let actual_pair: (&[u8], &[u8]) = input230.split_at(consumed0);
                            slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                        };
                    let s12: &[u8] = s1s20.fst;
                    let s22: &[u8] = s1s20.snd;
                    let res2: slice_pair__uint8_t = slice_pair__uint8_t { fst: s12, snd: s22 };
                    let left0: &[u8] = res2.fst;
                    let right0: &[u8] = res2.snd;
...
fn jump_header(input: &[u8], offset: usize) -> usize
{
    ...
    let _letpattern: slice_pair__uint8_t =
        {
            let actual_pair: (&[u8], &[u8]) = input.split_at(offset);
            slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
        };
    let s1: &[u8] = _letpattern.fst;
    let s2: &[u8] = _letpattern.snd;
...
fn validate_recursive_step_count_leaf(a: &[u8], bound: usize, prem: &mut [usize]) -> bool
{
    ...
    let _letpattern: slice_pair__uint8_t =
        {
            let actual_pair: (&[u8], &[u8]) = a.split_at(i);
            slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
        };
    let s1: &[u8] = _letpattern.fst;
    let s2: &[u8] = _letpattern.snd;
    ...
    cond
    {
        ...
        else
        {
            ...
            else
            {
                ...
                let _letpattern: slice_pair__uint8_t =
                    {
                        let actual_pair: (&[u8], &[u8]) = input.split_at(off);
                        slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                    };
                let s1: &[u8] = _letpattern.fst;
                let s2: &[u8] = _letpattern.snd;
...
fn jump_raw_data_item(input: &[u8], offset: usize) -> usize
{
    ...
    cond
    {
        ...
        let left0: &[u8] = res1.fst;
        let right0: &[u8] = res1.snd;
    ...
    <'a>
{
    ...
    let _letpattern: slice_pair__uint8_t =
        {
            let actual_pair: (&[u8], &[u8]) = i.split_at(i1);
            slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
        };
    let s1: &[u8] = _letpattern.fst;
    let s2: &[u8] = _letpattern.snd;
    ...
    let input10: &[u8] = res0.fst;
    let input20: &[u8] = res0.snd;
...
fn impl_deterministically_encoded_cbor_map_key_order(a1: &[u8], a2: &[u8]) -> bool
{
    ...
    let _letpattern: slice_pair__uint8_t =
        {
            let actual_pair: (&[u8], &[u8]) = a1.split_at(i);
            slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
        };
    let s1: &[u8] = _letpattern.fst;
    let s2: &[u8] = _letpattern.snd;
    ...
    let _letpattern0: slice_pair__uint8_t =
        {
            let actual_pair: (&[u8], &[u8]) = a2.split_at(i0);
            slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
        };
    let s10: &[u8] = _letpattern0.fst;
    let s20: &[u8] = _letpattern0.snd;
...
fn cbor_raw_sorted(a: &[u8]) -> bool
{
    ...
    if get_header_major_type(h) == cbor_major_type_map
    {
        ...
        else
        {
            ...
            let _letpattern0: slice_pair__uint8_t =
                {
                    let actual_pair: (&[u8], &[u8]) = ap.split_at(i0);
                    slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                };
            let s10: &[u8] = _letpattern0.fst;
            let s20: &[u8] = _letpattern0.snd;
            ...
            let s11: &[u8] = pl.fst;
            let s21: &[u8] = pl.snd;
            let mut phd: [&[u8]; 1] = [s11; 1usize];
            let mut ptl: [&[u8]; 1] = [s21; 1usize];
            ...
            cond
            {
                ...
                let _letpattern1: slice_pair__uint8_t =
                    {
                        let actual_pair: (&[u8], &[u8]) = stl.split_at(i2);
                        slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                    };
                let s110: &[u8] = _letpattern1.fst;
                let s210: &[u8] = _letpattern1.snd;
                let res4: slice_pair__uint8_t = slice_pair__uint8_t { fst: s110, snd: s210 };
                let input12: &[u8] = res4.fst;
                let input22: &[u8] = res4.snd;
                let res5: slice_pair__uint8_t = slice_pair__uint8_t { fst: input12, snd: input22 };
                let input13: &[u8] = res5.fst;
                let input23: &[u8] = res5.snd;
...
fn cbor_validate_det·(input: &[u8]) -> usize
{
    ...
    else
    {
        let _letpattern: slice_pair__uint8_t =
            {
                let actual_pair: (&[u8], &[u8]) = input.split_at(0usize);
                slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
            };
        let s1: &[u8] = _letpattern.fst;
        let s2: &[u8] = _letpattern.snd;
        ...
        cond
        {
            ...
            let _letpattern0: slice_pair__uint8_t =
                {
                    let actual_pair: (&[u8], &[u8]) = pi.split_at(i);
                    slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                };
            let s11: &[u8] = _letpattern0.fst;
            let s21: &[u8] = _letpattern0.snd;
            ...
            else
            {
                ...
                let _letpattern1: slice_pair__uint8_t =
                    {
                        let actual_pair: (&[u8], &[u8]) = pi.split_at(0usize);
                        slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                    };
                let s12: &[u8] = _letpattern1.fst;
                let s22: &[u8] = _letpattern1.snd;
                ...
                let s13: &[u8] = s1s20.fst;
                let s23: &[u8] = s1s20.snd;
                let res10: slice_pair__uint8_t = slice_pair__uint8_t { fst: s13, snd: s23 };
                let left0: &[u8] = res10.fst;
                let right0: &[u8] = res10.snd;
                let split230: slice_pair__uint8_t = slice_pair__uint8_t { fst: left0, snd: right0 };
                let input·: &[u8] = split230.fst;
                ...
                let _letpattern2: slice_pair__uint8_t =
                    {
                        let actual_pair: (&[u8], &[u8]) = pi.split_at(i0);
                        slice_pair__uint8_t { fst: actual_pair.0, snd: actual_pair.1 }
                    };
                let s14: &[u8] = _letpattern2.fst;
                let s24: &[u8] = _letpattern2.snd;
                let res12: slice_pair__uint8_t = slice_pair__uint8_t { fst: s14, snd: s24 };
                let input111: &[u8] = res12.fst;
                let input21: &[u8] = res12.snd;
        ...
        else
        {
            ...
            cond0
            {
                ...
                let input11: &[u8] = res3.fst;
                let input2: &[u8] = res3.snd;
                let res4: slice_pair__uint8_t = slice_pair__uint8_t { fst: input11, snd: input2 };
                let input110: &[u8] = res4.fst;
                let input20: &[u8] = res4.snd;
                let spl: slice_pair__uint8_t = slice_pair__uint8_t { fst: input110, snd: input20 };
                let res5: &[u8] = spl.fst;
                let px: &[u8] = res5;
