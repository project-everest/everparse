...
```pulse
ghost fn cbor_raw_match_aux_cases
...
ensures cbor_raw_match_aux parse_raw_data_item r pm x y ** pure (cbor_raw_match_cases_prop x y)
{
...
ensures cbor_raw_match pm x y ** pure (cbor_raw_match_cases_prop x y)
{
  ...
  cbor_raw_match_aux_cases cbor_raw_match pm x;
...
```pulse
fn cbor_raw_get_major_type_aux
  (r: perm -> cbor_raw -> raw_data_item -> slprop)
  (pm: perm) (x: cbor_raw) (#y: Ghost.erased raw_data_item)
requires cbor_raw_match_aux parse_raw_data_item r pm x y
returns res: major_type_t
ensures cbor_raw_match_aux parse_raw_data_item r pm x y ** pure (res == get_major_type y)
{
...
{
  cbor_raw_match_unfold_aux x;
  let res = cbor_raw_get_major_type_aux cbor_raw_match pm x;
  cbor_raw_match_fold_aux x;
  res
}
```
...
requires
  cbor_raw_match_content r parse_raw_data_item pm h xl c
ensures pure False
{
  let b = dfst h;
  let la = dsnd h;
  ...
  rewrite
    (cbor_raw_match_content r parse_raw_data_item pm h xl c)
    as
    (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) xl c);
  match xl {
    CBOR_Case_String v -> {
      rewrite
        (cbor_raw_match_content r parse_raw_data_item pm
  ...
  pure (String? (Ghost.reveal y) /\ v == (String?.v (Ghost.reveal y) <: Seq.seq U8.t))
{
  ...
  match x {
    CBOR_Case_String v -> {
      cbor_raw_get_string_content r pm
        (dfst (synth_raw_data_item_recip (Ghost.reveal y)))
        v
...
{
  cbor_raw_match_unfold_aux x;
  let s = cbor_raw_get_string_aux cbor_raw_match pm x ();
  // Build a trade cbor_raw_match_aux ... -> cbor_raw_match
  intro
    (trade
      (cbor_raw_match_aux parse_raw_data_item cbor_raw_match pm x y)
  ...
  // Compose the two trades
  with _pm' _vs . assert (S.pts_to s #_pm' _vs);
  Trade.trans
    (S.pts_to s #_pm' _vs)
    (cbor_raw_match_aux parse_raw_data_item cbor_raw_match pm x y)
    (cbor_raw_match pm x y);
    ...
    (cbor_raw_match pm (CBOR_Case_String str) xh)
{
  ...
  cbor_raw_get_string_content cbor_raw_match pm
    (dfst (synth_raw_data_item_recip (Ghost.reveal xh)))
    str
    (dsnd (synth_raw_data_item_recip (Ghost.reveal xh)))
    ();
...
requires
  cbor_raw_match_content r parse_raw_data_item pm h (CBOR_Case_Tagged v) c
ensures exists* (vl: cbor_raw).
  R.pts_to v.cbor_tagged_ptr #(pm *. v.cbor_tagged_ref_perm) vl **
  r (pm *. v.cbor_tagged_payload_perm) vl (content_as_raw_data_item (dfst h) (dsnd h) c) **
  trade
    (R.pts_to v.cbor_tagged_ptr #(pm *. v.cbor_tagged_ref_perm) vl **
     r (pm *. v.cbor_tagged_payload_perm) vl (content_as_raw_data_item (dfst h) (dsnd h) c))
    (cbor_raw_match_content r parse_raw_data_item pm h (CBOR_Case_Tagged v) c)
{
  ...
  let la = dsnd h;
  header_eta h;
  cbor_raw_match_content_eq_tagged r parse_raw_data_item pm b la v c;
  rewrite
    (cbor_raw_match_content r parse_raw_data_item pm h (CBOR_Case_Tagged v) c)
    as
    (cbor_tagged_content_slprop r pm v (content_as_raw_data_item b la c));
  unfold (cbor_tagged_content_slprop r pm v (content_as_raw_data_item b la c));
  unfold (vmatch_ref
    (fun (vl: cbor_raw) (vh: raw_data_item) ->
      r (pm *. v.cbor_tagged_payload_perm) vl vh)
  ...
  intro
    (trade
      (R.pts_to v.cbor_tagged_ptr #(pm *. v.cbor_tagged_ref_perm) vl **
       r (pm *. v.cbor_tagged_payload_perm) vl (content_as_raw_data_item b la c))
      (cbor_raw_match_content r parse_raw_data_item pm h (CBOR_Case_Tagged v) c))
    #emp
    fn _ {
      fold (vmatch_ref
        ...
        ({ v = v.cbor_tagged_ptr; p = pm *. v.cbor_tagged_ref_perm })
        (content_as_raw_data_item b la c));
      fold (cbor_tagged_content_slprop r pm v (content_as_raw_data_item b la c));
      cbor_raw_match_content_eq_tagged r parse_raw_data_item pm b la v c;
      rewrite
        (cbor_tagged_content_slprop r pm v (content_as_raw_data_item b la c))
        as
  ...
  pure False
{
  ...
  match xl {
    CBOR_Case_Invalid -> {
      rewrite
        (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) CBOR_Case_Invalid c)
        as (pure False)
    }
    CBOR_Case_Int i -> {
      rewrite
        (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Int i) c)
        as (pure False)
    }
    CBOR_Case_Simple sv -> {
      rewrite
        (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Simple sv) c)
        as (pure False)
    }
    CBOR_Case_String s -> {
      rewrite
        (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_String s) c)
        as (pure False)
    }
    CBOR_Case_Array a -> {
      rewrite
        (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Array a) c)
        as (pure False)
    }
    CBOR_Case_Map m -> {
      rewrite
        (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Map m) c)
        as (pure False)
    }
    CBOR_Case_Tagged t -> {
      rewrite
        (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Tagged t) c)
        as (pure False);
      unreachable ()
    }
    CBOR_Case_Tagged_Serialized ts -> {
      rewrite
        (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Tagged_Serialized ts) c)
        as (pure False);
      unreachable ()
    }
  }
...
fn cbor_raw_get_tagged_payload_aux
  (r: perm -> cbor_raw -> raw_data_item -> slprop)
  (pm: perm)
  (reader: PPB.zero_copy_parse_strong_prefix #cbor_raw #raw_data_item
    (r pm) #parse_raw_data_item_kind parse_raw_data_item)
...
ensures exists* (pm': perm) (payload: Ghost.erased raw_data_item).
  r pm' res payload **
  Trade.trade (r pm' res payload) (cbor_raw_match_aux parse_raw_data_item r pm x y) **
        ...
        Ghost.reveal payload == Tagged?.v (Ghost.reveal y))
{
  ...
  match x {
    ...
    CBOR_Case_Tagged_Serialized ts -> {
      ...
      // Compose: r pm res _ → pts_to_parsed → cbor_raw_match_aux pm x y
      Trade.trans
        (r pm res
          (content_as_raw_data_item
            (dfst (dfst (synth_raw_data_item_recip (Ghost.reveal y))))
...
{
  cbor_raw_match_unfold_aux x;
  let res = cbor_raw_get_tagged_payload_aux cbor_raw_match pm (cbor_raw_read pm f64) x ();
  // Build trade cbor_raw_match_aux ... → cbor_raw_match
  intro
    (trade
      (cbor_raw_match_aux parse_raw_data_item cbor_raw_match pm x y)
      (cbor_raw_match pm x y))
  ...
  with _pm' _vpl . assert (cbor_raw_match _pm' res _vpl);
  Trade.trans
    (cbor_raw_match _pm' res _vpl)
    (cbor_raw_match_aux parse_raw_data_item cbor_raw_match pm x y)
    (cbor_raw_match pm x y);
  res
}
...
requires
  cbor_raw_match_content r parse_raw_data_item pm h (CBOR_Case_Array v) c
  ...
  pure (pm' == pm *. v.cbor_array_slice_perm /\ l == content_as_list_raw (dfst h) (dsnd h) c)
{
  ...
  let la = dsnd h;
  header_eta h;
  cbor_raw_match_content_eq_array r parse_raw_data_item pm b la v c;
  rewrite
    (cbor_raw_match_content r parse_raw_data_item pm h (CBOR_Case_Array v) c)
    as
    (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Array v) c);
  rewrite
    (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Array v) c)
    as
    (I.mixed_list_match
        ...
        Ghost.reveal l == Array?.v (Ghost.reveal y))
{
  ...
  match x {
    CBOR_Case_Array v -> {
      cbor_raw_get_array_content r pm
        (dfst (synth_raw_data_item_recip (Ghost.reveal y)))
        v
        (dsnd (synth_raw_data_item_recip (Ghost.reveal y)))
        ();
      ...
      intro
        (trade
          (cbor_raw_match_content r parse_raw_data_item pm
            ...
            (CBOR_Case_Array v)
            (dsnd (synth_raw_data_item_recip (Ghost.reveal y))))
          (cbor_raw_match_aux parse_raw_data_item r pm (CBOR_Case_Array v) y))
    ...
    CBOR_Case_Invalid -> {
      cbor_raw_get_array_content_false r pm
        (dfst (synth_raw_data_item_recip (Ghost.reveal y)))
        CBOR_Case_Invalid
        (dsnd (synth_raw_data_item_recip (Ghost.reveal y)))
        ();
...
{
  cbor_raw_match_unfold_aux x;
  let res = cbor_raw_get_array_aux cbor_raw_match pm x ();
  // Build trade cbor_raw_match_aux ... → cbor_raw_match
  intro
    (trade
      (cbor_raw_match_aux parse_raw_data_item cbor_raw_match pm x y)
...
requires
  cbor_raw_match_content r parse_raw_data_item pm h (CBOR_Case_Map v) c
ensures exists* (pm': perm) (l: list (raw_data_item & raw_data_item)).
  I.mixed_list_match
    (fun (pm0: perm) (elem: cbor_map_entry cbor_raw) (vi: (raw_data_item & raw_data_item)) ->
      cbor_map_entry_match r pm0 elem vi)
  ...
  pure (pm' == pm *. v.cbor_map_slice_perm /\ l == content_as_list_pair (dfst h) (dsnd h) c)
{
  ...
  let la = dsnd h;
  header_eta h;
  cbor_raw_match_content_eq_map r parse_raw_data_item pm b la v c;
  rewrite
    (cbor_raw_match_content r parse_raw_data_item pm h (CBOR_Case_Map v) c)
    as
    (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Map v) c);
  rewrite
    (cbor_raw_match_content r parse_raw_data_item pm (| b, la |) (CBOR_Case_Map v) c)
    as
    (I.mixed_list_match
...
ensures exists* (pm': perm) (l: Ghost.erased (list (raw_data_item & raw_data_item))).
  I.mixed_list_match
    (fun (pm0: perm) (elem: cbor_map_entry cbor_raw) (v: (raw_data_item & raw_data_item)) ->
      cbor_map_entry_match r pm0 elem v)
    (nondep_then parse_raw_data_item parse_raw_data_item)
    pm'
    res
    l **
        ...
        Ghost.reveal l == Map?.v (Ghost.reveal y))
{
  ...
  match x {
      ...
      intro
        (trade
          (cbor_raw_match_content r parse_raw_data_item pm
            ...
            (CBOR_Case_Map v)
            (dsnd (synth_raw_data_item_recip (Ghost.reveal y))))
          (cbor_raw_match_aux parse_raw_data_item r pm (CBOR_Case_Map v) y))
    ...
    CBOR_Case_Invalid -> {
      cbor_raw_get_map_content_false r pm
        (dfst (synth_raw_data_item_recip (Ghost.reveal y)))
        CBOR_Case_Invalid
        (dsnd (synth_raw_data_item_recip (Ghost.reveal y)))
        ();
...
{
  cbor_raw_match_unfold_aux x;
  let res = cbor_raw_get_map_aux cbor_raw_match pm x ();
  // Build trade cbor_raw_match_aux ... → cbor_raw_match
  intro
    (trade
      (cbor_raw_match_aux parse_raw_data_item cbor_raw_match pm x y)
