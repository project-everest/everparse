...
int test1(void) {
  A destA;
  copy_buffer_t _cb_a_out = { .cb_base = (uint8_t*)&destA, .cb_len = sizeof(destA), .cb_pos = 0 };
  EVERPARSE_COPY_BUFFER_T a_out = (EVERPARSE_COPY_BUFFER_T) &_cb_a_out;
  B64 destB;
  copy_buffer_t _cb_b_out = { .cb_base = (uint8_t*)&destB, .cb_len = sizeof(destB), .cb_pos = 0 };
  EVERPARSE_COPY_BUFFER_T b_out = (EVERPARSE_COPY_BUFFER_T) &_cb_b_out;
