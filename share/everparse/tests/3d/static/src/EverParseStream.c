...
/* Peep: a pointer to the next n bytes, without consuming them, or NULL if they
   are not contiguous in a single cell. Only `static`/`extern` provide this;
   it is what backs the field_ptr_after action. */
uint8_t *EverParseStreamPeep(EVERPARSE_INPUT_STREAM_BASE const x, size_t n) {
  struct es_cell *head = x->head;
